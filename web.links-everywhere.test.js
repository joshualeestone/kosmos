...
test('direct messages link a URL (dmRow), both directions', () => {
  const fn = new Function('CURRENT', 'let LAST = []; let YOU_PIC = false; let DISC_TINTS = ["#dfe5ea"]; let DISC_INKS = ["#4a5560"];\n' + lift(['esc', 'pjInline', 'pjPreviewCard', 'pjSize', 'pjWords', 'pjFiles', 'pjAttachmentCards', 'pjFileWord', 'pjAttachmentCard', 'pjWhen', 'pjWhenPart', 'pjWhenLive', 'pjSentence', 'placedWords', 'pjVerdict', 'dmWho', 'pjAvatarVer', 'discTint', 'discInk', 'discIndex', 'initials', 'dmRow', 'pjRich', 'pjRichSpans', 'pjListDepth']) + '\nreturn dmRow;')(DANA);
...
test('a project message row links a URL (pjMsg)', () => {
  const src = lift(['esc', 'pjInline', 'pjPreviewCard', 'pjSize', 'pjWords', 'pjFiles', 'pjAttachmentCards', 'pjFileWord', 'pjAttachmentCard', 'pjWhen', 'pjWhenPart', 'pjWhenLive', 'pjSentence', 'placedWords', 'pjVerdict', 'pjMsg', 'pjRich', 'pjRichSpans', 'pjListDepth']);
...
test('no render site escapes a message text directly any more', () => {
  ...
  // text goes through an inline renderer -- pjInline for the room row, and
  // pjRich (its superset: same links + escaping, plus #2067 markdown) for the
  // talk thread and the project message row -- never esc(text) directly.
  for (const fn of ['dmRow', 'pjMsg', 'pjRoomRow']) {
    const body = page.lift(SCRIPT, fn);
    assert.doesNotMatch(body, /esc\((r|m)\.text\)/, fn + ' draws message text with esc() instead of pjInline()/pjRich()');
  }
});
...
test('a message that is only its attachment\'s name draws the card once, not the name twice (#358)', () => {
  ...
  assert.doesNotMatch(only, /<\/b>lease notes\.txt/, 'the file name is drawn as the message words above its own card');
  const words = fn({ from: 'dana', text: 'here is the lease', at: new Date().toISOString(), attachment: att }, 'Dana');
  assert.match(words, /<\/b>here is the lease/, 'real words were dropped because a file came with them');
});
...
test('a message with several files draws every card, and hides the joined names the same way (#420)', () => {
  ...
  assert.doesNotMatch(both, /<\/b>one\.txt, two\.pdf/, 'the joined names are drawn as words above their own cards');
  const captioned = fn({ from: 'dana', text: 'both of these', at, attachment: a, attachments: [a, b] }, 'Dana');
  assert.match(captioned, /<\/b>both of these/, 'the caption was dropped');
  ...
  assert.doesNotMatch(old, /<\/b>one\.txt/);
});
