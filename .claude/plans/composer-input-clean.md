...
## Test
- New browser-check `render-composer-stroke.js`: renders a seeded project one-view, asserts `#pj-post` and `#pj-say` composerbox border-width is 0 AND that each keeps a VISIBLE boundary (a non-transparent fill distinct from the bar behind it, the #3369 lesson) in tab AND consolidated layouts, both themes; CONTROL asserts `#d-say` (agent dialogue) keeps its border, proving the scoping. Reds on origin/main. 24 assertions pass.
