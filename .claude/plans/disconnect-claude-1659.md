...
## 🛑 THE ITERATION LEDGER IS NOT IN THIS FILE, AND THAT IS THE POINT

It lives in `disconnect-claude-1659-ledger.md`, and a blind reviewer must NOT be
sent there.

**I put it here first, and that was a method error I want on the record.** My
reviewer prompts say "read the plan file", and this file had grown 309 lines of
prior findings. So iterations 18 onward were handed the review history the
challenge loop exists to keep them from seeing. The skill is explicit that the
orchestrator is the sole holder of the ledger, because a reviewer who reads prior
findings confirms them rather than discovering new ones.

⭐ **What makes it worth recording rather than quietly fixing: it did NOT stop
those passes finding new defects, including defects in the immediately preceding
iteration's fix.** So the contamination weakened the independence claim without
emptying the results. Both halves are true and the proof file says so.

**This file should carry DECISIONS, REJECTED ALTERNATIVES and ACCEPTED GAPS.**
That is context a reviewer needs to judge intent. A list of what previous
reviewers found is not.
