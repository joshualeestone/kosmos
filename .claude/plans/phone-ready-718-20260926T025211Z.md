...
- Iteration 2: deferred, not reachable or not visible: the hub clamp can invert only on a chart under ~188px (the
  floor draws at least (120 * 0.7 + 42) * 2 = 252); a detached ORG_LIVE lingers inertly after a failed poll
  mid-drag until the pointer is released.
- Iteration 3: a repaint between pointerdown and the first 4px of movement rebuilt the nodes, and the drag went
  on steering a detached copy; pointermove now drops a drag whose node is no longer on the page (unit pin; the
  window is too narrow to hit reliably from a browser check). The "capture lost ends the drag" comment claimed
  more than the listener does (a removed node's lost capture goes to the document), and was deleted. The focus
  hand-off uses preventScroll; the orgBoxPlain comment sits above orgBoxPlain again.
