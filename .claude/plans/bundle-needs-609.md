...
## A failed cut leaves the site checkout claiming a version it never served (folded in, Splinter 23:05)

Cut 4 of 0.5.24 died at step 7 and left `dist/latest.json` (0.5.24) and `setup.sha256` uncommitted in the site checkout, plus the versioned tarball pair that made cut 5 refuse ("cache-immutable"). A stray site deploy in that state would publish a `latest.json` naming a version that does not exist. `release_site_restore <site> <version> <pair-existed-before>` (in `tools/lib/release-freeze.sh`) puts the two tracked files back to their committed bytes, removes the versioned pair when this cut created it, and never touches `versions.html` (hand-written; the re-cut needs it). The cut's EXIT trap calls it unless step 8 finished (`DEPLOYED=1`). Cases in `tools/test-release-detached.sh`: a temp site repo; the two files come back, the pair goes, versions.html stays, it says what it did; a pair that existed before the cut is left alone; a clean checkout is left silent. The pkg triple (3c) is deliberately not restored: its guard is keyed on inputs and the red branches print the note.
