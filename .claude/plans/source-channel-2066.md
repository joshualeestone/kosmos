...
- `store.ROOT` mirrors `engine/store.js dataRootFor` for this macOS-only installer:
  `$AGENT_WORKFORCE_DATA/AgentWorkforce` when the override is set (the sandbox the tests seed), else
  `~/Library/Application Support/AgentWorkforce`. So the write lands exactly where the read side looks.
