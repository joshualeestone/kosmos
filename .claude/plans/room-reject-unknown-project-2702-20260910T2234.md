...
- **Reject shape, matching siblings:**
  ...
  - JSON (the web): `sendJson(res, 404, { error: 'there is no project by that name' })`. Shape note (the siblings are not uniform): the CLI-facing `/api/post` and `/api/react` return `200` with an error-shaped body, and the operator room-react route 404s with `{ok:false, because}`; the `{error}` shape used here is the one the real web consumer reads -- `loadRoom()` in web/index.html keys on `body.error` -- so match that, not a single "canonical sibling".
