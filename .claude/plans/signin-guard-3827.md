...
## Round 19 review (sonnet)
- WARNING: a standing refresh that straddled a Forget AND a new sign-in wrote the old identity's standing onto the new one (the guard checked only forgetting / enrolled). It now records mac_id before the fetch and writes nothing if it changed. Test (the fetcher swaps the mac_id and answers "good": standing stays ""); control fails by name.
- WARNING: macRequest (the standing and federation polls) was a signed call outside Forget's wait. It is tracked now; it carries its own 20s bound, so the wait stays bounded. Test (a mac-request that finishes inside the bound records "done" before the retire); control (untracked) retires first.
- WARNING: abandonChangedIdentity dropped the certificate whenever the mac_id changed, but the tunnel keeps a certificate across a register at the same ADDRESS (certificate_survives). It now drops it only when the address changed; the tunnel is still stopped on a new mac_id. No dedicated test: with the code as it is, a failed register at the same address needs a Mac that is not enrolled but keeps its address and certificate (a missing tls.key, say), and the enrolled same-name case is taken by the shortcut first. The changed-address case keeps its test from round 18.

## Round 20 review (opus)
- WARNING: a Sign out during a rename whose certificate step then failed went through cancelledAfter, which never called abandonChangedIdentity. The new id stayed beside the old certificate, and a later Turn on ran them together. cancelledAfter now gets addressBefore and does the same clean-up on a failure. Test (a slow write-then-fail fixture, Sign out while it is out): certificate gone, no tunnel on Turn on. Control fails by name.
- WARNING: macRequest (phone-notification turn-on, federation, the standing poll) was neither refused during a register or a Forget nor covered if it started after Forget's snapshot, so it could sign with a key being replaced or retired. macRequest now refuses while busy(), and the standing refresh skips while busy(). Test (refused with "being forgotten" during a Forget); control fails by name.
- CONVENTION (accepted): the wizard's new "Kosmos+ is off, as you set it" branch and its 7-minute register timeout have no browser check. The route field is asserted in server.test.js, and the commits carry the Browser-check trailer. A browser assertion for the branch belongs with the next change to render-plus-signin-3478.
- CONVENTION: Forget's Promise.race could not fire (every tracked call carries its own kill bound, no longer than the retire bound), so it was untestable. It is removed; Forget awaits allSettled, which the per-call bounds keep bounded. The contradictory comment went with it.
