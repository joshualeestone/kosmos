...
2. **Route**: `GET /api/agent/:name/account-status` (on-demand, matching the existing `/api/agent/:name/...` detail-fetch pattern). Returns `{account, configDir, connected: bool|unknown, because, remedy}`. NEVER prints the credential value. Fails soft (UNKNOWN, never a false "connected").
