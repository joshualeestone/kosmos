...
- Two jobs, least privilege each (review iteration 3). `browser-checks-full` holds only
  ...
  and every check run there. Its checks step has its own timeout (120 minutes since iteration 5): a STEP timeout
...
- tools/browser-checks.sh also prints `FAILED-LIST:  a|b (why)|c`: some entries contain
  spaces ("regress-a-night (server did not boot)", a multi-check composite), so the
  space-joined FAILED: line could not be re-split. The collector reads FAILED-LIST, and the
  card splits entries on "|" only, so "NEW since the last red night" names whole entries.
