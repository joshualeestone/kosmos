...
- `docs/browser-checks/render-worlds-switcher-1704.js` (self-booting, own sandbox registry, in
  ...
  active-marked; Create disabled-then-enabled; create closes the modal, the switcher menu
  re-opens on its own, and the new world appears on refetch; the duplicate-name error path
  shows the server reason; switcher hidden in consolidated. 14/14 PASS. Reds on the pre-#1704
  page (switcher markup absent -> the first wait times out -> exit 1).
