...
- **Same recognized-name rule as the message.** `pjMentionHighlightHTML(text, keys)` mirrors the
  posted-message tokenizer (`pjRichSpans`) exactly, and a differential browser-check arm pins the two
  equal so they can never disagree. A partial/unknown `@name` stays plain; it turns blue only when the
  word is a project key, or the word with a trailing `._-` run stripped is (so `@mona-` flags `mona`);
  leading `_` is not peeled, `*~(` are. Keys come from `mentionCandidates`, the exact set the `@`
  picker offers. Both UI surfaces tokenize on whitespace, so they APPROXIMATE `engine/messages.js`
  rather than matching it byte for byte: the backend flags an `@name` after any non-identifier char
  mid-token (`foo/@mona`), which neither surface highlights. That is the safe direction (the UI
  under-signals, never falsely promises delivery), is inherited unchanged from part 1, and is tracked
...
- render-mention-blue-2922.js: all arms pass (chromium+webkit, light+dark) incl. mirror-metrics-parity,
  colour-only weight, transparent-text/inked-caret, the pre-`.mention-live` inked control, and a
  DIFFERENTIAL arm running shared fixtures through both pjMentionHighlightHTML and pjRichSpans to pin
  the live and posted @mention classification equal.
...
- Rendered-rect alignment (mirror rect == textarea rect, dx=dy=dw=0, both themes) was measured
  MANUALLY during development in a force-laid-out composer; it is NOT a committed CI check, because a
  bare file:// load does not lay out the project room. CI guards it indirectly through the computed
  font-size/line-height/padding parity arm (the CSS-metric proxy), which is what keeps the mirror on
  the caret by construction.
- Full suite green (unrelated tests flake under concurrent box load; each confirmed passing in isolation).
