...
- The Kosmos+ navy look (`body.plus-active`) pins `--warn-bg` and `--warn-border` to their dark
  ...
  values are near invisible on navy. The roadmap needs-you ring is pinned to its dark value there
  too. A fourth check arm (light Mac, navy look) asserts the unsure message takes the dark tint and
  the roadmap row takes the dark ring.
