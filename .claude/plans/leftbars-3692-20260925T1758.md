...
- Neutral notes: `border: 0.5px solid var(--border-strong); border-radius: var(--radius-control)`,
  ...
  sunken background. The even padding makes `.dmoff` and `.rolelimit` 16px taller.
...
- The Kosmos+ navy look (`body.plus-active`) pins `--warn-bg` and `--warn-border` to their dark
  values, next to #3724's `--warn-ink` pin: a warning is now marked by those two, and their light
