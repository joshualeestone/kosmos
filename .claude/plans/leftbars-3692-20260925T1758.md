...
- **Fixed here (13):** `.pj-replying` (the composer's "Replying to" strip, added on main by #3745 after the card was written), `.svc-door`, `.dmoff`, `.msg-valve`, `.pj-warn`,
...
- **New on main since the card, left as a quote:** `.msg-replyto` (#3745), the line inside a reply
  that shows which message it answers. It is a quote, the same call as `.msg .quoteb`. Sonya's call,
  reversible; raised with Liu Kang.
...
## Replacements
- Neutral notes: `border: 0.5px solid var(--border-strong); border-radius: var(--radius-control)`,
  with the padding made even on both sides. `--border-strong`, not `--separator`: a hairline is a
  quarter of a 2px bar's width, and `--separator` (the same colour as the old `--k-rule` bars) is too
  faint alone (the measured contrast note by `--border-strong` in the tokens). `.note` also keeps its
  sunken background. The even padding makes `.dmoff` and `.rolelimit` 16px taller and `.msg-valve`
  12px taller; `.pj-warn`, `.pj-folder-state.bad` and `.pj-question` gain vertical padding too.
...
- **The project tree lines stay** (`.pj-row.child` indent guide, and the roadmap `::before`
  ...
  **Decided by Liu Kang (m899, 2026-09-25): the tree lines stay, as structure. Josh may overrule.**
- **`.detail-said` keeps its quote rule** (the agent's own words in the detail view). It is a quote,
  like `.mdq` and `.msg .quoteb`, so it joins the allow-list with that reason and is unchanged from
  main. **Decided by Liu Kang (m899, 2026-09-25). Josh may overrule.**
...
- `docs/browser-checks/render-no-left-bars-3692.js`: it builds each fixed element on a sealed board in
  ...
  The first version passed the roadmap row vacuously: `#pj-list` carried `.asgrid`, so the
...
- The check now also proves the replacement exists: each fixed element carries an even border, a
  ...
  themes, and dropping the explicit dark ring rule fails the explicit arm. A fourth arm (below)
  covers the navy look.
- `.detail-said`: see Decided. Liu Kang ruled it a quote, so it is back to main's rule.
...
- In the Kosmos+ navy look (`body.plus-active`), the changed elements themselves take the dark
  values of `--warn-bg`, `--warn-border` and `--border-strong` (one rule, scoped to those elements,
  so the tokens' other users across the app are unchanged), in the spirit of #3724's `--warn-ink` pin: a warning is now marked by those two, and their light
  values are near invisible on navy. So are `--border-strong` (the neutral hairline; its light
  value on navy is about the navy itself) and the roadmap needs-you ring, both pinned to their dark
  values there. A fourth check arm (light Mac, navy look) asserts the unsure message takes the dark tint, a
  neutral hairline the dark colour, and the roadmap row the dark ring.
  It also checks every changed element on navy against its dark value for what the rule pins,
  so an element dropped from the scoped list fails by name (control: dropping `.pj-question`).
...
visual check; the roadmap needs-you row is measured in the real `#pj-list`, which the sheet covers,
so the ring is proven by its computed style rather than seen in the screenshot.
