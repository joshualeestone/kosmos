...
- Neutral notes: `border: 0.5px solid var(--separator); border-radius: var(--radius-control)`, with
  the padding made even on both sides. `.rolelimit` and `.note` keep the stronger `--border-strong`
  their bars used, since `--separator` is too faint on its own (the contrast note by `--border-strong`
  in the tokens). `.note` also keeps its sunken background.
...
- The Kosmos+ navy look (`body.plus-active`) pins `--warn-bg` and `--warn-border` to their dark
  ...
  values are near invisible on navy. A fourth check arm (light Mac, navy look) asserts the unsure
  message takes the dark tint there.
