...
- Settings-screen external doors: the project-settings "Project members" screen (#pjs-*) still shows disabled external-invite doors with a "later version" tooltip that this PR makes imprecise (the capability now exists in the create flow). Wiring those doors is a SEPARATE surface (inviting to an already-created project, arguably the more common case) and is out of scope for this Add-Project-screen card; tracked as a fast-follow, with that tooltip copy to be softened when it lands. This PR intentionally does not touch those pre-existing lines.
