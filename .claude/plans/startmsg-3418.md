...
- Part 7 wait-for + assertion: `/could not start/i` becomes `/has not come back/i`,
...
- Part 7 comment plus the 'ghost' fixture setup comment updated to describe the
  post-#3418 bootstrap-then-timeout path instead of the retired refusal.
- Plan file added.

## Decisions
- The mocked-refusal arms (Parts 3/3b) legitimately still say "could not start" and
  are deliberately left unchanged. Those arms fully MOCK /restart (page.route
  fulfilling a 400 `outcome:'refused'`), so no server code runs at all; the frontend
  sees `restartTook` false from the mocked 400 and takes the "could not start" path.
  That is a different situation from Part 7's real bootstrap-then-readiness-timeout.
- OUT OF SCOPE (tracked as a follow-up): the `d-start-wrap` comment in
  web/index.html (around 8601-8604) still frames #3418 as pending ("only once Angel's
  #3418 makes the FOUND.NONE branch bootstrap"). That is product code, would trip
  the surface gate and re-run the browser suite, and is unrelated to this test fix.
