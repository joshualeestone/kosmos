...
2. **`engine/selfcheck.js`** (new) -- the after-install verifier:
   ...
   - `fetchLatestJson` / `fetchManifestNamed`: fetch `latest.json` (throw if it has no
     `manifest` pointer) and fetch a named manifest (throw if unreachable) -- failing loudly
     so an absent/unreachable manifest never resolves to a vacuous pass.
   - `selfCheck({base, root, doFetch})`: reads the INSTALLED version (`app/package.json`) and
     ...
     `behind` reports the merely-behind state distinctly, and an `installed` boolean lets the
     CLI exit 1 for an installed-but-unverifiable machine but 0 for a from-source checkout.
     Cross-checks that the fetched manifest declares the installed version.
   - `reportLines(r)`: a pure function rendering the CLI output + exit code, tested directly.
