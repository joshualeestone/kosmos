...
- `export DEVELOPER_DIR=/Library/Developer/CommandLineTools` in each script: it works (measured for
  ...
  toolchain for everything the script runs. The prefer-system selector keeps CI byte-identical and
  touches only the interpreter choice.
...
## When neither interpreter runs
On a stock Mac without Homebrew, python3 on PATH IS the broken shim, so the fallback cannot help.
Each script re-probes the chosen interpreter and exits 1 at once with one line naming the cause
("FAIL  no runnable python3 ... #3578"), instead of failing later as "local server did not start".
