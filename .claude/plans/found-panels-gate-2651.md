...
- `docs/browser-checks/render-discovery-gate-2651.js` (new): hermetic file://, stubs /api/found-agents + /api/scan-agents, forces the Agents tab, drives the real paint functions. Five arms: (1) ON LOAD both panels hidden + trigger shown; (2) EXPLICIT PRESS opens both (found and scan each asserted) + hides the trigger; (3) DISMISSED FOREVER does not re-offer the trigger and never misreports the dismissal; (4) LABEL RESET restores the neutral label when the trigger re-shows after an empty look; (5) LABEL WRITE GUARDED - a no-change re-show does not rewrite the button text node (so its aria-live does not re-announce every poll). The load arm alone would pass on a page that never shows the panels, so the press arm is load-bearing. Proven can-fail: removing the `DISCOVERY_OPENED` gate reds arm 1; a scan-only regression reds arm 2's scan assertion; a wrongly-tracked dismissed flag reds arm 3; a dropped label reset reds arm 4; an unconditional label write reds arm 5.
