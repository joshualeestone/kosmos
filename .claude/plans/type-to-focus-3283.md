...
- `docs/browser-checks/render-type-to-focus-3283.js` (headless, self-spawning
  ...
  is not hijacked). Also a no-composer negative arm (a keystroke on the Agents
  board, where activeComposer() returns null, focuses nothing) and a WEBKIT arm
  (the product renders in WKWebView, and the char-retarget is engine-sensitive;
  verified the mechanism lands the char in WebKit too, not just Chromium).
  Proven-can-fail: neutering the handler reds the focus/land arms while setup +
  negative stay green.
