...
- engine/trust.js grows the side record (trust-writes.json under
  ...
  leave-the-line signal; a write over a corrupt record refuses rather
  than clobbering entries it could not read; displaced-absent is kept
...
- remove.js consumes it before the board-record step: a record means the
  ...
  only on success, so a failed take-back retries at the next removal.
