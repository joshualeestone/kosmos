...
check(Shell.loadFailure(domain: NSURLErrorDomain, code: NSURLErrorNetworkConnectionLost) == .offline, "connection lost: offline")
...
func link(_ s: String) -> Shell.LinkDecision { Shell.linkDecision(for: URL(string: s)!, coordinator: coordinator) }
...
check(link("https://billing.stripe.com/p/session/x") == .external, "Stripe's billing page: Safari")
check(link("https://kosmosplus.com/help") == .external, "the marketing site: Safari (not a Mac, not the coordinator)")
check(link("https://evil.example.com/") == .external, "another site: Safari, never inside the app")
check(link("https://login.kosmosplus.com.evil.example/") == .external, "a lookalike of the coordinator: Safari")
check(link("http://login.kosmosplus.com/") == .external, "plain http, even to Kosmos+: not loaded in the app")
check(link("https://a.b.kosmosplus.com/") == .external, "two labels deep: not a Mac, Safari")
...
check(link("data:text/html,hi") == .block, "data: refused")
check(link("kosmos://open") == .block, "a custom scheme: refused")
check(link("about:blank") == .inApp, "about:blank: allowed (WebKit uses it)")
