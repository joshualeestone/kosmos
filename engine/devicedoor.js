'use strict';
/**
 * A Connections door with a real flow (#529): a service's own CLI, its own
 * device flow, its own keyring. GitHub was the first; the shape is the same
 * for every service whose CLI signs in this way, so it lives here once.
 *
 ...
 * to ask it who is signed in, how to start its device flow, and what the
 * code and URL look like when it prints them. github.js and vercel.js are
 * specs; nothing else differs between them.
 */
...
function ghBin() {
  ...
  if (process.env[spec.binEnv]) return runnable(process.env[spec.binEnv]) ? process.env[spec.binEnv] : null;
  return spec.candidates.find(runnable) || null;
}
...
function childEnv() {
  // A sandbox can point the tool at its own config so a probe never touches
  // the operator's real sign-in; the product leaves it unset.
  const env = { ...process.env };
  if (spec.configEnv && process.env[spec.configEnv] && spec.configVar) env[spec.configVar] = process.env[spec.configEnv];
  return env;
}
function configArgs() {
  return (spec.configEnv && process.env[spec.configEnv] && spec.configArg) ? [spec.configArg, process.env[spec.configEnv]] : [];
}
...
const CODE_RE = spec.codeRe;
const URL_RE = spec.urlRe;
...
function start() {
  return new Promise((resolve) => {
    if (mem.phase === PHASE.STARTING || mem.phase === PHASE.AWAITING || mem.phase === PHASE.COMPLETING) {
      readState((s) => resolve({ ...s, refused: 'a ' + spec.serviceName + ' sign-in is already in progress' }));
      return;
    }
    const bin = ghBin();
    ...
    const onText = (chunk) => {
      ...
      if (mem.phase === PHASE.STARTING) {
        ...
        if (c) { mem.code = c[1]; mem.url = u ? u[1] : spec.deviceUrl; mem.phase = PHASE.AWAITING; }
      }
    };
    try {
      child = make();
    } catch (err) {
      mem = { ...mem, phase: PHASE.FAILED, because: 'we could not start ' + spec.toolName + ': ' + String((err && err.message) || err) };
      child = null;
      readState(resolve);
      return;
    }
    ...
    child.on('error', (err) => {
      mem = { ...mem, phase: PHASE.FAILED, because: spec.toolName + ' could not run: ' + String((err && err.message) || err) };
      child = null;
    });
    child.on('exit', (code) => {
      child = null;
      ...
      if (code === 0) {
        ...
        status((s) => { mem = { phase: PHASE.IDLE, code: null, url: null, because: s.connected ? null : spec.serviceName + ' said the sign-in finished, but ' + spec.toolName + ' does not show an account', startedAt: null }; });
      } else {
        mem = { ...mem, phase: PHASE.FAILED, because: 'the ' + spec.serviceName + ' sign-in did not finish' + (buf.trim() ? ': ' + buf.trim().split('\n').pop().slice(0, 160) : '') };
      }
    });
    readState(resolve);
  });
