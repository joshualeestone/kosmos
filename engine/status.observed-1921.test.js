...
test('a WORKING named-ours pane records an OK observation, an AUTH_FAILED one records 401', () => {
  ...
  assert.equal((observed.read(observed.PROVIDER.ANTHROPIC, 'worka') || {}).outcome, observed.OUTCOME.OK,
    'a working agent did not record an observed ok: ' + JSON.stringify(observed.all()));
  assert.equal((observed.read(observed.PROVIDER.ANTHROPIC, 'workb') || {}).outcome, observed.OUTCOME.REJECTED,
...
test('#1889: a pane WAITING ON A BACKGROUND AGENT records no observed OK', () => {
  ...
  assert.equal((observed.read(observed.PROVIDER.ANTHROPIC, 'bgwait') || {}).outcome, undefined,
  ...
  assert.equal((observed.read(observed.PROVIDER.ANTHROPIC, 'ordinary') || {}).outcome, observed.OUTCOME.OK,
...
test('an IMPOSTOR pane (a stranger under a name) records NOTHING, even scraping a 401', () => {
  ...
  assert.equal(observed.read(observed.PROVIDER.ANTHROPIC, 'stranger'), null,
...
test('a CODEX agent WORKING (no witnessed completion) records NOTHING -- not a Claude obs, not an OpenAI obs', () => {
  ...
  //
  // 🛑 #2413/#2790: the codex arm is ALSO positive-only AND keyed off a WITNESSED
  // ROLLOUT COMPLETION, never pane WORKING -- because a dead-credential 401 reconnect
  // loop scrapes as WORKING too and greening it would false-green a dead sign-in (#874).
  // This pane is merely WORKING with no rollout completion, so it records neither an
  // ANTHROPIC observation (the original cross-provider guarantee) NOR an OpenAI one.
  ...
  assert.equal(observed.read(observed.PROVIDER.ANTHROPIC, 'codexer'), null,
    'a working codex agent tainted a Claude account badge: ' + JSON.stringify(observed.all()));
  assert.equal(observed.read(observed.PROVIDER.OPENAI, 'codexer'), null,
    'a codex pane merely scraping WORKING (no witnessed rollout completion) recorded an OpenAI ok: ' + JSON.stringify(observed.all()));
});
...
test('a fresh SELF-REPORTED working (a claim, not witnessed) does NOT record ok -- green needs a WITNESSED scrape', () => {
  ...
  assert.equal(observed.read(observed.PROVIDER.ANTHROPIC, 'claimer'), null,
...
test('an IDLE tick does not clobber a prior real observation', () => {
  observed.saw(observed.PROVIDER.ANTHROPIC, 'worka', observed.OUTCOME.OK, Date.now() - 1000);
