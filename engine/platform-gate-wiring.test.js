...
test('firstrun.state() carries the platform facts, matching platform.describe()', async () => {
  ...
  /* No user-facing copy leaked into the wire shape -- only machine facts.
     `runnerDownloads` was the THIRD fact (#570's gate split, e3870c49): win32 gained
     `supported` so agents run, while the codex runner DOWNLOAD stays darwin-only.
     `claudeDownloads` is the FOURTH (#3159): Claude Code publishes a win32 build, so its
     download gate diverges from the codex one -- win32 is true here, false for
     runnerDownloads. describe() reports all four. Kept as an explicit list rather than
     derived from describe(), because its job is to notice a NEW key: a fifth one,
     especially a copy string, has to be looked at rather than waved through. */
  assert.deepEqual(Object.keys(s.platform).sort(), ['claudeDownloads', 'platform', 'runnerDownloads', 'supported']);
});
