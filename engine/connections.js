...
function blockBody() {
  return [
    ...
    'Subscription), and it signs in with Google in the browser: Kosmos runs',
    'Antigravity\'s sign-in out of sight and the person pastes the code Google',
    'shows into Kosmos. On the screen it is "Google Gemini (Google subscription)".',
    'It has no key and no Kosmos account folder. Once Kosmos has seen it signed',
    'in, `GET /api/accounts` lists it as one row with `authMode` `antigravity`',
    '(from Kosmos\'s last check, not a live one); `GET /api/antigravity` says',
