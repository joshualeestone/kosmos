...
function blockBody() {
  return [
    ...
    'Gemini on a Google subscription runs on Google\'s Antigravity, on a Mac (not',
    'on Windows yet). Kosmos',
    'installs it from Google when the person presses Install Antigravity under',
    'Sign in with Subscription on the Gemini row, and it signs in with Google in',
    'the browser. On the screen',
    'it is "Google Gemini (Google subscription)". It has no Kosmos account, so',
    'it is not in `GET /api/accounts`; `GET /api/antigravity` says whether it is',
    'installed (installed only: signing in happens in Google\'s own window).',
    ...
    'Grok and Gemini each take a subscription sign-in or a key. Keys and the',
    'OpenAI and Grok sign-ins are in Settings, AI Models, Add a provider; Gemini\'s',
    'Google subscription is set up from its row in the guided setup (Settings, Show',
    'the guided setup, then Connect on Gemini).',
