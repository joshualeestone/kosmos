...
function blockBody() {
  return [
    ...
    '`' + kosmosCliShown() + ' connections`. (If that path is not there any more,',
    'the command is `kosmos connections` and it is worth saying so rather than',
    'giving up: this line is written when your file is, so it can go stale if',
    'Kosmos moves.) It tells you which providers are',
