...
function blockBody() {
  return [
    ...
    'Gemini on a Google subscription runs on Google\'s Antigravity. The person',
    'installs it once by pasting Google\'s installer into Terminal (curl -fsSL',
    'https://antigravity.google/cli/install.sh | bash); it signs in with Google',
    'in the browser. On the screen it is "Google Gemini (Google subscription)".',
    ...
    'Google subscription is set up from its row when Kosmos is first set up, or by',
    'installing Antigravity as above. Each person uses their own account or their own',
