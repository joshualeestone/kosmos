...
function blockBody() {
  return [
    ...
    '`' + kosmosCliShown() + ' connections`. If that path is not there, try',
    '`kosmos connections` instead. This line was written at the same time as the',
    'rest of your file, so it can go stale if Kosmos moves; a path that has gone',
    'is not a feature that has gone.',
    '',
