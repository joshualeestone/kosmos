...
function blockBody() {
  return [
    ...
    'connected too: Gemini with an API key, Grok with an API key or an xAI',
    'subscription. Those four can be connected today.',
    ...
    'Gemini\'s and Grok\'s are smaller downloads, and on a Mac Kosmos installs',
    'them the same way. On Windows it does not install those two yet, so there',
    'they have to be on the computer already, and the screen says so if one is',
    'missing.',
    '',
    ...
    'and Grok take either a subscription sign-in or a key, and Gemini a key, all',
    'in Settings, AI Models, Add a provider. Each person uses their own account or their own',
