...
const dirPath = () => path.join(base(), 'commitments');
...
function recordPath(agent) {
  return path.join(dirPath(), store.safeKey(agent) + '.json');
}
...
module.exports = { get DIR() { return dirPath(); }, STATE, STALE_AFTER_MS, FUTURE_TOLERANCE_MS, MAX_COMMITMENTS, MAX_RECORD_BYTES, read, report, add, resolve, readAll, recordPath };
