...
function read(agent) {
  ...
  /* `neverReported` is a FIELD, for the same reason `absent` is one (see parseRecord): a caller
     that wants to say "has not reported yet" must not match this module's prose to decide it.
     Only a record that does not exist at all is "never reported"; an unreadable one is not. */
