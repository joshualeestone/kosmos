...
async function invite(remote, body) {
  ...
  try { fedseal.stashInviteSecret(body.project_ref, s); } catch (err) {
...
async function verify(remote, body) {
  ...
  verified.set(d.edge_id, Object.assign({ at: Date.now(), seal_s: sealS }, snap));
...
function joinSnapshot(edgeId) {
  ...
  const { at, ...snap } = held;
  return snap;   // includes seal_s (#3728): the join route keeps it, and sends it nowhere
}
