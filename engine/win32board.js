...
const BOOT_JS = [
  ...
  '   confirmed, before the pointer is read. Without a journal this is one existsSync. A failure is',
  '   reported and the boot goes on. */',
  ...
  '    const recoverer = record && !record.finished && Array.isArray(record.recoverFrom)',
  "      ? record.recoverFrom.find((f) => typeof f === 'string' && path.basename(f) === 'win32apply.js' && fs.existsSync(f)) : null;",
  '    if (recoverer) require(recoverer).recoverAtBoot(journal);',
  '  } catch (e) {',
