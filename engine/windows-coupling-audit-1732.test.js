...
const INVENTORY = [
  ...
  { file: 'engine/runners.js', family: 'path-delimiter-literal', count: 1, contains: ".split(';').map((e) => e.trim().toUpperCase())", disposition: 'win32-only-branch', why: "PATHEXT parse inside hasExecutableExt's win32 membership test (#2270); ';' is the separator Windows uses for PATHEXT, correct here rather than tolerated -- the ext check runs only on the win32 branch (platform-injected, asserted from macOS in runners.win-runnable-2270.test.js). A SECOND row, not a bumped count, per this file's rule" },
  ...
  { file: 'server.js', family: 'fs-const-platform-flag', count: 2, contains: 'const NONBLOCK = fs.constants.O_NONBLOCK', disposition: 'benign-nonblock', why: 'TWO byte-identical sites (#1652 /api/agent-import-file and #2682 /api/agent-import-folder): O_NONBLOCK is paired with O_NOFOLLOW to keep a fifo swapped into the TOCTOU window from blocking the open (as instructions.js/workerfile.js do); the fstat isFile check refuses the fifo, so its win32 absence is not a security regression. Count 2 for the same reason as the NOFOLLOW row above: identical lines cannot carry line-distinct `contains`' },
  { file: 'engine/brief.js', family: 'fs-const-platform-flag', count: 1, contains: 'const NOFOLLOW = fs.constants.O_NOFOLLOW', disposition: 'guarded-vanish', why: '#3595 BRIEF.md goal read: captured undefined-safe as (NOFOLLOW || 0), and an lstat isFile check refuses a symlink before the open on every platform (engine/brief.test.js symlink arm)' },
  { file: 'engine/brief.js', family: 'fs-const-platform-flag', count: 1, contains: 'const NONBLOCK = fs.constants.O_NONBLOCK', disposition: 'benign-nonblock', why: 'avoids blocking on a fifo during the BRIEF.md read-open (lstat already refuses a non-file); its win32 absence changes nothing that matters' },
