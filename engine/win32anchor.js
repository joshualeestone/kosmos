...
const BOOT_JS = [
  ...
  '   freezes the store root, or it would read the default world\'s store. An',
  '   engine too old to know about worlds has neither file and is left alone. */',
  'const args = process.argv.slice(2);',
  "const argvAt = path.join(engine, 'win32argv.js');",
  "const worldsAt = path.join(engine, 'worlds.js');",
  'if (fs.existsSync(argvAt) && fs.existsSync(worldsAt)) {',
