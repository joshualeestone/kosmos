...
test('every emit site in every check prints a line the gate can quote', () => {
  ...
  /* 🛑 27 SINCE THE #1801 REBASE (2026-09-02), DOWN FROM 29, AND CHECKED RATHER
     THAN BUMPED. The failure message says to check the diff first because the
     dangerous cause is matcher drift, so I did:
       - all 27 remaining sites are quotable and `bad` is empty
       - the two that left are click-first-run.js and render-first-run.js, whose
         emit shapes CHANGED in the rebase, not whose output became unquotable
       - both were verified against the runner's real grep and ARE still
         quotable: ` FAIL  <what>` and `FAIL  PROBLEMS (n):`
     ⇒ They are now the known-uncovered shape this file already names: a
     template whose literal prefix is EMPTY because the interpolation comes
     first, which no static read can evaluate. Quotable in fact, invisible to
     this scan, and said so rather than counted as coverage. */
