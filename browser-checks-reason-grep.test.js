...
test('every emit site in every check prints a line the gate can quote', () => {
  ...
  /* 29 after kosmos#1918 added render-reauth-reach-1918.js, whose per-problem
     `console.error('  FAIL  ' + p)` loop is one SHAPE-1 finding-emit site, confirmed
     quotable (the matcher counts it). */
  const EXPECTED_SITES = 29;
...
test('every catch/launch emit prints a line the gate can quote (#1864)', () => {
  ...
  /* 14 after kosmos#1918 added render-reauth-reach-1918.js, whose launch-failure
     `console.error('FAIL  render-reauth-reach-1918: could not start a browser' ...)` is
     one catch/launch emit site, confirmed quotable (the matcher counts it). */
  const EXPECTED_CATCH_SITES = 14;
