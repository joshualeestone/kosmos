...
test('piece ten: the + sits at the card heads and the minus on the member rows, in the consolidated view only', () => {
  ...
  // #3923: every row passes suppressTold=true; the verdicts are said once in the project notice.
  assert.match(src, /roster\.map\(\(m\) => pjMember\(m, true, true, true\)\)/, 'the project page asks for it (hideState=true, #3131)');
  ...
  // #3218: this rule used to also target `.pjcard-members > .pj-addmem`, but the Members card is
  // display:none in the consolidated view now, so it is scoped to #pj-newtask only. `.pj3 #pj-newtask`
  // (id + 3 classes/attrs) still outranks the `#pj-tasks-field > *` span-all rule (id + 2), so the +
  // lands in grid-column 2 rather than spanning -- the point this assertion guards.
  assert.match(block, /body\.consolidated \.pj3 #pj-newtask \{ grid-column: 2; grid-row: 1; width: 22px; height: 22px;[^}]*font-size: 0;/, 'the + rule outranks the card\'s span-all rule (it lost once, and sat over the label)');
