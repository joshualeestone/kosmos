...
test('the mode is gated on width and on the two tabs it merges; Settings stays a whole page', () => {
  ...
  // Window bumped 4000 -> 4600 (#1615: two syncPlusChrome() calls + comments) -> 4800
  // (#2651: the discovery-trigger off-tab hide + its DISCOVERY_TRIGGER_WAS_HIDDEN sync),
  // all legitimate showTab growth ahead of the `const cons` line. showTab is far longer
  // than the window, so this still slices strictly inside it (const cons ~4471, toggle ~4620).
  const st = SCRIPT.slice(SCRIPT.indexOf('function showTab('), SCRIPT.indexOf('function showTab(') + 4800);
