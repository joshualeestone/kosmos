...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, WKNavigationDelegate, WKUIDelegate {
    ...
    private func checkPromptRequests() {
        ...
        consumeRequest(named: "tmux-a11y-prompt-request") { [weak self] in
            // #2911: run an osascript automation op UNDER tmux so macOS prompts for tmux (the
            // responsible process agents run under), securing it up front. No AXIsProcessTrusted
            // here -- that would register the app; the whole point is to prompt for tmux.
    ...
    /// #2911: fire the macOS accessibility/automation prompt attributed to TMUX, so the user
    /// secures it UP FRONT instead of being ambushed mid-work (Josh's ruling).
    ///
    ...
    private func spawnTmuxAutomationPrompt(kosmosHome: String) {
        ...
        // ⚠️ THE ONE VERIFY-PINNED STRING (#2911). A fresh-install verify confirms which TCC
        // service fires (Accessibility via System Events, Automation/AppleEvents via Terminal,
        // or BOTH) and which binary macOS names. `System Events` UI-scripting raises the
        // Accessibility ("control your computer") prompt Josh described; `get name of first
