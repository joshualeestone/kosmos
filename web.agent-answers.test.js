...
test('a reply is escaped on its way to the screen', () => {
  ...
  const dmRow = new Function(
    ...
    + ['esc', 'pjInline', 'pjPreviewCard', 'pjSize', 'pjWords', 'pjFiles', 'pjAttachmentCards', 'pjFileWord', 'pjAttachmentCard', 'pjWhen', 'pjWhenPart', 'pjSentence', 'placedWords', 'pjVerdict', 'dmWho', 'dmRow'].map(slice).join('\n')
    + '; return dmRow;',
  )();
...
test('the person’s own row carries its time now that the receipt may say nothing', () => {
  ...
  const dmRow = new Function(
    'let CURRENT = ' + JSON.stringify(pick(card)) + ';\n'
    + ['esc', 'pjInline', 'pjPreviewCard', 'pjSize', 'pjWords', 'pjFiles', 'pjAttachmentCards', 'pjFileWord', 'pjAttachmentCard', 'pjWhen', 'pjWhenPart', 'pjSentence', 'placedWords', 'pjVerdict', 'dmWho', 'dmRow'].map(lift).join('\n')
    + '; return dmRow;',
  )();
  const at = new Date().toISOString();
