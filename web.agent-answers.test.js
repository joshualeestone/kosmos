...
test('a reply is escaped on its way to the screen', () => {
  ...
  const dmRow = new Function(
    ...
    + ['asSentence', 'esc', 'pjInline', 'pjPreviewCard', 'pjSize', 'pjWords', 'pjFiles', 'pjAttachmentCards', 'pjFileWord', 'pjAttachmentCard', 'pjWhen', 'pjWhenPart', 'pjSentence', 'placedWords', 'pjVerdict', 'dmWho', 'dmRow'].map(slice).join('\n')
    + '; return dmRow;',
  )();
...
test('the receipt speaks about what happens next, and is silent when there is nothing to say', () => {
  ...
  const pjVerdict = new Function(
    ['asSentence', 'esc', 'pjWhen', 'pjWhenPart', 'pjSentence', 'placedWords', 'pjVerdict'].map(lift).join('\n')
    + '; return pjVerdict;',
  )();
...
test('the person’s own row carries its time now that the receipt may say nothing', () => {
  ...
  const dmRow = new Function(
    'let CURRENT = ' + JSON.stringify(pick(card)) + ';\n'
    + ['asSentence', 'esc', 'pjInline', 'pjPreviewCard', 'pjSize', 'pjWords', 'pjFiles', 'pjAttachmentCards', 'pjFileWord', 'pjAttachmentCard', 'pjWhen', 'pjWhenPart', 'pjSentence', 'placedWords', 'pjVerdict', 'dmWho', 'dmRow'].map(lift).join('\n')
    + '; return dmRow;',
  )();
  const at = new Date().toISOString();
