 ...
 * injected `createAgent` so no real agent is launched. The load-bearing controls
 * are the DANGEROUS answers: it must NOT seed when there is no connected model
 * (createAgent refuses), no saved user name, or it was already seeded -- and it
 * must NOT throw or double-create. The positive case proves it DOES create,
 * names the agent after the user, uses the `setup` role, and copies the avatar.
 *
...
function reset() {
  ...
  // you's picture lives under the data root; clear whatever is there.
  try { const p = you.picturePath(); if (p) fs.rmSync(p, { force: true }); } catch { /* ok */ }
}

const createdOk = (calls) => (opts) => { calls.push(opts); return { outcome: 'created', name: opts.name }; };
const refused = (calls, because) => (opts) => { calls.push(opts); return { outcome: 'refused', because }; };
...
test('the setup role exists, is menu:false, and is NOT offered in the normal create menu', () => {
  ...
  // The create form filters to r.menu !== false (server.js), so setup must be excluded.
  ...
  // instructionsFor substitutes the name.
  const brief = roles.instructionsFor('setup', 'Dana');
  assert.match(brief, /You are \*\*Dana\*\*, the Kosmos setup guide\./);
});

test('POSITIVE: with a saved user name and a runnable model, it seeds -- named after the user, role setup', () => {
  you.save({ name: 'Testuser', does: 'runs things' });
  const calls = [];
  const res = setupAssistant.seedSetupAssistant({ createAgent: createdOk(calls) });
...
test('CONTROL: no saved user name -> does NOT seed and does NOT call createAgent', () => {
  // you.read() is `absent` on a fresh sandbox (reset removed you.json).
  const calls = [];
  const res = setupAssistant.seedSetupAssistant({ createAgent: createdOk(calls) });
...
test('CONTROL: no connected model (createAgent REFUSES) -> does NOT seed and writes NO flag', () => {
  you.save({ name: 'Testuser', does: 'runs things' });
  const calls = [];
...
test('CONTROL: createAgent throwing is swallowed -- never propagates to break onboarding', () => {
  ...
  let res;
  assert.doesNotThrow(() => {
    res = setupAssistant.seedSetupAssistant({ createAgent: () => { throw new Error('boom'); } });
  });
  assert.equal(res.seeded, false);
...
test('ONCE-EVER: once the flag is written, a second seed is a no-op (createAgent not called again)', () => {
  you.save({ name: 'Testuser', does: 'runs things' });
  const calls = [];
  const first = setupAssistant.seedSetupAssistant({ createAgent: createdOk(calls) });
  assert.equal(first.seeded, true);
  // The caller writes the flag on success (mirrors markWelcomeSeeded).
  setupAssistant.markSetupAssistantSeeded({ name: first.name, via: 'test' });
  assert.equal(setupAssistant.setupAssistantSeeded(), true);

  const second = setupAssistant.seedSetupAssistant({ createAgent: createdOk(calls) });
