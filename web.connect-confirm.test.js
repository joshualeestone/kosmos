...
test('a connected provider row goes green with a check, and does not read as disabled', () => {
  ...
  /* ⚠️ Anchored on the OWNING FUNCTION, not a positional char-slice from an
     fr-llm-connect getElementById. The old anchor did CODE.indexOf(<that lookup>)
     + slice(at, at+600), and it failed the fourth time on 2026-09-06: a NEW
     same-id lookup earlier in the file (frClaudeConfirm) made indexOf grab the
     wrong block, and the toggle -- still present, in frPaintSubscription -- sat
     8854 chars past the anchor, outside the 600 window. Widening 600 would keep
     the exact fragility (the next insert breaks it silently); binding the block to
     the function that OWNS the toggle fixes the class: an insert inside it still
     matches, and an insert outside it cannot break this. */
  const fnStart = CODE.indexOf('function frPaintSubscription');
  assert.ok(fnStart > 0, 'frPaintSubscription (the connect button owner) moved; this test is measuring nothing');
  const fnEnd = CODE.indexOf('\nfunction ', fnStart + 1);
  const block = CODE.slice(fnStart, fnEnd > fnStart ? fnEnd : fnStart + 6000);
