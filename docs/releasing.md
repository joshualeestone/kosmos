...
1. **Clean main, and read the log.** The release ships what is merged, not what
   ...
   **1c. Test-sign with the cut's own identity (#3579)**, still before anything is
   ...
   and fails with `errSecInternalComponent`. That killed the 0.6.91 cut twice at step
   4, after the full suite and the page layer, and a cut cannot resume. Now it stops
