      ...
      for (const theme of key === 'existing-2fa' ? ['light', 'dark'] : []) {
        ...
        const sep = await page.evaluate(() => {
          ...
          const prim = [...card.querySelectorAll('.btn.uprime')].map((b) => {
            ...
            return { id: b.id, r: Math.max(worst(fill.length ? fill : ['rgba(0,0,0,0)']), worst([cs.borderTopColor])) };
          });
        ...
        chk(named.every((id) => primIds.includes(id)) && low(sep.prim).length === 0, `[${key}] #3841 every primary button stands off the navy card (${theme})`, primIds.length + ' ' + low(sep.prim).join(' '));
      }
      if (key === 'existing-2fa') {
