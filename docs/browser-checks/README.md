...
| `render-qask-grant-2808.js` | kosmos#2808 (grant half): DRIVES the agent-page one-click GRANT button (`#d-qask-grant`) end to end, the served wiring the node test (`web.qask-grant-2808.test.js`, which lifts the gate + click handler against stubs) cannot see. Hermetic (file://), answers every route from `addInitScript`, drives `paintTalk` the render-talk way with an asking thread carrying a permission menu + a parsed `options` array (affirmative opt[0]). Asserts: the grant button is visible + REACHABLE via `elementFromPoint`, reads Josh's exact "Give this agent permission and clear this message", targets option 1 ("Yes"), and the raw option buttons COEXIST below it. A real click through the shipped listener POSTs `/api/agent/april/thread` once with the affirmative digit (`text:"1"`, `chose:"Yes"`) and a placed send takes the whole `#d-qask` box OFF screen (the "clear" half). Red-capable contrast: the folder-trust prompt (answerNote) HIDES the grant button while the box still paints and Trust & Restart shows. Every assertion can return the dangerous answer. Not CI-allowlisted (matches render-qask-clear-2808; runs at the cut). |
