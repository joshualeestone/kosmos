...
| `render-needsyou-dealarm-2808.js` | #2808 class 2 (Josh 2026-09-14 16:49): a needs_you the agent reported DELIBERATELY (`stateReportedBy` 'agent' -- its own substantive question) renders as a CALM "Has a question" card, not the red "Needs you" alarm a QA tester read as "app broken". Hermetic (file://), calls the real `card()` builder for a class-2 (`by:'agent'`) agent plus two negative controls, injects them, and reads COMPUTED style. Asserts the class-2 card has NO `attn` class, HAS the `question` class + an `st-question` pill + a `.ask` (not `.haz`) glyph + reads "Has a question", and its computed border color DIFFERS from the alarm's. Negative arms (load-bearing): a class-1 (`by:'auto'`, the permission/trust junk, PigeonPete's invisible-handle target) STAYS red and reads "Needs you", and a SCRAPED needs_you (null provenance) STAYS red -- only the KNOWN agent case is de-alarmed (selfreport.js:389). The border-color-differs arm reds if `.acard.question` fails to override the alarm border. |
