...
that matters, and that gap killed a cut (#1720): a two-press-confirm change was
...
from the moment it landed. Before you move an id, class or text, `grep` this
...
| `render-model-change.js` | The Model section (#386) and the #1373 OpenAI sign-in picker: the rows offered, the preselect, and the arm that hides it again. Seals all three homes and stubs the models endpoint, so no real key is involved; see the check's own header for why. |
