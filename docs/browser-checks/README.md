...
| `render-start-agent-3410.js` | The "Start this agent" affordance in the detail-header identity column (#3410, Josh 0.6.88 live test): a prominent button shown ONLY when the open agent is confidently not running (`cardStOf(a).pres === 'off'`, i.e. 'stopped'), because Josh had agents that never connected and no UI way to start one without the buried Fresh-start section. Installs a stopped agent and an idle one, opens each via the real `openDetail`, and asserts (1) the stopped agent shows `#d-start-wrap` with the button reading "Start this agent", enabled; (2) the idle agent hides it; (3) clicking Start POSTs the EXISTING `/api/agent/<name>/restart` route (not a new endpoint), and a refused outcome shows the honest `restartFailureLine` ("We could not start Nyx. …"), not a false "started", and re-enables the button. The button verifies real readiness via `restartReadyWait` before ever claiming success, so it stays honest on the #3418 restart false-success. Drives the real painter + handler and asserts the real DOM. |
