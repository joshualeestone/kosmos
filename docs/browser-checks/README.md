...
| `render-composer-stroke.js` | The project composers (`#pj-post` room post box, `#pj-say` agent-say box) render with NO resting border in both the tab and consolidated layouts and both themes (Josh, 2026-09-21: the input reads cleaner as a filled field with no stroke). CONTROL: the agent-dialogue composer (`#d-say`) keeps its border, proving the removal is scoped to the project composers and did not reach the base `.composerbox` rule. Hermetic (file://). Reds on origin/main, where the project composers still carry the 1px `--k-rule` border. |
