...
| `render-projects-map.js` | kosmos#2458 (Josh, 2026-09-08 "map view right now"): the Projects tab's third layout, Map, drawn to Mona's design/subprojects.html "The Map" spec. Hermetic (file://). Injects a project hierarchy (Alpha 2 agents; Beta needs-you with Gamma idle under it and Delta 3 agents under Gamma), clicks the real Map viewtoggle button, and asserts `paintProjectsMap` (1) reveals `#pj-map` and hides the list surfaces at computed `display`, (2) draws a Kosmos root carrying the fleet count, (3) NESTS the real parent/child shape (Beta > Gamma > Delta), and (4) marks a needs-you project `.attn` with a "needs you" line while an empty one reads "idle" and a staffed one reads "N agents". Each assertion can return the dangerous answer (a flat render, a wrong count, a missing nest all FAIL). |
