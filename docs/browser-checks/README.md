...
| `render-needsyou-fallback-2456.js` | kosmos#2456 (PR #2493): the project-room question banner (`#pj-question-label`) falls back to the agent's REPORTED needs_you sentence when the live pane no longer shows the question, instead of the false "we cannot find the question on its screen right now"; and the label is source-aware ("This is what it told us it needs:" for a reported question vs "This is the part of its screen that asked:" for a live-pane one). Hermetic (file://, `addInitScript` fetch stub), drives the real `paintThread` through four states and reads the rendered label: 1 reported question -> "told us it needs"; 2 live-pane question -> "part of its screen that asked" (contrast to 1); 3 asking + a supplied `questionBecause` -> shows that reported sentence (the #2456 fix); 4 asking + nothing reported -> "we cannot find the question on its screen right now" (contrast to 3). The two contrasts (1-vs-2, 3-vs-4) make each assertion able to return the dangerous answer; proven RED by neutralizing the `questionBecause` fallback. Covers the PROJECT ROOM (`paintThread`); the AGENT PAGE (`paintTalk`) carries the identical strings/branches and is driven by `render-talk.js` (noted residual: a per-surface divergence there is not asserted here). |
