...
| `render-model-spinners-2365.js` | kosmos#2365 / Josh 0.6.40 retest #11: the existing inline `.kspin` breathing spinner appears on the Choose-a-Model screen loading/waiting lines. Hermetic (file://), driving the page's own painters (`frPaintConnect` into `#fr-sub`, `frCheckRow`). Asserts each SYSTEM-working connect phase (downloading, installing, signin-launching, signin-completing) renders a `.kspin`, the OpenAI "Adding..." validate state (stubbed never-resolving POST) renders a `.kspin`, and the initial connection check (frCheckRow state `checking`, allowLocal) renders a `.kspin` in its mark. CONTROL: `signin-browser-open` is waiting on the PERSON to sign in in the browser (not the system working), so it must NOT get a spinner. Whole-feature control: the pre-fix page renders none of these `.kspin` and reds. |
