...
| `render-talk-fill-2622.js` | The Talk dialog box fills to the bottom of the window (#2622 part 2): `#d-talk-box` bottom lands near the viewport bottom at two window heights (1100 and 700), the thread (`#d-dmthread`) scrolls INTERNALLY on a long conversation (the `.dmthread` max-height:15rem cap lifted), and the composer pins near the bottom. Scoping arm: a non-Talk section (Model) stays content-height, so the `:has()`-gated fill is safe. The fill arms red on the pre-change page (box was content-height, sat far above the bottom). |
