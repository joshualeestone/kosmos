...
| `render-nav-badges-3216.js` | kosmos#3216 (Josh 6.74): red unread-count badges on the Agents and Projects nav tabs. Hermetic (file://). Asserts the badge spans sit on the correct tabs, and the `setNavBadge()` render contract: a positive count shows the number, zero/null hides it (display:none, never a guessed zero), over-99 caps at `99+`, and the badge is the shared unread red (#b3261e) with white text. The two data wirings are asserted by source (seeding real per-agent `dmUnread` and per-project `unread` end-to-end is engine-derived and not hermetically reproducible): the Agents badge is set from the fleet `dmTotal` in tick(), and the Projects badge sums `p.unread` across `PROJECTS` excluding the open project. |
