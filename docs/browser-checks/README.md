...
| `render-account-name-2095.js` | kosmos#2095: the human-chosen account name is the PRIMARY label in the Settings AI-models row; the key last-4 is a secondary detail. Drives the real paintAccounts against a stubbed /api/accounts (no board) and asserts a NAMED OpenAI row shows its name (never the key) as `.acct-who b` with the key on a secondary `.acct-org`; an unnamed OpenAI row and a Claude row are unchanged; and an arbitrary name is HTML-escaped (no injected element child). Reds on origin/main (the row shows the key, not the name). |
