...
| `render-msg-counter-3403.js` | kosmos#3403 (Josh: raise the message cap, "it just cut off mid-message with no warning"): the cap is 8000 and a SOFT near-limit counter (`#d-say-count`, class `cpost-count`) appears in the last 500 chars before it and turns amber (`.near`) within 100, so the cap is never a silent cutoff. Boots a seeded server with a fixture agent, opens the agent Talk view to reach the real `#d-say`, and asserts, in BOTH themes: a CONTROL that d-say caps at 8000; the counter is HIDDEN empty and HIDDEN far from the cap (the setup control, or every "shows N" arm is vacuous); it APPEARS at 500 left reading "500 characters left" and not-yet-amber; at 100 left it reads "100 characters left" AND turns a real computed amber (colour != the muted base -- the browser-only fact); singular "1 character left" at one; "0 characters left" still shown at the cap; clearing hides it; and a REAL-KEYS arm typing across the threshold proves the shipped d-say input listener drives it (not just a dispatched event). |
