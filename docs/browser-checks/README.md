...
| `render-talk-fill-2622.js` | The agent Talk box (#2622, #3497). WIDE layout: the box meets the header rule, the right edge and the bottom edge; the composer's bottom margin equals its side margins; no page scroll; the same under a 60px taller header; the identity block does not move and its scroll box starts below the back link; the build marker sits clear of the box; a phone-pairing card above and a connection notice below stay visible (A1-A1k, A2 at 1400x1100 and 1400x700). NARROW (500x900): the page scrolls, the header is not sticky, and the box is its own window-tall block; scrolled to it, and at the end of the page, the whole box from its header row to the composer is on screen and uncovered (A2b, A2d, A2e). Also: the thread scrolls internally with the cap lifted (A5), a long thread in a short window keeps the composer reachable (A9), a non-Talk section stays content-height (A6), no page errors (A7). |
