...
| `render-home-phone-718.js` | kosmos#718 row 6: the home grid and the agents list on a phone. Real server, three agents with a profile and a folder and no login job (so the survival panel shows), at iPhone SE / iPhone 15 / Pro Max / mid Android (touch): "Set them to start at login" is at least 44px tall; a real touch tap on an empty part of an agent's card (grid) and of its row (list), away from its name and any control, opens that agent, which is why the 16px names are left as they are; the page is no wider than the screen in either layout; at 1280 the button keeps its desktop height (34px, the same with any min-height taken away). Tab layout only (the consolidated layout does not show the survival panel). Controls, measured: on main the button arm fails at all four sizes in both engines (34px) and the rest pass; with the click handler opening an agent only from its name, both tap arms fail at all four sizes; with the button rule not limited to phone width, the desktop arm fails (44 against 34). Gate runs Chromium; `ENGINES=chromium,webkit` by hand (WebKit is an engine approximation, not Safari). |
