...
| `render-tasks-view-3559.js` | The Tasks view (#3559). The 25-task gate (Josh): below 25 tasks ever, the Tasks tab and the consolidated rail's Tasks button are hidden after a real status tick, and shown once the saved flag is set; then the tab opens `#panel-tasks`, and in the consolidated view (tab bar hidden) the rail button does. #3949 (Josh, 2026-09-26): five single-label tiles in his order (Needs Your Decision, red and the only red; In progress; Assigned but not started; Unassigned; Completed) with the right counts and no byline; Completed is also the folded list, its count read off the fold; no Built-but-waiting (#3951), Done-check-it or category is drawn. The decision task comes from a stubbed `tasks.waitingOnPerson` (the fleet fixture cannot make a question name a project). Layout: no left Projects column, the search about half the width with the open-task count to its right, Project and Created: dropdowns on one row above the tiles, Group by and Sort dropdowns on one row under them (phone width may wrap each pair), no tile hint. A task its agent names sits under In progress; in status grouping no row repeats its group's name, and grouped by project each row carries its state. Both halved gaps are measured, in the consolidated column too, and a message appearing does not move the list. Search finds tasks by agent name and by what they say, and combines with the project dropdown; Group by Project regroups them; ticking two rows and Close them closes both with the note on each history. No element in the view carries a coloured left border (Josh, 2026-09-24). Light, dark, 760 and 390 wide (the search goes full width). Self-hosted sandboxed server; headless-fine. |
...
| `render-alltasks.js` | The project's View-all door opens the Tasks view scoped to that project (#1382, #3703): its rows only, finished ones in the Completed fold, the open count equal to the open rows on screen; "+ New task" files to the picked project or asks which on All tasks; an archived project's door still lists its tasks; consolidated and 390 wide. |
