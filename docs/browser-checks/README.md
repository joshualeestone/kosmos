...
| `render-firstrun-access-onebox.js` | Screen 2 (Access), #2910 + #2685. FILENAME IS HISTORICAL: it began at 0.6.39 #8 asserting ONE compact preview; #2910 reversed that to SIX macOS-style prompt previews in TWO labeled groups (tmux + Kosmos, three folders each), and #2685 corrected the file-access group's label from "Terminal" to "tmux" (the bundled tmux is the process macOS holds responsible for agent file access, so it is the name on the real folder prompts). HERMETIC (loads web/index.html over file://, boots no server): unhides the `#fr-pane-2 .s2-dlg-fan` chain and reads static markup + computed style. Arms: exactly SIX `.s2-dlg` in two `.s2-appgrp` groups; the two group labels name tmux and Kosmos; the file-access group names tmux NOT Terminal (old generic "...in your folders." line gone) and carries its explaining `.s2-grpnote` micro-line; each app covers Documents/Downloads/Desktop; the preview copy is compact dialog-sized (~12-13px/600), not the overlay body; every preview renders Don't Allow + Allow; every mock Allow is a live `.s2-mockallow` affordance; and the Allow button's `::after` draws a solid gold ring while Allow itself is the blue macOS default button. The ring is a computed `::after` a source read cannot see. Covers the FILE-access screen only; the functional `.s2-gate-row` grant and the S3 accessibility step are not asserted here. chromium + webkit. |
