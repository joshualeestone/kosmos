...
| `render-firstrun-access-onebox.js` | Screen 2 (Access), #3031 (was #2910 + #2685). FILENAME IS HISTORICAL: it began at 0.6.39 #8 asserting ONE compact preview; #2910 grew it to SIX previews in two labeled groups, and #3031 (Josh 2026-09-14) cut it back to two previews (Kosmos, tmux); 2026-09-19 (Josh, 0.6.81 QA) then removed the tmux preview, leaving ONE Kosmos preview, a single "would like to access files." prompt naming its app, with the folder-count labels and the group micro-note removed and the Don't Allow/Allow buttons no longer wrapping. #2685's "Terminal" -> "tmux" relabel stands (the bundled tmux is the process macOS holds responsible for agent file access, so it is the name on the real folder prompts). HERMETIC (loads web/index.html over file://, boots no server): unhides the `#fr-pane-2 .s2-dlg-fan` chain and reads static markup + computed style. Arms: exactly ONE `.s2-dlg`, no `.s2-appgrp` groups and no `.s2-applbl` labels; the preview names Kosmos NOT tmux and NOT Terminal; each preview uses the generic "would like to access files." line, no per-folder text, old "...in your folders." line gone; the folder-count labels and the `.s2-grpnote` micro-note are gone; the preview copy is compact dialog-sized (~12-14px/600), not the overlay body; the preview renders Don't Allow + Allow (count one); neither button wraps (white-space:nowrap + single-line height, and the two buttons in the row are equal height); the mock Allow is a live `.s2-mockallow` affordance; and the Allow button's `::after` draws a solid gold ring while Allow itself is the blue macOS default button. The ring is a computed `::after` a source read cannot see. Covers the FILE-access screen only; the functional `.s2-gate-row` grant and the S3 accessibility step are not asserted here. chromium + webkit. |
