...
| `render-settings-agy-3874.js` | kosmos#3874: Settings, AI Models, Add a provider offers Gemini on a Google subscription (Antigravity), as the first-run Gemini row does. HERMETIC (file://, fetch stubbed). Where it is offered, Google Gemini shows Sign in with Subscription / Use an API key first; the key path goes through the Gemini CLI download; the subscription walks check, Install Antigravity, Open, Check again, Ready in the dialog; Stop returns to the choice. Control: not offered, no choice, straight to the download. `SHOT_DIR=<dir>` saves screenshots. |
