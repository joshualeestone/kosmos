...
| `render-dm-chatfirst-718.js` | kosmos#718 chat first on a phone (some arms are regression guards that also pass on main; main fails 250 of 510) (Liu Kang decided the build, 2026-09-24). HERMETIC (file://, fetch stubbed). The agent page with Direct Message open at iPhone SE / iPhone 15 / mid Android / Pro Max, light+dark: the page is exactly the screen and the thread scrolls inside it, the composer and Post are on screen, the composer stays above a SIMULATED keyboard (the visible-height variable the page's own visualViewport listener writes) while the header steps aside, Post, add-a-file and the Profile tab are at least 44px, the visualViewport listener's keyboard detection (stubbed: drop, pinch, layout-shrinking keyboards, rotation), Profile is one tap away, no sideways scroll. At 800 and 1280 the stacked tiles and the "Direct Message to" caption are unchanged. Gate runs Chromium; `ENGINES=chromium,webkit` by hand (WebKit is an engine approximation, not Safari). |
