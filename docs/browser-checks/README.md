...
| `render-win32-start-at-sign-in.js` | win32-installer-native (#3010): the "Start Kosmos when I sign in to Windows" switch on Settings > This computer, rendered and clicked. The knob's position is a COMPUTED CSS transform on `.toggle.on i`, which a markup test cannot see, and a click that repainted from the click instead of from the engine's read-back would pass every source assertion. HTTP-SERVED (KOSMOS_URL, not file://): the first version was hermetic over file:// and could not CLICK the switch in CI (a hit-test/actionability timeout, measured to fix clean over http); this runs against the served board and page.route-mocks only `/api/machine`, `/api/machine/start-at-sign-in` and `/api/first-run` to a win32 state (the render-token-usage-2617 pattern), with a scheduler that remembers the switch. In chromium and webkit: a row read as on draws a visible, labelled `role="switch"` with the knob slid right; a click POSTs `{on:false}` and a fresh `/api/machine` read repaints it off with the knob at rest and no message; a 409 shows Windows' sentence in `#set-machine-msg` and leaves the switch where the engine says; rows that cannot be read again after a click take the switch away and say so; a row whose state could not be read draws no switch; no page errors on either page. CONTROL: an unstamped Mac page renders its autostart row with no switch. |
