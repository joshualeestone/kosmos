...
| `mobile-shots.js` | kosmos#718: phone screenshots for the mobile apps. Boots a THROWAWAY board (temp HOME and data roots, fake tmux, seeded sample agents, DM, room and ask), shoots named screens at four phone sizes (375x667, 393x852, 430x932, 412x915) in light and dark, Chromium and WebKit (an engine approximation, NOT Safari), writes `report.md` and flags horizontal overflow. A leak guard exits 3 if the page shows a real email, a key fragment or this Mac's name. `--list` prints the screens; add yours to SCREENS. The gate runs a strict slice (the frame and accounts screens at 375px); the full 160-shot sweep is by hand. |
