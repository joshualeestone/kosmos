...
| `render-worldrename-1704.js` | kosmos#1704 item 14.1 (Josh 0.6.36): rename a Kosmos via a cog on its switcher row. HERMETIC (loads web/index.html over file://, boots no server): calls the page's real `worldswRender()` with fixture worlds (default + one named) and opens the menu, then asserts the rename cog is present on the NAMED world and ABSENT on the default (whose name is the fixed "Kosmos 1"), that tapping the cog opens the rename modal pre-filled with the world's name, and that Save posts `POST /api/worlds/rename` with `{ id, name }` (via a fetch stub). Reds on the pre-14.1 switcher (no cog / no rename modal). The engine `renameWorld` + the route are unit-tested separately (engine.worlds-rename-1704 + server.test.js #1704 14.1). |
| `render-workindicator-2146.js` | kosmos#2146 (render half): a needs_you/blocked agent that is ALSO actively working shows the board's working glyph alongside its pending state (coexistence, not precedence). HERMETIC (file://): calls the page's real `card()` with fixture agents and asserts that the engine's additive `activeWhileWaiting` flag paints a `.alsowork` affordance (the reused `.act` dots + a "Working now" label, a sibling of the pill) WITHOUT changing the state pill class, its label, or the card's ground treatment. Controls that can each return the dangerous answer: the SAME needs_you agent with the flag off shows no badge and an identical pill/label/ground (so the flag's only effect is additive); a BLOCKED agent with the flag also gets the badge (driven by the flag, not scoped to the red pill); a plain working agent is NOT double-marked. Reds on the pre-#2146 page (no `.alsowork`). The engine `activeWhileWaiting` derivation is unit-tested separately (engine/status.activewhilewaiting-2146). |
...
| `render-worldswitch-2238.js` | kosmos#2238: the multiple-Kosmos SWITCH is actionable (the 0.6.35 bug was read-only rows: create worked, selecting a world did nothing). Hermetic (file://) against a stateful board stub. Asserts a NON-active row is a real, actionable native <button> and clicking it POSTs `/api/worlds/active` with that world's id; on success the active marker + promoted name move to the chosen world and an HONEST status banner appears naming the world and saying it loads after a board restart (roots load only at board boot; the in-app auto-restart is a separate #2238 follow-up, so the slice does not claim the switch is live). Control: the pre-fix page has no row click handler, so the POST-called + marker-moved arms red on it. |
...
| `render-firstrun-s6-2037.js` | kosmos#2037 + #2020 (PR-C2): the first-run wizard SCREEN 6 "self improving" consent switches are real, keyboard-operable, default-ON opt-out controls. HERMETIC (loads web/index.html over file://, boots no server): shows step 6 via the real `frGo(6)`, reads the two role=switch spans (`#fr-s6-feedback` daily report, `#fr-s6-createping` create ping) as default aria-checked="true", confirms the on-show refresh (which fails to read on file://) leaves them ON (never a false Off), then drives a real bound CLICK on the feedback switch and a real KEYDOWN(Space) on the create-ping switch, asserting each flips aria-checked to "false" AND sends a PUT to its own backend (`/api/feedback-setting` / `/api/ping-setting`) via a fetch stub. Reds on a page where the wiring is absent (the spans render but a click does nothing and no PUT is sent). |
| `render-firstrun-import-1652.js` | The fresh-install "find my agents" affordance (#1652, Mona's mock): the AUTO first-run scan is TCC-free (#2125) so it skips Documents/Downloads/Desktop, and this adds a contextual first-run link in screen-9's create arm that opens the create IMPORT panel (openCreate then pickMode('import'), firing the on-demand `/api/scan-import` of those folders). Self-boots a sandboxed server; forces the create empty state and asserts the link + the folder/permission copy render alongside "Giddy Up", clicks it and confirms the import panel opens with the mode selected and the scan container present and `/api/scan-import` fired; CONTROLS: the adopt ending (a real fleet) shows no link, and bare `openCreate()` lands on prompt mode not import. DOM-state only, headless. |
...
| `render-reactions-2255.js` | Emoji reactions on room posts (#2255): self-boots a sandboxed server, creates a project + agent, posts a message, opens the room, and drives the operator flow - a post shows a "+" opener with no initial pills and a hidden palette; the "+" reveals the quick palette (aria-expanded); clicking a palette emoji adds a pill with count 1 marked `.mine`/aria-pressed; clicking the pill again toggles the reaction OFF (the round-trip control leaves no pill). DOM-state only, both themes. |
