...
| `render-help-tips-3574.js` | First-run help and first-visit tips (#3574). On a fresh sandbox the welcome tour shows on the board with the dim and numbered pins (T1); Got it closes it and the board records it (T2); a first-visit screen tip follows, with no dim, and Escape counts as seen (T3); a reload does not bring the tour back (T4); the ? menu reopens the tour and the ring explainer with its three gauge colours (T5); Stop showing tips turns tips off, the Settings switch reads off, no tip shows with tips off, and the switch turns them back on (T6); an unreadable tips store shows no tips and hides the switch (T7); no page errors (T8); Projects, Settings and an agent's page each show their own tip once, and a closed one does not return (T9). |
