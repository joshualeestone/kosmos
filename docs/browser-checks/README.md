...
| `render-consolidated-white-3264.js` | The consolidated view's message ground is WHITE (--k-surface), Josh's brand-owner call (#3264), reversing #980's cream (--k-bg). Puts the page in the consolidated layout and injects the `.pj3 > .pjmid > .composer` + a `.msg-bd`, then asserts the computed background of the consolidated `.pjmid`, its sticky `.pjmid .composer`, and the wing-carve `.msg:not(.you) .msg-bd::after` all resolve to `--k-surface` (== a painted `var(--k-surface)` probe, != `var(--k-bg)`) in light AND dark -- the three that must move in LOCKSTEP or #980's mismatched band / a cream tail sliver returns -- plus a no-regression control that the NON-consolidated (tab) carve keeps `--k-bg` (the override did not leak). Token-pinned (a `--k-surface` retune stays green; any of the three reverting to `--k-bg` reds it). file://, HEADED optional. |
| `render-detail-header-1841.js` | The view-agent-detail header redesign (#1841): the working-rules prompt moved onto the Instructions tab for both doctrine cases (red tab dot, "Add Instructions & Restart"), the hand-edited stale case redesigned as the header restart card ("[name] needs to be restarted" + [Restart], never "it"), the reported self-quote dropped from the task line beside the bubble and relocated to the #d-why explanation note (#3043; a non-reported state keeps its honest why-line), and the role bolded in the meta line. Drives the real painters and asserts the real DOM. |
