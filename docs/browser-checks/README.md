...
| `render-build-marker-2066.js` | kosmos#2066 + kosmos#2658: the board's build marker (`paintBuildMark`) in web/index.html. #2658 (Baron's call) SUPERSEDED #2066's loud coloured staging badge with ONE uniform `beta build v<version>` marker for EVERY channel, moving the staging tell to the hover title (+ /api/accounts channel metadata). Drives the real `paintBuildMark` against a file-load (no board): the version text renders uniformly, and the load-bearing COMPUTED-STYLE assertions are that staging's background + ink are IDENTICAL to prod's (no leftover loud badge, `--stag` is gone) while the safety tell survives in the hover title (staging's title names 'staging', prod's names 'prod', and they differ); an undefined channel falls to prod; no version hides. Reds on origin/main (no `paintBuildMark`, no `#buildmark`). |
