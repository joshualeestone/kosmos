...
| `render-agent-pill-3958.js` | kosmos#3958 (Josh, 2026-09-26): the agent page's status pill follows the poll. Boots a sandboxed board with one agent, opens its page while it is idle, then flips it working, idle, working, working one poll apart through the fixture. On chromium and webkit after each flip: the pill's word and state class match; while working the pill's dots are present AND moving (12 samples over 1.2s); the pill and the DM line agree; on the second working poll in a row the pill's dots are the same nodes (not rebuilt, so not restarted), an arm asserted to have run. Control: the first reading, idle, shows Idle with no dots. Removing the poll's paintDetailState call reds 12 arms (the pill stays Idle); rewriting the pill every poll reds the same-nodes arm on both engines. Self-hosted sandboxed server; headless-fine. |
| `render-thread-steady-3966.js` | kosmos#3966 (Josh, 2026-09-26: "a flash every 5 seconds" on an agent's page): a busy thread is not rebuilt when only its relative times tick over. Boots a sandboxed board, serves the agent a sixteen-message thread spread over the last hour (service workers blocked so webkit's fetch reaches the route), opens the agent page and watches thirty seconds of polls. On chromium and webkit: preconditions that all sixteen rows carry a live time and that some row's words changed during the watch; then no thread node was replaced (every node tagged before the watch survives). Comparing the raw markup instead of its shape reds it on both engines (all 224 nodes replaced). Self-hosted sandboxed server; headless-fine. |
