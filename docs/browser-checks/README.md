...
| `render-detail-header-1841.js` | The view-agent-detail header redesign (#1841): the working-rules prompt moved onto the Instructions tab for both doctrine cases (red tab dot, "Add Instructions & Restart"), the hand-edited stale case redesigned as the header restart card ("[name] needs to be restarted" + [Restart], never "it"), the reported self-quote dropped from the task line beside the bubble (#3043), and the role bolded in the meta line. #3271 (Josh 2026-09-18): the #d-why reason line under the name was REMOVED (no status line up there), so Part 3 now asserts #d-why does not exist while the #d-task engine-state sentence is kept. Drives the real painters and asserts the real DOM. |
