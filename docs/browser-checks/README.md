...
| `render-world-import-2563.js` | kosmos#2563: the create-time "Add my agents from an existing Kosmos" selector on #world-add-modal. HERMETIC (loads web/index.html over file://, boots no server, fetch stubbed): opening the create modal calls `worldImportFetch()` (GET /api/worlds/list) and `worldImportRender()`. Scenario A (list present, two Kosmoses): the control shows with one labelled checkbox each ("Client work (2 agents)", "Side project (1 agent)"), the checkbox value is the world id, and checking both then submitting sends `importAgentsFrom:["w1","w2"]` on POST /api/worlds (COPY semantics). Scenario B (endpoint absent -> 404, the engine slice not live yet): #world-add-import stays HIDDEN so the create flow is unchanged (the graceful-degrade that lets this web slice ship before the engine). CONTROL: on the pre-#2563 page #world-add-import / the worldImport* functions do not exist, so the check reds. The render/fetch/submit units are covered by web.world-import-2563.test.js. |
...
| `render-discovery-gate-2651.js` | kosmos#2651(a) (Josh, product review 2026-09-10): the found + scan DISCOVERY PANELS must NOT auto-scan-and-show on Agents-page load. Josh read that auto-appearance ("we found agents on your computer", widened by #2414/#2410/#2452/#2243) as Kosmos importing every agent file on the machine without consent. The panels are now gated behind an explicit "Look for agents already on this computer" press (`DISCOVERY_OPENED`); a full page reload starts collapsed again. Hermetic (file://), stubs /api/found-agents and /api/scan-agents, forces the Agents tab (`boardbar.hidden=false`), drives the real `paintFoundBoard`/`paintScanBoard`/`paintDiscoveryTrigger`. Six arms: (1) ON LOAD both panels stay hidden and only the trigger shows; (2) the EXPLICIT PRESS opens both panels (found AND scan, each asserted) and hides the trigger; (3) DISMISSED FOREVER (`body.dismissed`) does not re-offer the trigger and the empty-look message never misreports the dismissal; (4) LABEL RESET - a stale empty-look message is restored to the neutral label on a hidden->shown re-show TRANSITION (not on every shown poll); (5) LABEL WRITE GUARDED - a re-show does NOT rewrite the button text node when the label is unchanged, so its aria-live does not re-announce; (6) MESSAGE PERSISTS - an empty-look message is NOT wiped by the next poll while the trigger stays shown (no false transition). The load arm alone would pass on a page that never shows the panels at all, so the press arm is load-bearing. Controls, each traced and proven RED: removing the `DISCOVERY_OPENED` gate reds arm 1; a scan-only gate regression reds arm 2's `scanHidden` assertion; a wrongly-tracked `DISCOVERY_DISMISSED` reds arm 3; a dropped label reset reds arm 4; an unconditional label write reds arm 5; a reset that fires on every poll (not only the transition) reds arm 6. |
