...
| `render-firstrun-grok-3386.js` | kosmos#3386 (Josh, 2026-09-21): xAI Grok shown up front on the first-run model step (fr-pane-5) grouped with Gemini before the "Runs on this computer" tier. HERMETIC (loads web/index.html over file://, deep-links `?first-run=1&fr-step=5`): asserts the Grok/xAI tile is present with its "After setup" pill (#3566: it connects in Settings, AI Models), uses the "X" initial-letter chip (`.llm-chip`, no wrong-brand SVG - xAI has no vendor mark in this file), sits immediately after Gemini and before the tier divider, that the model step carries exactly nine not-connectable-here tiles, and that the chip lays out and renders the visible "X". Both themes. |
