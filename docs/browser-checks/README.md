...
| `render-composer-stroke.js` | The project composers (`#pj-post` room post box, `#pj-say` agent-say box) render with NO resting border AND still keep a visible boundary (a non-transparent recessed fill distinct from the bar behind them) in both the tab and consolidated layouts and both themes (Josh, 2026-09-21: the input reads cleaner as a filled field with no stroke). The boundary assertion is the #3369 lesson: border:0 alone left the field invisible in the light theme, where the box fill equalled its white container. CONTROL: the agent-dialogue composer (`#d-say`) keeps its border, proving the removal is scoped to the project composers and did not reach the base `.composerbox` rule. Hermetic (file://). Reds on origin/main, where the project composers still carry the 1px `--k-rule` border. |
