...
| `render-orgchart-phone-718.js` | kosmos#718 the org chart on a phone. Real server, five-agent fleet, at iPhone SE / iPhone 15 / Pro Max / mid Android (touch): the page is no wider than the screen, every agent is drawn, fully on screen and at least 44x44, and a tap on a face opens that agent; at 1400px the chart is the natural square from the page's own arithmetic, as before. Control, measured: on the unfixed page "no sideways scroll" fails at 375, 393 and 412 in both engines (page 420px); the other arms pass there. Gate runs Chromium; `ENGINES=chromium,webkit` by hand (WebKit is an engine approximation, not Safari). |
