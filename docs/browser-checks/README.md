...
| `render-model-change.js` | The Model section (#386): three chained menus, a change confirmed with its cost, the outcome reported in the dialog. Also the #1373 OpenAI sign-in picker: the rows offered, the preselect, and the negative arm that hides it again. Seals all three homes `defaultHome()` reads and drives a local OpenAI models stub with its own 401/200 control, so no real key is involved and the verdict does not move with network reachability. Writes `SHOT_1373` / `SHOT_1373_BEFORE` when those name a path. |
