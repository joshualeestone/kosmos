...
| `render-import-default-model.js` | kosmos#2453 follow-up (Mona, 2026-09-08): an IMPORTED OpenAI agent lands on the provider default model (a concrete base GPT), not "Let OpenAI choose". Hermetic (file://). Drives the real `paintOpenaiCreateModel` with `IMPORT_OPENAI_MODEL` set + a stubbed `/api/accounts/openai/models`: asserts it pre-selects the default when the account OFFERS that key, falls back to "" ("Let OpenAI choose") when it does NOT (graceful, never worse), and consumes the one-shot pref either way. Each assertion can return the dangerous answer. |
