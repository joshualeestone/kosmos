...
| `render-talk-fill-2622.js` | The Talk dialog box fills to the bottom of the window (#2622 part 2): `#d-talk-box` bottom lands near the viewport bottom at three window sizes (1100 and 700 tall two-column, and 500x900 narrow single-column below the 56rem breakpoint), the thread (`#d-dmthread`) scrolls INTERNALLY on a long conversation AND grows past the old 15rem cap (A5a/A5b), and the composer pins near the bottom. Scoping/regression arms: a non-Talk section (Model) stays content-height, and the wrapped nav row does not balloon at narrow width (A2c, guarding the `grid-template-rows` fix), so the `:has()`-gated fill is safe. The fill arms red on the pre-change page (box was content-height, sat far above the bottom). #3497 (wide layout): the box meets the header rule, the right edge and the bottom edge; the composer's bottom margin equals its side margins; no page scroll; the same under a 60px taller header; the identity block does not move and its scroll box starts below the back link; the build marker sits clear of the box; a phone-pairing card above and a connection notice below stay visible. |
