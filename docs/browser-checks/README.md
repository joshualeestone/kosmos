...
| `render-worldswitch-2238.js` | kosmos#2238: the multiple-Kosmos SWITCH is actionable (the 0.6.35 bug was read-only rows: create worked, selecting a world did nothing). Hermetic (file://) against a stateful board stub. Asserts a NON-active row is a real control (role=button) and clicking it POSTs `/api/worlds/active` with that world's id; on success the active marker + promoted name move to the chosen world and the restart banner appears (roots load at board boot, so the switch is honest a restart is needed); and "Restart now" POSTs `/api/board/restart` and, when the board reports it cannot self-restart (`{ok:false}` at HTTP 200 - branch on the BODY's ok, not res.ok), the copy tells the person to reopen Kosmos. Control: the pre-fix page has no row click handler, so the POST-called + marker-moved arms red on it. The LIVE board restart booting into the new world is verified on an install, not here. |
