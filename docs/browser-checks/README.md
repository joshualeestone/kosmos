...
| `render-no-left-bars-3692.js` | kosmos#3692 (Josh's rule, 2026-09-24): no solid left bars. Builds each card, note and warning that used to carry a left stripe on a sealed board, in light and dark, and asserts equal borders on all four sides and no x-only inset shadow. It leads with two controls that must detect a bar (a quote and an inset left shadow), and it asserts that the roadmap rules applied before judging the roadmap needs-you row. |
