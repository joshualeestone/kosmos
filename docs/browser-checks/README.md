...
| `render-dm-phone-718.js` | kosmos#718 (Josh: "make sure the designs fit well on mobile"). HERMETIC (file://, fetch stubbed). The DM thread at iPhone SE / 15 / Pro Max / mid Android widths, light+dark (ENGINES=chromium,webkit for both engines; WebKit is an engine approximation, not Safari): no bubble off either screen edge (an attachment message sat 112px off the left of an SE), every attachment card inside its bubble, table cells wrap at words and the wide table scrolls in `.mdtablewrap` instead of crushing to a letter per line, no sideways page scroll; at 1280 the attachment bubble keeps its desktop width. |
