...
| `render-worldswitch-2238.js` | kosmos#2238: the multiple-Kosmos SWITCH is actionable (the 0.6.35 bug was read-only rows: create worked, selecting a world did nothing). Hermetic (file://) against a stateful board stub. Asserts a NON-active row is a real, actionable `menuitem` (role=menuitem, tabindex=0) and clicking it POSTs `/api/worlds/active` with that world's id; on success the active marker + promoted name move to the chosen world and an HONEST status banner appears naming the world and saying it loads after a board restart (roots load only at board boot; the in-app auto-restart is a separate #2238 follow-up, so the slice does not claim the switch is live). Control: the pre-fix page has no row click handler, so the POST-called + marker-moved arms red on it. |
