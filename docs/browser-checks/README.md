...
| `render-fed-prod-gate-3330.js` | #3330: the #3312 federation UI is gated to the STAGING channel because its transport and coordinator are not built, so its external doors open onto nothing. The status tick stamps the update channel on `<html>` as `data-source-channel` (from `/api/status`), and one CSS rule `html:not([data-source-channel="staging"]) .pj-mode, #pj-add-ext-person, #pj-add-ext-agent, #pj-invite-panel, #pj-join-mode { display:none }` hides the entry points by default and on prod, revealing them only on staging. Both are COMPUTED results a source grep cannot see: a rule that lost to a more specific selector would leave the not-ready UI on a prod board, and a rule that hid the entry points in every channel would silently kill the staging review surface. HERMETIC (file://, fetch stubbed): un-hides the fed nodes' ancestors and clears the two follow-on panels' own `hidden` attr, then asserts prod AND the unstamped default hide all five entry points while staging shows them (the load-bearing prod-specific control), with `#pj-add-agent` (the local agent add) visible in every channel as the not-over-hidden control. |
