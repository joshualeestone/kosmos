...
| `render-working-pulse-3956.js` | kosmos#3956 (Josh, 2026-09-26): a working agent's green ground pulses slowly, a little greener and back. Boots a sandboxed board with one working and one idle agent. On chromium and webkit: the working grid card runs `working-pulse` (3s or more, infinite); its ground, sampled as sRGB through a canvas across one cycle, swings toward green and stays light at its greenest (light theme); watched across three five-second polls (at least two rebuilds asserted), a rebuilt card continues the pulse with no frame-to-frame jump, where without the phase pin it snapped back to white; a list row and a project member box placed into the real page carry the same animation, the row also under the one-screen folded layout (with its wash still drawn); in the dark theme the working ground still swings (the Kosmos+ navy theme is not measured). Controls: the idle card and an idle row do not pulse and the idle ground holds still over the same cycle. Under prefers-reduced-motion nothing pulses and the static wash remains. Removing the animation reds the pulse arms; removing the reduced-motion rule reds eight arms (the reduced-motion ground is sampled across a whole cycle); disabling pinWorkingPulse reds the rebuild arm on both engines. Self-hosted sandboxed server; headless-fine. |
