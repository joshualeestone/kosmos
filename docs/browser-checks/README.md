...
| `render-help-tips-3574.js` | First-run help and first-visit tips (#3574). On a fresh sandbox the welcome tour shows on the board, dims, and numbers four places with four pins (T1); Got it closes it and the board records it (T2); the ring tip then shows by itself with the three gauge colours and Escape counts as seen (T3), and the Agents tip follows (T3b); a reload brings nothing closed back (T4); the ? closes the user menu, reopens the tour and the ring explainer, and returns focus to itself (T5); Stop showing tips turns tips off in one write, the Settings switch reads off, nothing shows with tips off and nothing seen, and turning the switch on shows the tour from that same state (T6); New agent, Projects, inside a project, Settings and an agent's page each show their own tip once (T9); at 390 wide the card sits in the page with no arrow (T10); the card takes each scheme's elevated ground (T11); under the consolidated layout the tour finds the rail heads and its title counts what it numbers (T12); an unreadable store shows no tips and hides the switch (T7); no page errors (T8). |
