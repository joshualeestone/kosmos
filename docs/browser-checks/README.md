...
| `render-engmode-gate-2131.js` | The terminal/agent screen is gated on Engineering (Advanced) mode (#2131 regression guard). With eng-mode OFF the raw terminal (`.pj-viewport`, `#pj-thread`, the detail `#d-window`) is HIDDEN on both the project and conversation views; the CONTROL turns eng-mode ON and asserts the SAME elements become VISIBLE (so the OFF arm is not vacuous); and the SAFETY arm pins the exemption the fix must never break - an ASKING agent keeps its question panel (`#d-qask`) VISIBLE even in Off (it is how the answer is typed). The reported v0.6.28 leak does not reproduce on current main; this locks the invariant so it cannot silently return. |
