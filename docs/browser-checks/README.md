...
| `render-dm-phone-718.js` | kosmos#718 (Josh: "make sure the designs fit well on mobile"). HERMETIC (file://, fetch stubbed). The DM thread at iPhone SE / iPhone 15 / mid Android / Pro Max widths, light+dark, plus 900 and 1280: no bubble outside its row (a message with files sat 112px off the left of an SE because the person's content-sized bubble took the one-line file name's width; the fix is a cap on that bubble, and a 120-character name covers the wide windows too), every attachment card inside its bubble with its picture-preview slot laid out (the images themselves cannot load over file://), no table cell narrower than its longest word and the table's scroll box inside its bubble, neither the page nor the thread scrolls sideways, the gutter opposite each avatar equals avatar plus gap (#3340), the bubble tail's ground mask never reaches the avatar, every file name stays on one line, and at 1280 a short-named card's bubble still fits its content. The gate runs Chromium only; `ENGINES=chromium,webkit` by hand adds WebKit, an engine approximation, not Safari. |
