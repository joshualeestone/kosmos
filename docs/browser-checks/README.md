...
| `render-talk-fill-2622.js` | The Talk dialog box fills to the bottom of the window (#2622 part 2): `#d-talk-box` bottom lands near the viewport bottom at three window sizes (1100 and 700 tall two-column, and 500x900 narrow single-column below the 56rem breakpoint), the thread (`#d-dmthread`) scrolls INTERNALLY on a long conversation AND grows past the old 15rem cap (A5a/A5b), and the composer pins near the bottom. Scoping/regression arms: a non-Talk section (Model) stays content-height, and the wrapped nav row does not balloon at narrow width (A2c, guarding the `grid-template-rows` fix), so the `:has()`-gated fill is safe. The fill arms red on the pre-change page (box was content-height, sat far above the bottom). |
...
| `render-fed-plus-gate.js` | Federation Kosmos+ launch gate (Josh, 2026-09-21; supersedes the #3330 prod-channel fed gate): the #3312 federation UI now shows only to a paid Kosmos+ member on a live channel, a non-member/not-logged-in viewer gets a "sign up for Kosmos+" prompt instead, and before the coordinated flip prod shows nothing. `fedGateMode()` computes `show`/`signup`/`hidden` from `sourceChannel` + a `federationLive` flip flag + the viewer's `kosmos_plus` bool, and `fedGateStamp()` stamps it on `<html>` as `data-fed-ui`; two CSS rules hide the five fed entry points unless `show` and `#pj-plus-signup` unless `signup`. HERMETIC (file://, fetch stubbed): un-hides the gated nodes' ancestors, then drives the REAL `fedGateStamp` with fixture `/api/status` data across every mode and reads computed display. THE LEAK CONTROL: on a live prod, a non-member AND an unknown/absent `kosmos_plus` both keep the fed UI hidden and show the sign-up prompt (a member-only paid feature can never leak). Also asserts ready-to-flip (prod hidden until `federationLive`), staging review continuity, the no-first-tick-flash default, `#pj-add-agent` (local add) never gated, and that the sign-up button routes into the in-app Plus section. |
...
| `render-firstrun-grok-3386.js` | kosmos#3386 (Josh, 2026-09-21): xAI Grok shown up front on the first-run model step (fr-pane-5) as a "coming soon" tile, grouped with Gemini before the "Runs on this computer" tier. HERMETIC (loads web/index.html over file://, deep-links `?first-run=1&fr-step=5`): asserts the Grok/xAI tile is present with its "Coming soon" pill, uses the "X" initial-letter chip (`.llm-chip`, no wrong-brand SVG - xAI has no vendor mark in this file), sits immediately after Gemini and before the tier divider, that the model step carries exactly nine coming-soon tiles, and that the chip lays out and renders the visible "X". Both themes. |
...
- render-push-718.js -- #718: the PWA push client end-to-end -- `/sw.js` serves as a worker, the client registers it and it controls the page, and a REAL push delivered through the worker's own `push` handler (via the Chromium DevTools `ServiceWorker.deliverPushMessage` verb) shows the notification MAPPED from the coordinator's real `{kind, agent, project, id, address}` payload, with the click-through built from the Mac address. The notification-RENDER assertion is HEADED-ONLY: headless Chromium delivers the push to the handler but does not surface the notification to `getNotifications()`, so under `HEADED=0` (which includes the cut gate) that one read is skipped with a printed SKIP line while serve/register/control and the CDP delivery still run; the payload mapping itself is covered headless by the node suite `web.sw-718.test.js`. Chromium-only by design (Web Push + the CDP verb are Chromium's). Needs a first-run-complete board.
