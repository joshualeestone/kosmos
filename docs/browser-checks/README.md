...
| `render-firstrun-access-onebox.js` | Screen 2 (Access), #3031 (was #2910 + #2685). FILENAME IS HISTORICAL: it began at 0.6.39 #8 asserting ONE compact preview; #2910 grew it to SIX previews in two labeled groups, and #3031 (Josh 2026-09-14) cut it back to TWO previews side by side, one per app (Kosmos, tmux), each a single "would like to access files." prompt naming its app, with the folder-count labels and the group micro-note removed and the Don't Allow/Allow buttons no longer wrapping. #2685's "Terminal" -> "tmux" relabel stands (the bundled tmux is the process macOS holds responsible for agent file access, so it is the name on the real folder prompts). HERMETIC (loads web/index.html over file://, boots no server): unhides the `#fr-pane-2 .s2-dlg-fan` chain and reads static markup + computed style. Arms: exactly TWO `.s2-dlg`, no `.s2-appgrp` groups and no `.s2-applbl` labels; the two previews name Kosmos and tmux NOT Terminal; each preview uses the generic "would like to access files." line, no per-folder text, old "...in your folders." line gone; the folder-count labels and the `.s2-grpnote` micro-note are gone; the preview copy is compact dialog-sized (~12-14px/600), not the overlay body; every preview renders Don't Allow + Allow (count two); neither button wraps (white-space:nowrap + single-line height, and the two buttons in a row are equal height); the two previews sit side by side at the real ~552px onboarding pane width (same top, differing left), a guard the 1280px viewport would hide; every mock Allow is a live `.s2-mockallow` affordance; and the Allow button's `::after` draws a solid gold ring while Allow itself is the blue macOS default button. The ring is a computed `::after` a source read cannot see. Covers the FILE-access screen only; the functional `.s2-gate-row` grant and the S3 accessibility step are not asserted here. chromium + webkit. |
...
| `render-gated-next.js` | The S2/S3 permission-gated Next, driven for real: S2 file-access AND both S3 accessibility rows -- the app's grant (data-gate="tmux", /api/a11y-status) and tmux's own grant (data-gate="tmux-a11y", /api/tmux-a11y-status) -- disable Next on a measured not-granted reading and unlock on the grant (re-gated #2559/#2911); only sleep is advisory (#2587). Uncheckable (a browser, no FDA), a path-mismatch, tmux-not-yet-listed (present:false) and any fetch failure fail SAFE (never block, never false-green), so the re-gate can never re-trap (#2912); the poll (FR_GATE_POLL_MS, 750ms) re-checks so a grant unlocks on its own, and a "Check again" control (#2451/#2559) lets the user force it now. Separate arms isolate app-AX-alone-blocks, tmux-AX-alone-blocks, all-granted-enables, both-uncheckable-failsafe and tmux-not-yet-listed-advisory (red-capable both ways). Also covers #2587: a blocked sleep row never disables Next (a laptop that sleeps on battery cannot satisfy it), and the honest note replaces the useless Turn On on that battOnly row. Subsumes the retired render-a11y-gate-2125 (tmux gate) + render-sleep-button (sleep gate). |
