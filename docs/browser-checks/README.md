...
| `render-webhooks-1307.js` | A project's webhooks in its settings (#1307, Josh 2026-08-28). The Webhooks section sits directly below Project members with + New webhook; New webhook makes one named "Webhook 1", shows its full link once with a Copy button, "shows it only once" and the JSON hint, and focuses the link. POSTing JSON to that link adds a task marked with the webhook. Done hides the link and reopening settings never shows it again; the row says when it was last used. Renaming keeps the same link working under the new name. Delete asks first (Keep it cancels), then the row goes and the link answers 404. Light, dark, 390 wide, no sideways scroll, no page errors. Self-hosted sandboxed server; headless-fine. |
