...
| `render-help-tips-3574.js` | First-run help and first-visit tips (#3574). On a fresh sandbox the step-through welcome tour shows on the board at step 1 of 4 with the New agent button ringed, the rest dimmed and four dots (T1); Next walks Agents, Projects and your name, the last step says Got it with no Skip, and Got it closes it and the board records it (T2); the ring tip then shows by itself with the three gauge colours and Escape counts as seen (T3), and the Agents tip follows (T3b); a reload brings nothing closed back (T4); the ? closes the user menu, reopens the tour and the ring explainer, and returns focus to itself (T5); Stop showing tips turns tips off in one write, the Settings switch reads off, nothing shows with tips off and nothing seen, and turning the switch on shows the tour from that same state (T6); New agent, Projects, inside a project, Settings and an agent's page each show their own tip once (T9); at 390 wide the card is pinned 12px from each edge with no arrow (T10); the New agent tip sits beside the form's heading, over none of its fields (T9); closing an auto tip from the keyboard moves focus to the ? (T15); a tip steps aside while a dialog is open and returns after (T13); scrolling moves the card with its target (T14); leaving a screen closes its auto tip unrecorded and coming back shows it again (T16); one Escape closes an open dialog and leaves the tip behind it (T17); Skip ends the tour, the x closes a screen tip, a click outside ends the tour, and a click on a dialog over the tour does not end it (T18); Stop showing tips is a single save (T6); under the consolidated layout the tour does not start by itself over Settings and "this screen" there is the Settings tip, not the ring (T19); Escape with a picker open leaves a screen tip, and closes it with none open (T20); a header menu opened over a tip makes it step aside at once and return (T21); the tour replaced from the ? is recorded and does not come back (T22); the live region announces an auto tip by its title (T3); the card takes each scheme's elevated ground (T11); under the consolidated layout the tour still finds all four places on the rail heads (T12); an unreadable store shows no tips and hides the Tips box (T7); no page errors (T8). Who sees tips by themselves (Josh's decision 2026-09-24 18:10, only someone new): a board with agents at its first answer shows and records nothing (T23), and removing its last agent, even with tips turned on afterwards, brings no tour (T26, T27). On a new board, once there is an agent the tour counts as seen and the screen tips follow: after a tour that went without a click (T24), after an ordinary close whose save failed (T25), and after New agent was clicked before the tour showed, with the Make an agent tip shown there (T28). A failed tips read is mended by the next save (T29) and retried by itself (T30). Each of T23-T30 was run against the code with its fix removed and failed. |
