...
| `render-firstrun-keyed-connect-3658.js` | kosmos#3658 (Josh, 2026-09-24 17:46). HERMETIC (file://, fetch stubbed). First run's model step: one uninterrupted list starting Claude, GPT, Gemini, Grok, with no "Runs on this computer" heading, no "After setup" pill and no "connect later in Settings" line; Gemini's and Grok's gold Connect open the shared key box for that provider and switching clears the other's key; an empty Add is refused without a request; Add POSTs the trimmed key to /api/accounts/<route>/apikey, closes the box and turns the row into a disabled Connected; a missing runner (needsRunner) is said plainly and the box stays. #3713: a missing tool now opens the download box (naming the tool and its size) instead of a dead end; Not now closes it; a refused download says why and re-arms; a good one shows its progress from the engine's byte counts and then opens the key box by itself. |
| `render-keyed-install-3713.js` | kosmos#3713: Connect installs the Gemini CLI and the Grok CLI itself. HERMETIC (file://, fetch stubbed; the install is a fake job that reports downloading, then present). First run, Grok: the download, then Grok's box on its sign-in with the key beside it. Settings: a missing gemini or grok shows the download step instead of the key step or Grok's choice, with the tool and its size; Download shows progress, then the next step by itself; a present tool goes straight on (control); switching provider mid-download drops the box and the finished download paints nothing; a tool gone missing before Add leads to the download step. |
...
| `render-grok-subscription-3391.js` | kosmos#3391 part 2. HERMETIC (file://, fetch stubbed, the status poll ticked by hand). Settings > AI Models: picking Grok shows a subscription-or-key choice with the key step hidden; the sign-in POSTs start with no reauthDir, shows xAI's link and code and a way to stop, and on connected shows the gold success box with the email; an engine error is said in its own words, a missing `grok` names the tool, Stop and closing the dialog cancel on the engine and a late answer paints nothing. A Grok subscription row's Sign in again opens straight onto the sign-in with that account as reauthDir, and a fresh add afterwards is not a sign-in again. First run: Grok's box offers the sign-in beside the key, closing it cancels the sign-in, and connected closes the box, names the account and leaves Gemini's row alone. |
