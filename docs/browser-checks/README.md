...
| `render-subproject-columns-3135.js` | Hierarchical projects LIST columns (#3135, Josh 6.68 "it keeps the headline of each project at 100% width... can't even see half of my projects... keep everything inside of one page"): drives the shipped `paintProjects`/`projectCard` against a fixture tree (12 top-level projects + a k/app/site subtree) in the non-consolidated list view. Asserts: the list is columnised (`column-width` set, not a single flex column) and a wide page (1600px) shows the projects across more than one column; a child row carries `break-before:avoid` and every row `break-inside:avoid` (a subtree stays together in one column, no child orphaned at a column top); each row is a compact card (description stacks under the name, not on the same line); the per-depth indent is unchanged (hierarchy kept); and the CONTROL that a narrow page (460px) falls back to a single column. Each arm returns the dangerous answer on origin/main (one 100%-width flex column). |
