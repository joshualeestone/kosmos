...
| `render-no-left-bars-3692.js` | kosmos#3692 (Josh's rule, 2026-09-24): no solid left bars. Builds each card, note and warning that used to carry a left stripe on a sealed board, in four arms: light, dark (media query), explicit `data-theme="dark"`, and the Kosmos+ navy look (`body.plus-active`) on a light-mode Mac, which must take the dark warn tint and the dark roadmap ring. It asserts no left bar (equal borders on all four sides, no x-only inset shadow) and that each element carries its own replacement: a hairline, a hairline plus warn tint, a tint (the unsure room message, which must differ from a plain one), or the roadmap ring. It leads with two controls that must detect a bar, asserts that the roadmap rules applied before judging the roadmap needs-you row, and checks that the explicit dark theme takes the dark ring. |
