...
| `render-autohello-switch-2716.js` | kosmos#2716 (follow-up to #2686): the model-switch and provider-switch dialogs auto-send the wake `hello` after they restart the agent, confirming it in the changeDialog message (chg-msg) via the shared `autoHelloAfterRestart`. HERMETIC (file://, fetch stubbed). Drives the wiring helper `autoHelloOnSwitchRestart` and asserts: a restarting->idle recovery POSTs one `hello` and updates chg-msg to "Reactivated on <provider>, and said hello"; an unconfirmed delivery and a stays-restarting timeout leave the manual "Say hello to reactivate" line (no false confirmation); and the guarded write is SUPPRESSED when the dialog was closed mid-wait, the person switched agents, or a different dialog was reopened (the confirmation never lands in a closed or another agent's dialog). It also covers a report that arrives BEFORE changeDialog's floor paints the manual line (the hold is 4400ms, the readiness accept can come at 4000ms): the confirmation still lands once the line is painted, nothing is written if the dialog closes first, and the wait gives up after RESTART_HOLD_MS + 1s. |
