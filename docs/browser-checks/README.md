...
| `render-orgchart-phone-718.js` | kosmos#718 the org chart on a phone. Real server, five-agent fleet, at iPhone SE / iPhone 15 / Pro Max / mid Android (touch): the page is no wider than the screen, every agent is drawn, fully on screen and at least 44x44, and a tap on a face opens that agent; at 1400px the chart is the natural square from the page's own arithmetic, as before. Then a three-level fleet at 375px, too deep to fit even at the squeeze floor: the page still does not scroll sideways, the chart's own box does and opens centred on the hub (and stays centred when narrowed to 360px), a node held at the top edge keeps its name callout inside that box, the chart allows touch panning, and (Chromium) a real touch swipe scrolls the box. A chart that fits (every phone size, and desktop) is not a scroller, so it clips nothing. Controls, measured: on the unfixed page "no sideways scroll" fails at 375, 393 and 412 in both engines (page 420px); the other flat-fleet arms pass there. With the `.orgmap.orgwide` touch rule removed, the deep-fleet touch-action and swipe arms fail (scrollLeft 0). An always-on scroller fails the does-not-clip arms; without the scroller's top padding the held node's callout is cut off (45px); without the centring the box opens at 0; keeping the old scroll offset across a width change fails the narrowed arm. Gate runs Chromium; `ENGINES=chromium,webkit` by hand (WebKit is an engine approximation, not Safari). |
