...
| `render-user-menu-3051.js` | kosmos#3051 (Josh 0.6.63, for 6.65): the upper-right nav is a user avatar + name button (`#userpop-btn`) that opens a dropdown (`#userpop-menu`). HERMETIC (loads web/index.html over file://, boots no server). Asserts the top nav is exactly Agents + Projects (no Settings tab), the button renders (avatar slot + name), the menu is closed by default (aria-expanded=false), opening it reveals the four items (the Settings link `#userpop-settings`, the light/dark `.themepick`, the board-view `.laypick`, the agent-status `#checked`), the Settings link opens `#panel-settings` with no tab lit (`settings` is in `BUTTONLESS`) and closes the menu, and Escape + an outside click each close it. Both themes. Reds on origin/main, where there is a third Settings tab, no `#userpop`, and the light/dark + view controls sit bare on the header row. |
