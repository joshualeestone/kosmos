...
| `render-grok-subscription-3391.js` | kosmos#3391 part 2, updated for #3731: a Grok SUBSCRIPTION sign-in from Settings > AI Models and from first run, and a row signing in again. HERMETIC (file://, fetch stubbed; the status poll is ticked by hand). Settings: the choice, the key step, the sign-in (link, code, Stop), connected, errors, a missing grok (the install; Windows says it plainly), Stop, closing, switching, a late start, sign in again. First run, in Grok's own panel: the choice, the sign-in starts at once as a new account, choosing the key or another provider or leaving the step cancels it on the engine, connected closes the panel and names the account. |
