...
| `render-agent-files-3614.js` | The agent page Files block (#3614), as #3757 reshaped it (Josh, 0.6.94): on screen directly under the four-pack in its column (both themes, 760 wide, and 412px phone Profile); an agent with files lists them newest first with a date and a size, with no Open in Finder beside them; an agent with no files shows no Files section at all; more files than the list's ten shows View All at the right of the header, which opens the Files screen (#d-sec-files) listing every file with Open in Finder; a row on either list reaches the opener; Open in Finder makes the folder on first use; the nav's labels are the agent title's size and its boxes are shorter (84 and 73px before, measured) with icons and the two-across grid unchanged. #718 pins 44px minimum targets for both sets of file rows, View All, and the Files-screen Direct Message back control on a 412px phone, with the measured 32px, 24px, and 15px desktop geometry unchanged. Shots to argv[2]. |
| `render-tasks-view-3559.js` | The Tasks view (#3559). The 25-task gate (Josh): below 25 tasks ever, the Tasks tab and the consolidated rail's Tasks button are hidden after a real status tick, and shown once the saved flag is set; then the tab opens `#panel-tasks`, and in the consolidated view (tab bar hidden) the rail button does. The tiles are exactly the three OPEN groups the engine can prove (Nobody on it, Assigned not started, In progress) with the right counts; Closed is the folded list, its count read off the fold (Mona's look review of #3701); no Waiting-on-you, Done-check-it or category is drawn. A task its agent names sits under In progress; in status grouping no row repeats its group's name, and grouped by project each row carries its state. Both halved gaps are measured, in the consolidated column too, and a message appearing does not move the list. Search finds tasks by agent name and by what they say, and combines with the project rail; Group by Project regroups them; ticking two rows and Close them closes both with the note on each history. No element in the view carries a coloured left border (Josh, 2026-09-24). Light, dark, and 760 wide (the rail becomes a dropdown). Self-hosted sandboxed server; headless-fine. |
| `render-stale-auth-1930.js` | The "haunted" board (#1930, found by an external tester). An agent whose pane still holds a real Claude 401 envelope from before a sign-in repair, with its launch job on file so the engine asks the account. With the account check reading connected, the board status does not report auth_failed and the card does not say "Sign-in isn't working" (the stale rejection is set aside). Control: the same pane with the account check reading signed out reports auth_failed and the card says so. Sub-case 2 (a live 401 loop under a cached healthy check) is pinned by engine/status.authfresh-1930.test.js. Self-hosted sandboxed server; headless-fine. |
| `render-subtasks-3861.js` | Subtasks on the screen (#3861 part 2). In the Tasks view a subtask sits indented under its parent when both are in the same list, two levels deep; a third level is drawn at the second and names its parent ("Part of #N ..."), and a row its indent explains says nothing more. A parent carries its direct-subtasks chip ("1/3"), which folds the family away and back (aria-expanded follows). A closed child under an open parent stays in Closed and names its parent. A parent whose subtasks are all closed says "all subtasks done", and Close it closes it. The project column nests the same way under its five-cap and shows the chip. A task page lists its direct subtasks (Open/Done), a subtask names what it is part of, and "+ Add subtask" opens New task with Part of preset; the new task is filed under it and the page stays put; a plain New task starts at None. Light, dark, and 390 wide, no sideways scroll, no page errors. Self-hosted sandboxed server; headless-fine. |
...
| `render-nav-badges-3216.js` | kosmos#3216 (Josh 6.74): red unread-count badges on the Agents and Projects nav tabs. Hermetic (file://). Asserts the badge spans sit on the correct tabs, the badge sits within its tab (geometry, not clipped), and the `setNavBadge()` render contract: a positive count shows the number, zero/null hides it (display:none, never a guessed zero), over-99 caps at `99+`, the badge is the shared unread red (#b3261e/white) in light mode and swaps to the family dark red (#ff8c82/#0c0d0f) via BOTH dark rules (the `[data-theme=dark]` toggle and the `@media (prefers-color-scheme: dark)` auto-detect path). The two data wirings are asserted by source (seeding real per-agent `dmUnread` and per-project `unread` end-to-end is engine-derived and not hermetically reproducible): the Agents badge is set from the fleet `dmTotal` in tick(), and the Projects badge is set from the always-polled server total `c.projectsUnread` minus the open room's own unread (option i, agreed with Angel: subtracting the open room prevents the badge flashing a count for the room you are reading during the async /seen window; an archived open project is not subtracted). Badges live in the tab bar only (hidden in the consolidated layout, a deliberate follow-up cut). |
...
| `render-agentpage-fullwidth-2012.js` | The full-width agent page (#2012): the content column fills the width past the old 544px cap, `#d-window` fills the page height past the old 560px cap, and the message body keeps a ~66ch measure. #3385 moved the header into a 220px left identity column, so assertion 2 was rewritten from "header spans full width" to the left-column reality (a narrow header sitting to the left of the content), which reds on the pre-#3385 full-width banner; the other arms still compare against the old #2012 caps so they red on the pre-#2012 page. |
| `render-talk-fill-2622.js` | The agent Talk box (#2622, #3497). WIDE layout: the box meets the header rule, the right edge and the bottom edge; the composer's bottom margin equals its side margins; no page scroll; the same under a 60px taller header; the identity block does not move and its scroll box starts below the back link; a phone-pairing card above and a connection notice below stay visible (A1-A1k at 1400x1100, A2 at 1400x700, the edge arms again at Josh's 1000x660 as A1l). The wide Talk view drops the #1309 scrollbar gutter and the board, Model and narrow Talk keep it (A1m); with real 15px scrollbars (their own Chromium, --hide-scrollbars off, and WebKit, the Mac app's engine) at 1000x660 the box reaches the window edge and the header does not move between the board, Talk and Model (A1n); the edge arm is a control only where the runner reserves a 15px gutter on a page that does not scroll (a Mac showing scrollbars) and prints a NOTE that it is a guard where it reserves none (overlay scrollbars); so a cut on a Mac with overlay scrollbars (Automatic, no mouse) does not exercise the edge control, only the header no-move arm; the header's right padding is 24px plus the gutter the root reserves (measured off html itself) less the width the page gives up on that view, and the plain 24px under the consolidated layout preference (A1o); the width is re-measured on resize, on a pointer press and when the Windows stamp is applied (A1p; the Windows width itself is unverified headless); if the width cannot be measured, Talk keeps the gutter and the header is not padded (A1q), an engine without scrollbar-gutter is never marked measured (A1s), nor one whose 100vw leaves out the gutter (A1t); leaving the consolidated layout re-measures (A1u). NARROW (500x900): the page scrolls, the header is not sticky, and the box is its own window-tall block; scrolled to it, and at the end of the page, the whole box from its header row to the composer is on screen and uncovered (A2b, A2d, A2e); focusing the box keeps the composer on screen (A2f). Also: the thread scrolls internally with the cap lifted (A5), a long thread in a short window keeps the composer reachable and the page does not scroll (A9, A9c), a non-Talk section stays content-height (A6), no page errors (A7). |
| `render-help-tips-3574.js` | First-run help and first-visit tips (#3574). On a fresh sandbox the step-through welcome tour shows on the board at step 1 of 4 with the New agent button ringed, the rest dimmed and four dots (T1); Next walks Agents, Projects and your name, the last step says Got it with no Skip, and Got it closes it and the board records it (T2); the Agents tip shows on the board, and an agent's page shows its ring first (1 of 6, beside it, the three gauge colours, Josh's words), then one step on each of her five buttons in his words, each ringed and pointed at, and Got it records them (T3, T34; with no memory reading there is no ring step, 1 of 5, and the ring's explainer then shows by itself beside it the first time her page draws one, T3c, T3e); a reload brings nothing closed back (T4); there is no "?" or help menu and no tip or Settings line points at one (T5, #3755); Stop showing tips turns tips off in one write, the Settings switch reads off, nothing shows with tips off and nothing seen, and turning the switch on shows the tour from that same state (T6); Create Agent shows no tip, and Projects (Setup a Project, Josh's words), inside a project and an agent's page each show theirs once, Settings none (T9); inside a project four steps each ring its own area, Members, Conversation, Files, Tasks (T35); at 390 wide the card is pinned 12px from each edge with no arrow (T10); closing an auto tip from the keyboard leaves focus on the page (T15); a tip steps aside while a dialog is open and returns after (T13); scrolling moves the card with its target (T14); leaving a screen closes its auto tip unrecorded and coming back shows it again (T16); one Escape closes an open dialog and leaves the tip behind it (T17); Skip ends the tour, the x closes a screen tip, a click outside ends the tour, and a click on a dialog over the tour does not end it (T18); Stop showing tips is a single save (T6); under the consolidated layout the tour does not start by itself over Settings (T19); Escape with a picker open leaves a screen tip, and closes it with none open (T20); a header menu opened over a tip makes it step aside at once and return (T21); the live region announces an auto tip by its title (T3); the card takes each scheme's elevated ground (T11); under the consolidated layout the tour still finds all four places on the rail heads (T12); an unreadable store shows no tips and hides the Tips box (T7); no page errors (T8). Who sees tips by themselves (Josh's decision 2026-09-24 18:10, only someone new): a board with agents at its first answer shows and records nothing (T23), and removing its last agent, even with tips turned on afterwards, brings no tour (T26, T27). On a new board, once there is an agent the tour counts as seen and the screen tips follow: after a tour that went without a click (T24), after an ordinary close whose save failed (T25), and after New agent was clicked before the tour showed, with no tip on the create form (T28). A failed tips read is mended by the next save (T29) and retried by itself (T30). Each of T23-T30 was run against the code with its fix removed and failed. |
| `render-assistant-bubble-3034.js` | The setup assistant bubble (#3034, Josh 2026-09-24: bottom-right app-wide, context aware; 18:02 the first-x choice and a Settings switch). The board is real (GET /api/setup-guide, /api/settings setupAssistant, the page-context POSTs); the guide's thread is answered by a route in the check. With no guide nothing is drawn and Settings has no row (B1; control B2). With a guide: the bubble with the guide's picture 16px from the content corner and the nudge (B2); clicking opens the chat in place, headed "Josh, Kosmos Guide" with no pill and no footer, the page's one opening message first, the placeholder "Ask about Kosmos…", the guide's bubble the DM cream and the person's the DM blue (by token), neither cut intro line anywhere in the page, the cursor in the box, the page still usable, and the guide told the screen (B3, #3738); a screen change is reported (B4); sending reaches the thread once and empties the box (B5); the minus folds it, a reply while folded lights the dot, opening clears it, and the nudge never returns after writing (B6); the first x asks once, Close for now keeps it and records the ask, a later x after writing in the chat just closes (B7); opened again with nothing typed (an old unsent draft does not count), x asks again although answered, and Close for now keeps the bubble (B7c, #3034, Josh 09-25 07:38); the ask reads Close for now, Close forever and "You can turn it back on under Settings > Computer", and that Settings section is named Computer (B7b, #3758); Close forever turns it off and Settings > Help shows the switch off, which brings it back (B8); hidden during first run (B9, with control); a guide created after the page loaded arrives without a reload (B1b); a reply already in the thread lights no dot on load (B2); an unconfirmed-and-unkept send and a failed send keep the words in the box and say so (B5b, B5c); a guide gone while folded is noticed on the next read and forgotten (B10), and a fresh page with none shows none (B10b); a refused page report with 409 keeps the guide and the open chat (B12); a settings read that fails at load is retried (B13); a folder that is not the guide's (a new agent took the name, 409 not-guide) is not used as the guide, with its control (B14); while the guide works, the open chat shows the app's working row with one continuous set of dots, the thread stays at its end as the row shows and a reply that lands is in view, a landed reply outranks a stale "working", working is said in its own live region and does not replace a reply, folded the bubble shows the dots, a broken sign-in says so and is spoken once in words, and idle shows neither (B20, #3733). Fixtures use the production slug ('josh'). chromium. |
| `render-swarm-ui-3564.js` | Agent Swarms, the UI (#3564; engine is Renet's), built as the approved mock (chaoskosmos-site design/agent-swarms.html, c8284df) against the contract on #3564. The board is real; the engine's side (the /api/status `swarms` flag, each row's `swarm` field, the swarm routes) is answered in the check as the contract says. Without the flag New agent offers no Swarm choice and no card is a cluster (S1, control S2). With it: the Agent / Swarm choice, Swarm's settings, Claude held, Make this swarm (S2); the slider moves the value, the warning (helpers plus the lead) and the cluster, the limit in full (S3); the create request carries kind, maxHelpers, dailyTokenLimit, and a plain Agent none of them (S4, S4b); the swarm card is its cluster with the working/most badge and 'Swarm · 3 of 5 helpers working', an ordinary card is not (S5); the swarm's page shows Active / Paused, Stop now, Most helpers and Today (tokens in full, the bar), an ordinary page does not (S6); the controls send the contract's PUT / POST (S7); a limit pause says so and disables Stop now (S8); Members shows the swarm's cluster and On / Off, and Off sends PUT /api/project/<id>/swarm/<name> (S9). chromium. |
| `render-assistant-hosted-3660.js` | The setup assistant on Kosmos's own model, the bubble side (#3660, Josh 2026-09-24 18:07). With no guide, the bubble asks POST /api/setup-guide/hosted, but only where GET /api/setup-guide says `hosted` (a connector at a real path; H1 CONTROL: none, no bubble). It checks the bundled picture is served, the note that says where it runs, and the question with its screen. The conversation is carried, the allowance is said when low, a refusal keeps the words in the box, and the conversation survives a reload. When a guide is made, the bubble moves to it. A 501 connector says so and steps aside, and the switch off hides it. The fallback (Josh 2026-09-25 07:22): a guide whose own model cannot answer (the board's own_model_failing, given in the check) gets one line saying so with the fix, and its questions go to the backup, not its thread (H27); a backup refusal of own_model says their own AI is answering again, keeps the words and ends nothing (H28); once the board says the guide answers, questions go to its thread and the line goes (H29). 🛑 The connector is a fake and the hosted route is answered in the check; H12 asserts nothing reached a coordinator. |
| `render-plus-bar-3837.js` | The Kosmos+ remote bar (#3837 F, Josh 2026-09-25 17:21-17:22: "a tiny KOSMOS+ logo on the left, and a Log Out on the far right with a blue background"). The real board, reached two ways: through 127.0.0.1 there is no bar (P1, the CONTROL); through remote.test (mapped to loopback, accepted by the board's AGENT_WORKFORCE_ALLOWED_HOSTS opt-in) the bar is the sticky header's first row, edge to edge at the top, the real Kosmos+ dots drawn on its left, a blue Log out on its right, on the Kosmos+ navy, and it stays at the top when scrolled (P2). A refused log out says so and leaves the page as it was (P3); one that works posts once to the tunnel's /_kosmos/logout and goes to the address's start (P4). Under the consolidated layout it is still the first row, edge to edge (P5). No page errors (P6). The tunnel's route is kosmos-relay's (door.rs 6b). chromium. |
...
| `render-cluster-reorder-2929.js` | Consolidated cluster drag-reorder (#2929 slice 2, Josh 6.59 QA "drag it up to the very top"): a whole top-level project cluster can be dragged to reorder it in the consolidated view. Drives the shipped `paintProjects`/`pjTreeRows`/`wirePjClusterDrag` against a real fixture tree (a,b,c + a1 under a) by dispatching HTML5 DragEvents with a shared DataTransfer (mouse-drag does not fire native drag events). Asserts: pre-drag top order is the sorted a,b,c with a1 under a and only top-level rows draggable; dragging Charlie onto Alpha's upper half makes the top order c,a,b with a1 STILL under a (the cluster moves as a unit, only top-level reorders); the new order persists to `localStorage['kosmos.order.projects']` and `PJ_ORDER`; choosing a sort CLEARS the manual order and reverts (mutually exclusive, last action wins); and the key scoping control, the tab (non-consolidated) view IGNORES `PJ_ORDER` and keeps the sorted order. #3127: also the drag placement LINE (upper-half dragover sets `.pj-drop-before`, lower-half `.pj-drop-after`, moving off a row clears it, over the dragged row shows none, and it is gone after drop+dragend). Both themes. |
...
| `render-projects-roadmap-3276.js` | kosmos#3276 (Josh, 2026-09-18): the Projects board settles on TWO views, Grid + Roadmap, replacing three (Grid / List / org-chart Map). The Roadmap is the indented tree upgraded to mock2. Hermetic (file://). Drives the real Projects viewtoggle against an injected tree and asserts: the toggle offers exactly Grid + Roadmap (no List, no Map button); the Roadmap shows the tree (`#pj-list`, not `.asgrid`) and sets `body.pj-roadmap`; a parent row shows a VISIBLE fold caret (a leaf none) whose click COLLAPSES the branch (child computed `display:none`, `aria-expanded=false`) and re-EXPANDS it; the density drops the description + faces while keeping the agent-count pill, with the GRID still showing faces as the scope control; the top strip is shown only in the Roadmap, its summary reflects the count, and its Collapse all / Expand all fold every branch; and all of it holds in forced + system dark. Every assertion can return the dangerous answer. (Replaces the retired org-chart Map check, removed with #3276.) |
...
| `render-worldhide-2935.js` | kosmos#2935 (Josh's soft-delete ruling): hide (soft-delete) a Kosmos from its settings cog. Hiding drops a Kosmos off the Kosmoses list but the on-disk store stays accessible, so the copy must SAY the files remain and where, and Kosmos 1 is never hideable. HERMETIC (loads web/index.html over file://, boots no server, fetch stubbed): calls the page's real `worldswRender()` with three fixture worlds (default + two named), opens each Kosmos's settings pane and asserts: a NAMED Kosmos shows a "Hide this Kosmos" section whose hint states the files are not deleted and where they stay, and whose confirm copy restates that they remain accessible; hiding is a deliberate two step ("Hide from my Kosmoses list" reveals a confirm row, Cancel returns to step 1 with NO POST, only "Hide" there POSTs `POST /api/worlds/hide` with `{ id }`); on success the modal closes and the switcher refetches (worldsFetch -> plain `/api/worlds`, not `/api/worlds/list`); a route refusal (409 EACTIVE for the active world) surfaces its `because` text and leaves the modal open; and Kosmos 1's pane has NO hide section (the default is never hideable). Reds on a page with no hide section or a one-step (no confirm) hide. The engine `hideWorld` + the route are unit-tested separately (engine.worlds-hide-2935 + server.test.js #2935). |
| `render-openai-install-refusal.js` | Founder on a clean Windows 11 laptop (prod 0.6.72): first-run GPT card, Confirm, then "We could not start that install." with no reason, because the engine's reason sat at `job.because` and the handler read only `out.error || out.because`. HERMETIC (file://, fetch stubbed): clicks the REAL `#fr-openai-confirm-go` and `#acct-openai-install-go`. Asserts a refusal in the old engine shape paints the reason (not the empty fallback), hides the bar and re-enables Confirm; an install that starts and then FAILS paints the job's reason within seconds instead of the five-minute "taking longer" line; Settings paints the reason instead of sticking on "Starting..." (its catch used to call an out-of-scope `showBar` and throw). Control: an install whose runner appears clears the line and opens the picker. The pure sentence function is unit-tested in web.runner-install-refusal.test.js. |
| `render-fed-plus-gate.js` | Federation Kosmos+ gate (#3330 -> Josh 2026-09-21 -> #3495). `fedGateMode()` computes `show`/`signup`/`hidden` from `sourceChannel` + the `federationLive` flip + the viewer's `kosmos_plus` bool; `fedGateStamp()` stamps it on `<html>` as `data-fed-ui` (and `data-fed-member`). The Create/Join toggle and both Add-external buttons always show; only the Plus-only content (`#pj-invite-panel`, `#pj-join-mode`) is display-gated to `show`. A non-`show` viewer who picks Join or an Add-external button gets the shared `#plus-gate-modal` with Josh's verbatim copy; a member gated only by the pre-flip gets a coming-soon line with no Sign up button. HERMETIC (file://, fetch stubbed): drives the REAL `fedGateStamp` and the real Join/Add-external handlers across every mode, reads computed display and the modal's open state + copy, asserts the removed standalone `#pj-plus-signup` card stays gone, and covers the allowed branch (a live member is not gated). |
| `render-fed-external-3311.js` | kosmos#3311: a message from outside, in a federated project's room. Renders `external` rows through the page's own `pjRoomRow`, light and dark: marked `.msg.ext` with an External / External agent tag, the sender's name and words stay TEXT (an HTML payload creates no element), line breaks survive, the tag is drawn. HERMETIC (file://). Control: dropping `esc()` on the name reds the text and no-element arms. |
...
| `render-worldsw-lockout-3055.js` | kosmos#3055: the Kosmos switcher must not FAIL CLOSED. `#worldsw` is hidden by default and `worldsFetch` (called once on load, no retry) used to bail on any non-ok `/api/worlds` (a not-signed-in board 500s after a broken switch) or an unreachable board, leaving the switcher hidden forever with no code path to show it again -- the user was locked out of their Kosmoses with no way back (Josh, 0.6.63). The fix persists the last-known world list (localStorage) and renders the switcher from it on a failed read, so the user can always switch back to a working Kosmos; an honest "last-known list" note shows in the switcher's own status banner (guarded so a live switch never fights it), and `worldsFetch` retries so a recovered board self-heals to live. HERMETIC (file://), `/api/worlds` stubbed, localStorage seeded per arm: a 500-with-cache AND an unreachable-with-cache both un-hide `#worldsw` with the last-known rows + the note; a live read shows live, clears the note, and caches the list. CONTROL: a failed read with NO cache stays hidden (today's clean single-world degrade), the arm that reds a naive always-show fix. Reds on the pre-fix `worldsFetch` (the 500-with-cache arm leaves the switcher hidden). |
...
| `render-docs-subfolders-2245.js` | "Files in this project" lists subfolder files (#2245). HERMETIC (loads web/index.html over file://, stubs `/api/project/<id>/documents`) and drives the shipped `pjLoadDocs` and `pjLinkPaths`. Arms, with the list truncated and complete: a subfolder file is a row named and opened by its relative path; two files both named `report.pdf` stay distinct rows; a truncated list shows the partial-list note and a complete one does not (the control); a cited `sub/report.pdf` chips to itself rather than the top-level twin; an absolute cite finds the subfolder file; a bare `report.pdf` still chips to the top-level file (the control). 14/14 on the fix; 5 fail on the pre-#2245 page, including the wrong-file chip. |
...
| `render-firstrun-grok-3386.js` | kosmos#3386 (Josh, 2026-09-21), #3658, #3708: xAI Grok on the first-run model step (fr-pane-5). HERMETIC (loads web/index.html over file://, deep-links `?first-run=1&fr-step=5`): asserts the Grok row connects right there (gold Connect `fr-grok-connect`, no pill), sits immediately after Gemini in one list with no tier divider, that seven not-yet-available tiles follow, that Grok wears its real vendor mark (`data-pmark="xai"`, an inlined SVG, no `.llm-chip`) laid out at a real size, that its subtitle reads "xAI · subscription or API key today", and that the Settings pickers get the same mark through `providerMarkNode('xai')`. Both themes. |
| `render-conn-top-3708.js` | kosmos#3708 (from the 0.6.94 QA): the "Kosmos cannot reach a Claude subscription" line (`#conn`) is at the top of every view. Boots a sandboxed board with no Claude account so the line shows (a precondition arm fails if it never does), then on Agents, Settings > Updates, Projects, an agent's Talk page and New agent asserts the line sits below the header, above that view's content, and at the same position as on Agents; and that the Talk page still fits the window. Moving `#conn` back below the panels reds 8 arms; dropping its Talk-page top gap reds 1. |
| `render-firstrun-keyed-connect-3658.js` | kosmos#3658, rebuilt for #3731 (Josh, 2026-09-25, testing 0.6.94): on first run's model step Gemini and Grok connect EXACTLY like GPT, each directly under its own row. HERMETIC (file://, fetch stubbed). The list (one list, Claude, GPT, Gemini, Grok first); each provider's panels sit right under its row, with no command-line or "runner" wording; a missing tool shows GPT's install confirm in GPT's words and never a key box; Not now; a refused install; the real progress; then Gemini's key step (its subscription waits on #3568) and Grok's choice; Grok's "Sign in with Subscription" starts xAI's sign-in at once with no second screen; the key (empty Add, a switch mid-request, an unconfirmed key, away and back, Enter, the accessible name, a slow read, step entry); on Windows a missing tool offers the same install as the Mac. Screenshots to argv[2]. |
| `render-keyed-install-3713.js` | kosmos#3713, updated for #3731: Connect installs the Gemini CLI and the Grok CLI itself. HERMETIC (file://, fetch stubbed; the install is a fake job). First run: a missing grok offers GPT's install under Grok's row, then Grok's choice; leaving first run stops the watcher; a return joins a running install; Not now returns focus; a Grok sign-in with no grok offers the install in place; Windows offers the same install as the Mac (Connect and Add), and Settings on Windows shows the same download step. Settings: GPT's install step instead of the key step or Grok's choice, its progress, the next step by itself, a present tool goes straight on, a switch mid-install paints nothing, a tool gone before Add leads to the install, a slow probe starts it once, focus after a sign in again. |
| `render-no-conflict-3729.js` | kosmos#3729 (Josh, 2026-09-25 07:32, testing 0.6.94): no agent-status sentence on any agent surface, and no slot for one. Boots a sandboxed board with four agents and INJECTS into every agent object on every /api response every `conflict` sentence in engine/status.js (both the plain and the ternary shape; indexed by a running total, and asserted that all of them reached the page), and turns one agent unknown and one needs-trust. On the grid, list, org chart, an agent page, the unknown agent's page, the Projects tab, a project's member boxes (a real project with the unknown and needs-trust agents as members) and the one-screen layout (each asserted to be showing, the needs-trust card asserted to exist with its note on the grid), on Mac and Windows: no conflict sentence and no unknown-card "Not the same as idle" note on screen, no #d-conflict, no note on any visible card except the needs-trust one (it asks the person to act), no empty note. Against main's page it fails 16 arms; the sentence itself shows on the grid and both agent pages, the other surfaces fail on the slot alone (main never rendered it there). |
| `render-no-left-bars-3692.js` | kosmos#3692 (Josh's rule, 2026-09-24): no solid left bars. Builds each card, note and warning that used to carry a left stripe on a sealed board, in four arms: light, dark (media query), explicit `data-theme="dark"`, and the Kosmos+ navy look (`body.plus-active`) on a light-mode Mac, which must take the dark warn tint, the dark neutral hairline colour and the dark roadmap ring. It asserts no left bar (equal borders on all four sides, no x-only inset shadow) and that each element carries its own replacement: a hairline, a hairline plus warn tint, a tint (the unsure room message, which must differ from a plain one), or the roadmap ring. It leads with two controls that must detect a bar, asserts that the roadmap rules applied before judging the roadmap needs-you row, and checks that the explicit dark theme takes the dark ring. |
| `render-grok-subscription-3391.js` | kosmos#3391 part 2, updated for #3731: a Grok SUBSCRIPTION sign-in from Settings > AI Models and from first run, and a row signing in again. HERMETIC (file://, fetch stubbed; the status poll is ticked by hand). Settings: the choice, the key step, the sign-in (link, code, Stop), connected, errors, a missing grok (the install, the same on Windows), Stop, closing, switching, a late start, sign in again. First run, in Grok's own panel: the choice, the sign-in starts at once as a new account, choosing the key or another provider or leaving the step cancels it on the engine, connected closes the panel and names the account. |
| `render-settings-agy-3874.js` | kosmos#3874: Settings, AI Models, Add a provider offers Gemini on a Google subscription (Antigravity), as the first-run Gemini row does. HERMETIC (file://, fetch stubbed). Where it is offered, Google Gemini shows Sign in with Google / Use an API key first; the key path goes through the Gemini CLI download; the subscription walks check, Install Antigravity, Open, Check again, Ready in the dialog; Stop returns to the choice. Control: not offered, no choice, straight to the download. `SHOT_DIR=<dir>` saves screenshots. |
...
| `render-openai-devicecode-3436.js` | kosmos#3436: on Windows the ChatGPT subscription sign-in runs by device code, because codex's own browser window opened behind Kosmos. HERMETIC (loads web/index.html over file://, fetch stubbed): with the page stamped win32 and the engine answering mode device, the first-run and Settings sign-in steps show the polled link on the Open the sign-in page button and the one-time code in a copy row with a Copy button that answers; with no readable code, codex's own words are shown; the explainer reads the Windows sentence. Since 0.6.96 (Josh): in Settings ONE click on Sign in with ChatGPT starts the sign-in (start asked once, no repeat button on screen), the code line names the address (go to auth.openai.com/codex/device), the link reads Open the sign-in page again on Windows, and Stop this sign-in leaves the link pointing at #. CONTROL: a Mac page answered browser mode shows no code line and no Copy button, and keeps the Mac explainer and link words. Reds on origin/main (the poll never paints a device link or code) and on 93aaddcd5 (Settings needs two clicks). |
...
| `render-firstrun-access-onebox.js` | Screen 2 (Access), #3031 (was #2910 + #2685). FILENAME IS HISTORICAL: it began at 0.6.39 #8 asserting ONE compact preview; #2910 grew it to SIX previews in two labeled groups, and #3031 (Josh 2026-09-14) cut it back to two previews (Kosmos, tmux); 2026-09-19 (Josh, 0.6.81 QA) then removed the tmux preview, leaving ONE Kosmos preview, a single "would like to access files." prompt naming its app, with the folder-count labels and the group micro-note removed. #3336 (Josh 6.83) then removed the mock Don't Allow/Allow buttons, wrapped the caption to two lines, and shrunk the box (~1/3 width, ~1/2 height). #2685's "Terminal" -> "tmux" relabel stands (the bundled tmux is the process macOS holds responsible for agent file access, so it is the name on the real folder prompts). HERMETIC (loads web/index.html over file://, boots no server): unhides the `#fr-pane-2 .s2-dlg-fan` chain and reads static markup + computed style. Arms: exactly ONE `.s2-dlg`, no `.s2-appgrp` groups and no `.s2-applbl` labels; the preview names Kosmos NOT tmux and NOT Terminal; each preview uses the generic "would like to access files." line, no per-folder text, old "...in your folders." line gone; the folder-count labels and the `.s2-grpnote` micro-note are gone; the preview copy is compact dialog-sized (~12-14px/600), not the overlay body; the caption is the two-line `"Kosmos" would like<br>to access files.` copy (no per-folder text); the preview has NO `.s2-db` buttons and NO `.s2-mockallow` (#3336 removed them, non-vacuous: it read 2 and 1 before); and the `.s2-dlg` is shrunk to a small fixed box (~205px wide), not the old pane-filling box. The shrunk width and the compact caption weight are computed results a source read cannot see. Covers the FILE-access screen only; the functional `.s2-gate-row` grant and the S3 accessibility step are not asserted here. chromium + webkit. |
| `render-firstrun-stepcap-gear-0640.js` | Josh 0.6.40 re-test #9 + #10. HERMETIC (loads web/index.html over file://, boots no server): unhides `#fr-pane-3` / `#fr-pane-4` and reads computed style. #9: the two `.s3-step-cap` numbered captions render compact (<=12px, weight 600), NOT the 17px/400 they inherited when a bare `.s3-step-cap` lost to `#firstrun .fr-body p` (the gigantic+stretched bug), and no `.s3-standin` element remains (the "(stand-in graphic)" dev-note leak is removed). #10: `.s4-gear` box hugs the cog (box 48-58px, glyph 40-48px) after the 0.6.45 tighten, not the old 76px box nor the original 38px/22px. #768-batch (Josh, 5.44.01, said three times; Mona third round): the S4 title `.s4-nt` is BOLD (computed weight >= 700) at the SMALL body size (.6875rem, ~11px, band 10.5-11.5), and the body `.s4-nb` is normal weight (< 700) at that same size (bold, NOT larger, NOT 17px) -- BOTH kept being dragged to 1.0625rem/17px because a bare class loses the cascade to `#firstrun .fr-body p` (400/1.0625rem), so the computed weight was 400 and the size 17px whatever the rules said; the size-band and weight arms red on that pre-fix page (the old check pinned only ntSize==nbSize, which passed at 17px==17px). And the S4 cog is flex-centred (display:flex, align+justify center) WITH line-height collapsed to the glyph (== font size, not `normal`'s ~1.2x), which reds on the pre-fix `display:grid;place-items:center`. Each earlier arm reds against the pre-0.6.40 page (17px captions, standin present, 38px gear) and arm 3 also reds against the pre-0.6.45 76px box. The caption size and title weight are computed cascade results a source read cannot see. #3337 (Josh 6.83): the S4 bash notification copy is now the one-liner "bash can run in the background." with "Login Items" gone (superseding the 0.6.42 "S4 still says Login Items" scope control), and the `.s4-notif` box is halved to max-width 230px; both pinned. chromium + webkit. |
...
| `render-busy-anim-3421.js` | The "<name> is working…" busy line's three-dot `.act` animation must run as ONE continuous loop, not reset every poll (#3421, Josh 0.6.88: "plays four times then hard-resets"). Root cause: `paintBusy` rewrote `#d-busy` innerHTML unconditionally every ~5s poll, recreating the `.act` nodes and restarting their CSS loop. Drives the real `paintBusy` on a working-agent fixture and asserts DOM-node IDENTITY: repeated repaints of the same working agent PRESERVE the `.act` node (probe survives → animation uninterrupted), while a real state change rebuilds it (control: the change-guard is not a frozen node). Node-identity, not a screenshot, because the defect is temporal. |
...
| `render-provider-combobox-1040.js` | kosmos#1040 2b: the provider logo combobox (`enhanceProviderSelect` over `#d-provider`). Drives the shipped widget in the real page and asserts the WAI-ARIA combobox contract: the native `<select>` stays the source of truth (visually hidden, still in the DOM with its options), the trigger shows the selected mark+label, click/keyboard open + ArrowUp/Down navigate (aria-activedescendant, skipping disabled) + Enter selects and syncs the hidden select's `.value` and fires `change` + Esc closes and refocuses the trigger, a coming-soon option is aria-disabled and not selectable, Grok/xai shows its real Grok mark (#3708; it was an initial-letter chip), and a programmatic value change re-renders the trigger. Both themes, plus a screenshot. This is the a11y CONTRACT; a real screen-reader pass is the human follow-up. It also checks that "Sign in again" hides the whole chooser, and that a REAL mouse pick of Gemini or Grok in the Add-a-provider dialog keeps the dialog open (the 0.6.95 flash); at least one picked option must sit past the dialog's bottom edge, the geometry that bug needed, and each pick's geometry prints as a NOTE line saying which. |
| `render-provider-order-3651.js` | kosmos#3651 (Josh, 2026-09-24: "Claude/OpenAI/Gemini/Grok" in all lists). HERMETIC (file://). Asserts PROVIDER_ORDER starts Claude, OpenAI, Gemini, Grok; that #d-provider, #acct-provider-pick and #create-provider list their options in that order and their logo comboboxes show the same order; that first-run shows Claude, GPT, Gemini, Grok; and that the Settings account boxes come in provider order even when the accounts arrive reversed. CONTROL: a picker shuffled Grok-before-Gemini is put back by orderProviderOptions, so the order comes from the constant, not only from the HTML. Also #2234: the Add a provider dialog carries Josh's help button, "Help - I'm confused about selecting a provider", under the picker, to the stable https://installkosmos.com/help/connect-provider in a new tab, telling a screen reader it opens the browser. |
| `render-gemini-logo-3422.js` | kosmos#3422: the Gemini provider mark paints in the `enhanceProviderSelect` combobox. Drives the real clone path (`providerMarkNode('google', ...)`), inserts the returned node under a known `color`, and asserts the mark is a non-empty SVG `path` that inherits `currentColor` (so it actually paints the ink), carries no `url(#...)` gradient dependency, and is not the fallback initial-letter chip. The pre-fix Gemini mark was a fixed-id `radialGradient` that clone-blanked in the combobox; Claude is the positive control (a mark that always worked) and a synthetic gradient node is the negative control (the paint + url checks must reject it), so neither assertion is vacuous. Both themes. |
...
| `render-chip-filters-3423.js` | The Agents top-chip filters (#3423, Josh 0.6.88): "Issue" (`st-attn-tile`) and "No project" (`st-attn-noproj-tile`) become clickable, mutually-exclusive filter toggles like the Messages tile. Self-serve (real server + fleet). Markers key on the SERVER state to match the tile counts exactly: `data-attn` = needs the person, engine/status.js `needsPerson`: needs_you, needs_trust, or a connection Kosmos gave up reconnecting (== c.needsYou; arm 2f checks card, list row and org node against `needsPerson`), `data-noproj` adds `&& a.stateProject===null` (== c.needsYouUnattributed). Asserts: a needs_you (no-project) card carries both markers and an idle card neither; the attn-vs-noproj distinction via the real `card()` on a clone WITH a project (data-attn, NOT data-noproj); `setBoardFilter` mutual exclusivity (exactly one body.filter-* class + one tile aria-pressed at a time; null clears all); the CSS actually hides the non-matching grid card while a filter is on and the "View all agents" exit restores it; and clicking the Issue tile toggles the filter. No-project is NOT demoted (it stays the #1898 needs-you-no-project drill-down; clicking jumps to the agent). |
...
| `render-tophead-stable-2624.js` | kosmos#2624 (Josh): the top header is PIXEL-STABLE across views. HERMETIC (file://, no server). At 1440px and 1100px, in the tab view and the consolidated view, with every notice slot off and then with one 84px notice in `#login-adv-slot`, reads getBoundingClientRect: the K mark, the Kosmos switcher and the You menu (`.headright`) must sit at the consolidated no-notice tops, and the header's bottom rule must match with no notice. Controls: the center tabs render in the tab view and not in consolidated, the injected notice grows the bar, and at 800px (tab view only, outside the fix's 960px scope) the K mark does NOT take the wide value. Reds on origin/main before #2624, where the tab view's controls sat 16px lower and a notice moved them in either view. |
...
| `render-build-marker-2066.js` | kosmos#3641 (Josh, 2026-09-24): the corner version marker (#2066 / #2658) is REMOVED. On a sandboxed board, in the tab and consolidated layouts, after the status poll: the version still reads in Settings > Updates (M3, the control that the page loaded and polled); no marker element (M1); no "beta build" words anywhere on the board (M2); no version in the bottom-right corner (M4); none on an agent's page (M5); no page errors (M6). |
...
| `render-room-reply-3745.js` | #3745: replying to a room post. A sandboxed real board with a project and an agent: the Reply button sits in the post's hover bar (hidden until hovered); it shows "Replying to <who>: <first line>" above the composer with an x that cancels it; the posted reply shows a header naming the post it answers, which jumps to and highlights the original; the agent is typed which post is answered and its first words (read from a tmux wrapper in the sandbox); a reply whose original is gone says so. Controls: the strip starts hidden, and a post that is not a reply has no header. Screenshots of composing and of the posted reply. |
| `render-dm-reactions-3650.js` | kosmos#3650 (Josh: reactions "in both their conversations and projects"). HERMETIC (file://, fetch stubbed). Paints a DM thread through the real paintTalk and asserts: every agent row carries a reaction row keyed by its `at` and the person's own row does not; a stored reaction shows as a pressed pill; a quick-bar click POSTs {at, emoji} to /api/agent/<name>/thread/react and repaints from the answer; the grey smiley opens the SHARED picker for that DM row (data-at, no data-post) and a pick goes to the DM route, then closes the picker; a late answer after switching agents does not repaint the old row; a pick into a DM no longer on screen sends nothing; a quiet poll leaves a DM-opened picker open and a poll that rewrites the thread closes it. |
...
| `render-alltasks.js` | The project's View-all door opens the Tasks view scoped to that project (#1382, #3703): its rows only, finished ones in the Closed fold, the open count equal to the open rows on screen; "+ New task" files to the picked project or asks which on All tasks; an archived project's door still lists its tasks; consolidated and 390 wide. |
| `render-subview-cleanup-3502.js` | The consolidated project sub-view cleanup (#3502/#3503/#3504, Josh 2026-09-23): the "Back to ..." buttons are hidden in the consolidated Documents/Tasks/Settings views (still shown in the tab view, a control), the headings keep a top inset with the back button gone (Josh flagged Settings by name), Documents and Tasks carry a left inset matching the right one while centered Settings does not, and the "Open this folder in Finder" button (`#docs-finder`) is the gold primary button rather than a plain text link. Computed-style + rendered-geometry facts, each paired with a tab-view control; positive-controlled (assertions fail on the pre-fix markup, all pass on the fix). |
| `render-composer-reset.js` | The composer goes back to one line after a send, measured as a rendered height (#1303 C). |
| `render-composer-stroke.js` | The project composers (`#pj-post` room post box, `#pj-say` agent-say box) render with NO resting border AND still keep a visible boundary (a non-transparent recessed fill distinct from the bar behind them) in both the tab and consolidated layouts across light, dark AND plus-active (Josh, 2026-09-21: the input reads cleaner as a filled field with no stroke). The boundary assertion is the #3369 lesson: border:0 alone left the field invisible in the light theme, where the box fill equalled its white container. Also asserts the #2868 file-drag gold tint still wins over the recessed fill on drag-over. CONTROL: the agent-dialogue composer (`#d-say`) keeps its border, proving the removal is scoped to the project composers and did not reach the base `.composerbox` rule. Hermetic (file://). Reds on origin/main, where the project composers still carry the 1px `--k-rule` border. |
| `render-type-to-focus-3283.js` | kosmos#3283 (Josh): a printable keystroke typed with NOTHING focused routes into the current surface's composer, no click first, on all three surfaces -- the agent conversation view (`#d-say`), the project GRID view and the project CONSOLIDATED view (both `#pj-post`). Boots a seeded server with a fixture agent, and on each surface blurs everything, asserts the setup control (nothing focused first, or the assertion is vacuous), types real keys, and asserts the surface's composer takes focus AND the characters land in it (the mechanism is focus-on-keydown without preventDefault, which JSDOM cannot observe). Negative arm: a button already focused is NOT hijacked to the composer. Proven RED by neutering the handler (the 6 focus/land arms fail; setup + negative stay green). |
| `render-msg-counter-3403.js` | kosmos#3403 (Josh: raise the message cap, "it just cut off mid-message with no warning"): the cap is 10000 (chat.MAX_TEXT) and a SOFT near-limit counter (a `cpost-count` block sibling AFTER the composerbox) appears in the last 500 chars before it and turns amber (`.near`) within 100, so the cap is never a silent cutoff. Boots a seeded server with a fixture agent and drives the two multi-line message composers: the agent Talk box (`#d-say`, both themes) and the project room box (`#pj-post`, once). Asserts a CONTROL that each caps at 10000; the counter is HIDDEN empty and HIDDEN far from the cap (the setup control, or every "shows N" arm is vacuous); it APPEARS at 500 left reading "500 characters left" and not-yet-amber; at 100 left it turns a real computed amber (colour != the muted base -- the browser-only fact); singular "1 character left"; "0 characters left" still shown at the cap; clearing hides it; a GEOMETRY arm proves the counter is a sibling of the flex composerbox (not a squished child) on its own line below the input, full-width right-aligned (a text-only check passes on the squished bug); and a REAL-KEYS arm on d-say proves the shipped input listener drives it. Single-line quick-input composers (pj-say/d-term-say) and the task composer (tk-say, a separate engine cap) are out of this check's scope. |
...
| `render-gated-next.js` | The S2/S3 permission-gated Next, driven for real: S2 file-access AND the S3 app-accessibility row (data-gate="tmux", /api/a11y-status) disable Next on a measured not-granted reading and unlock on the grant (re-gated #2559); only sleep is advisory (#2587). (2026-09-19, Josh 0.6.81 QA: the bundled-tmux own-grant row (data-gate="tmux-a11y") was removed from onboarding, so S3 gates on the app row alone.) Uncheckable (a browser, no FDA), a path-mismatch and any fetch failure fail SAFE (never block, never false-green), so the re-gate can never re-trap (#2912); the poll (FR_GATE_POLL_MS, 750ms) re-checks so a grant unlocks on its own, and a "Check again" control (#2451/#2559) lets the user force it now. Separate arms isolate app-AX-alone-blocks, all-granted-enables and uncheckable-failsafe (red-capable both ways). Also covers #2587: a blocked sleep row never disables Next (a laptop that sleeps on battery cannot satisfy it), and the honest note replaces the useless Turn On on that battOnly row. Subsumes the retired render-a11y-gate-2125 (tmux gate) + render-sleep-button (sleep gate). |
...
| `render-token-usage-2617.js` | Settings > Token Usage is the approved /design/token-value value view (#2617/#2840), not a plain white box: a hero (the blended total = human-cost equation, plus Human Work Hours / Years of Human Work / Equivalent Token API Cost and an active-days eyebrow), four per-class daily mini-charts, a per-model table + token-share donut, the usage-history list, and the METR method footnote. Asserts the old full-number cards / combined shared-axis chart / output money box are GONE (Josh's exact-to-spec replacement), that the donut ring actually paints (a lone 100% slice is a stroked circle, not a dropped 360-degree arc), that nothing overflows the fixed ~544px settings column, and (re-mocked at the design's production scale, 150.0B / $135.0M) that the hero figures fit their boxes rather than clipping. /api/usage is mocked to a fixed response, so the numbers are deterministic on any board. The pixel/dark-theme match to Mona's mockup is the headed pass. |
...
| `render-home-discovery-removed-3048.js` | #3048 (Josh 6.63): the permanent home "Look for agents already on this computer" trigger and its two panels (found #2651, scan #1938) were REMOVED from the Agents board. Hermetic (file://), forces the Agents tab, stubs /api/found-agents and /api/scan-agents WITH candidates, and asserts the removal cannot silently regress: the six home ids (`found-scan-trigger`, `found-wrap`, `scan-wrap`, `found-list`, `scan-list`, `found-scan-look`) are ABSENT on load; running the poll's paint sequence (`paintFoundBoard`/`paintScanBoard`/`paintDiscoveryTrigger`) creates NO discovery element and throws nothing (the painters null-guard now the markup is gone); and the three painters + `DISCOVERY_OPENED` still EXIST, because firstrun's shared `.fr-adopt*` handlers call them typeof-guarded and a firstrun regression would be silent otherwise. Replaces render-found-board.js / render-scan-board.js / render-discovery-gate-2651.js, which tested the removed feature. |
...
| `render-orgchart-import-1280.js` | The org-chart import (#1280): the fifth create-agent option pastes names+titles, previews a count with title-derived machine names (names off by default, opt-in uses the person), and on Create surfaces the /api/team response -- created, refused-with-reason, and the over-cap message verbatim. /api/team is mocked, so nothing real is created. |
...
| `render-autohello-switch-2716.js` | kosmos#2716 (follow-up to #2686): the model-switch and provider-switch dialogs auto-send the wake `hello` after they restart the agent, via the shared `autoHelloAfterRestart`: the dialog shows "Restarted on <provider>. Waking them..." and resolves it to the confirmation, or to the manual "Say hello to reactivate" line when the hello does not land. HERMETIC (file://, fetch stubbed). Drives the wiring helper `autoHelloOnSwitchRestart` and asserts: a restarting->idle recovery POSTs one `hello` and updates chg-msg to "Reactivated on <provider>, and said hello"; an unconfirmed delivery and a stays-restarting timeout resolve to the manual "Say hello to reactivate" line (no false confirmation); and the guarded write is SUPPRESSED when the dialog was closed mid-wait, the person switched agents, or a different dialog was reopened (the confirmation never lands in a closed or another agent's dialog). It also covers a report that arrives BEFORE changeDialog's floor paints the line (the hold is 4400ms, the readiness accept can come at 4000ms): the confirmation still lands once the line is painted, including inside the one-second margin, nothing is written if the dialog closes first, and the wait gives up after the hold + 1s. A stale report from an earlier restart of the same agent does not land in the next dialog, and the real #d-model-go and #d-provider-go flows are driven end to end with the hold longer than two readiness polls; a switch that does not restart sends no hello. |
...
| `mobile-shots.js` | kosmos#718: phone screenshots for the mobile apps. Boots a THROWAWAY board (temp HOME and data roots, fake tmux, seeded sample agents, DM, room and ask), shoots named screens at four phone sizes (375x667, 393x852, 430x932, 412x915) in light and dark, Chromium and WebKit (an engine approximation, NOT Safari), writes `report.md` and flags horizontal overflow. A leak guard exits 3 if the page shows a real email, a key fragment or this Mac's name. `--list` prints the screens; add yours to SCREENS. The gate runs a strict slice (the frame and accounts screens at 375px) and two leak controls that must exit 3 (`MSHOTS_LEAK_CONTROL=account` and `=page`); the full sweep (16 shots per screen) is by hand. The `allow-card` screen stubs `/api/remote/pending` in the page only, because the throwaway board has Plus off and no tunnel. |
...
| `render-create-form.js` | Step two of Create an agent: no rules, half widths, Josh's order, the stepped model group #3566: also compares the Gemini/Grok provider options against the page's own account state, so they are offered exactly when a usable account of theirs is connected. |
...
| `render-pjcreate-nav-3134.js` | Project-create navigation (#3134, Josh 6.68): after "Create project" the shipped `#pj-create` handler returns to the projects LIST (list shown, `#pj-add-view` + `#pj-one-view` hidden, new project in the list), in both the tab and consolidated views, instead of diving into the new project's empty detail. Non-vacuous: origin/main shows `#pj-one-view` there. Server-based, sandboxed roots. |
| `render-create-prefs-3081.js` | #3081: Create an agent remembers the last account and model. Makes an agent with a non-default account and model (the create answer is stubbed, nothing launches), reopens Create after a reload, and asserts both come back pre-selected and can still be changed. Control: a fresh page has no remembered choice. Provider restore is not observed (Claude-only fixture). |
| `render-connlost-reconnect-3410.js` | #3410: a card whose agent lost its connection says what Kosmos is doing about it. A real board with an agent whose screen reads as a lost connection; for each reconnect phase (none, waiting, retried, gave up) it checks the label (Reconnecting… or Connection lost), the sentence under it, and the look (paused while reconnecting, the needs-you border and a red members row once it gave up), with an idle card as the control. Screenshots per phase. |
| `render-guide-hidden-3739.js` | #3739: the setup guide is hidden (Josh ruled 09:22) and titled Kosmos Guide. A real board with a guide (the seed's name and marker stubbed in-process) and an ordinary agent: the grid, the Agents tile and the connection banner leave the guide out while the page still holds it for lookups, and the guide's own page (opened by its URL) says Kosmos Guide, not the long title and never Unknown for its model. Screenshots of the grid and the guide's page. |
| `render-account-problem-3723.js` | #3723: an agent stopped by its account is told where the person looks. A real board with a Codex agent whose screen shows Codex's own out-of-credits message under its empty prompt: its card is not shown as Idle, and its Direct Message shows one Kosmos line (in Kosmos's band, no avatar or name) naming OpenAI and quoting Codex's sentence, with the link clickable only on a known vendor host. Controls: an idle agent (Idle card, no line); a non-vendor address and a user:password address stay plain text. Screenshots of the board and the DM. |
...
| `render-plus-signin-3478.js` | The Kosmos+ sign-in links open the IN-APP wizard, not the web (#3478). Stubs /api/remote unenrolled (so state 1 and the sign-in link are on screen) and stubs the signin-* POSTs per scenario, then clicks "Sign in" and walks the wizard to a named, addressed computer across all three signin-verify branches: an account with an existing second factor (email code -> phone code -> name), one that must set up a second factor (email code -> authenticator key -> confirm -> name), and one that lands straight on a session (email code -> name). Asserts the sign-in link does NOT navigate away (the whole bug), that state 1 gives way to the wizard, that the why-authenticator copy and the authenticator key render, and that the new address is shown. Backend (server.js signin-* proxies + engine stage machine) is #3149; this guards the client wiring. |
| `render-plus-stars-3778.js` | #3778 (Josh, 2026-09-25 13:07: the Kosmos+ background "circles are stretched out into really long elliptical ovals"). Hermetic (file://, fetch stubbed), deviceScaleFactor 2, at 1400x900 and 900x700: the #plus-stars buffer's aspect equals the canvas's displayed aspect once Kosmos+ settles, after the section's content grows, after a window resize, and after leaving and coming back; leaving disconnects the size watcher. RED on main (0.664 against 0.817 at 1400x900). Shots to argv[2]. |
| `render-plus-panel-3829.js` | #3829 (Josh, 2026-09-25 16:54: the connected Kosmos+ panel and device approval "terribly worded and designed"; his 17:19 shots: "the phone is showing the code" for a Windows browser, a list row named just "device"). Sandboxed board, /api/remote stubbed per state. Off: Off pill, Turn on primary, no chip. Connected: Connected pill, the address in one chip with Open, one line, Turn off quiet, View my account to the web account. One request: one compact card, the code large, device-neutral sentence, Allow / Deny, not repeated in the devices list, no left bar. Two: the older-than-an-hour card faded, an unnamed one reads "Unknown device", and no list row is just "device". Shots of all four states to argv[2]. |
...
| `render-recommender-live-3595.js` | The Recommender control is live (#3595): the toggle shows only once the read lands and reads OFF by default, the guards hide and the off note shows while off; turning it on shows all three guards ticked plus the instruction-only disclosure; unticking one guard stores exactly that change (read back from the route); a failed read hides the toggle and says so. The hint says what ships. Both themes. |
| `render-assigner-live-3595.js` | The Assigner control is live and default ON (#3595 phase 2): with nothing stored the toggle shows once the read lands and reads on, no "not active yet" note is left; clicking stores off and clicking again stores on (read back from the route); a failed read hides the toggle and says so. The hint names what ships. Both themes. |
...
| `render-profile-field-widths-2697.js` | kosmos#2697 (Josh, design channel 2026-09-10, on the agent Profile tab): the three Profile fields (Name, What they do, Reports to) ran the full container width; Josh asked for Name ~25%, What they do ~50%, Reports to ~25%, each on its own row, plus spacing above "Reports to" and the long Name helper wrapped at ~50%. Hermetic (file:// with the shared sandbox-home guard), unhides `#panel-detail` + `#d-sec-profile` + `#d-reports-wrap` and pins the panel to 1200px so the percentage widths clear the shared `min-width:220px` floor (which is left in place so a narrow panel does not crush the fields). Measures each field against its own `.frow`: Name ~25%, What they do ~50%, Reports to ~25% (with tolerance), What-they-do wider than the other two, `#d-reports-wrap` margin-top > 0, and the Name helper width at ~50% of the form. Controls, proven RED: reverting each `flex` back to `flex:1` reds that field's ratio and the 50/25/25 relationship; removing `#d-reports-wrap`'s margin-top reds the spacing assertion; removing the Name helper `max-width` reds the wrap assertion. Scoped to the detail-form ids, so no other form's fields change; a CSSOM scoping negative control asserts every 25%/50% narrowing rule targets only `#d-rename`/`#d-role`/`#d-reports` and no create-form id, so a future edit leaking the sizing to the create form reds even though that panel does not lay out while hidden. #718 also measures the 412px Profile and Instructions surfaces: text fields at least 16px, Save and Add skill at least 44px high, and the Name help using at least 80% of the Profile width. |
| `render-project-needsyou-2699.js` | kosmos#2699 (Josh, design channel 2026-09-10): on the project view a needs-you member "just says 'needs you' in text and doesn't show" - it sits in the same spot as Idle/Working and blends in. Josh asked for the little red triangle over the agent's icon and the status text in red. Hermetic (file://), calls the real `pjMember()` row builder for a `needs_you` member and an `idle` member, injects both, and reads computed DOM. Asserts: the needs-you row's `.pj-face .lwarn` triangle is present, is NOT `display:none` (the base `.lav .lwarn{display:none}` is overridden for `.pj-member .pj-face`), and its bounding box overlaps the avatar; the status `<small>` carries `pj-attn` and renders a distinct (red) color from the idle status; the row reads "Issue" (Josh, #chaoskosmos-design 2026-09-15, single-worded the state labels: "Needs you" -> "Issue"). Negative arm: an idle member gets NEITHER the triangle NOR the red. Controls proven RED: dropping the `LROW_WARN` emit reds the triangle-present arm; leaving the base `display:none` (no show/position rule) reds the display + overlap arms; dropping `.pj-attn { color }` reds the distinct-color arm. |
| `render-reach-mark-3212.js` | kosmos#3212 (Josh 6.72, 2026-09-17): "if it can't reach someone it puts a question mark icon over their avatar, then we stick with not having to print any status text." Calls the real `pjMember()` for an unreachable (`!present`) member, a present idle member, and a present needs-you member, in the room column (hideState) and the Settings list. Asserts: the unreachable member's `.pj-face` carries a shown `.lreach` question-mark badge (text `?`) and stays `.unseen`, and in the room column prints NO status `<small>`; a present member gets NO badge; a needs-you member keeps the red-! `.lwarn` triangle and does NOT get the badge (mutually exclusive by `present` gating); the Settings list shows the badge AND keeps the "cannot see this agent" reason. Natural negative control: origin/main has no `.lreach`, so the badge arms fail there and pass on the branch. |
| `render-needsyou-dealarm-2808.js` | #2808 class 2 (Josh 2026-09-14 16:49): a needs_you the agent reported DELIBERATELY (`stateReportedBy` 'agent' -- its own substantive question) renders as a CALM "Question" card, not the red "Issue" alarm a QA tester read as "app broken" (the state labels were single-worded per Josh: "Has a question" -> "Question", "Needs you" -> "Issue"). Hermetic (file://), calls the real `card()` builder for a class-2 (`by:'agent'`) agent plus two negative controls, injects them, and reads COMPUTED style. Asserts the class-2 card has NO `attn` class, HAS the `question` class + an `st-question` pill + a `.ask` (not `.haz`) glyph + reads "Question", and its computed border color DIFFERS from the alarm's. Negative arms (load-bearing): a class-1 (`by:'auto'`, the permission/trust junk, PigeonPete's invisible-handle target) STAYS red and reads "Issue", and a SCRAPED needs_you (null provenance) STAYS red -- only the KNOWN agent case is de-alarmed (selfreport.js:389). The border-color-differs arm reds if `.acard.question` fails to override the alarm border. |
| `render-waiting-phone-718.js` | #718 mobile (Josh 2026-09-24): the "waiting on you" controls on a PHONE, from the real page functions (hermetic, file://). (a) The Allow card painted by the real `paintAsk` has NO left accent bar (its left border equals its top) and, in a touch context (`hasTouch` + `isMobile`, so `(hover: none)` applies), every button is at least 44px tall. (b) `Answer` on a real needs-you `card()` has an extended `::after` hit area on a touchscreen and none with a mouse. (b) also hit-tests: a tap 5px above the Answer text lands on Answer on touch, and not with a mouse. (c) `detailRevealTalkOnPhone` scrolls `#d-sec-talk` to the top at 375px and asks for NO scroll at 1200px (counted, because a wide layout has nothing to scroll, so a position check there could not fail). (d) The position hold with REAL timers: with scroll anchoring off (standing in for an engine without it, like Safari; Chromium's anchoring compensates on its own), 300px of late content above the conversation is put back. (e) END TO END from a page load, as a push tap arrives: `?tab=detail&agent=<name>` at 375px with `/api/status` stubbed and the page's own timers real, through the poll, `settleWantAgent`, `openDetail` and the reveal: a present agent lands with its conversation in the top quarter of the screen; an agent not on the board goes to the board home (the agents tab) after the grace. (The address loses `agent=` at boot either way, so it is not asserted.) Controls, measured: the Allow left bar restored fails (a); the reveal's width gate removed fails the wide arm; the hold disabled fails (d) (settled 300); the poll's settle call removed fails both (e) arms. Chromium only: no committed check covers WebKit here (it was measured once with an ad hoc audit that is not in the repo). |
| `render-orgchart-phone-718.js` | kosmos#718 the org chart on a phone. Real server, five-agent fleet, at iPhone SE / iPhone 15 / Pro Max / mid Android (touch): the page is no wider than the screen, every agent is drawn, fully on screen and at least 44x44, a tap on a face opens that agent, and long names and roles in the leftmost and rightmost nodes' (hidden, still laid out) callouts stay inside the box and do not widen the page; the hub dragged to the edge stays inside the box; resized mid-drag and held through one 5s poll, the dragged node stays on the page and the chart repaints at the new width on release; a poll that fails mid-drag clears the chart, and it draws again once polls recover; a node dragged to the edge at 375 keeps its glow inside the box; turned to landscape while an agent was open and back to portrait on the chart, the page still does not scroll sideways; at 1400px the chart is the natural square from the page's own arithmetic, as before. Then a three-level fleet at 375px, too deep to fit even at the squeeze floor: the page still does not scroll sideways, the chart's own box does and opens centred on the hub (and stays centred when narrowed to 360px, keeps its middle point when widened back from the right edge, and stays centred when a ring is added while scrolled, while a repaint at the same width never writes its scroll), takes keyboard focus with a name (arrow keys pan it: Chromium; headless WebKit does not pan a focused box by keyboard), keeps its status note in view while scrolled, goes back to a plain, empty box when a poll fails (no scroll region, no blank square the size of the last chart), a node held at the top edge keeps its name callout inside that box, the chart allows touch panning, and (Chromium) a real touch swipe scrolls the box. A chart that fits (every phone size, and desktop) is not a scroller, so it clips nothing. Controls, measured: on the unfixed page "no sideways scroll" fails at 375, 393 and 412 in both engines (page 420px); the other flat-fleet arms pass there. With the `.orgmap.orgwide` touch rule removed, the deep-fleet touch-action and swipe arms fail (scrollLeft 0). An always-on scroller fails the does-not-clip arms; without the scroller's top padding the held node's callout is cut off (45px); without the centring the box opens at 0; keeping the old scroll offset across a width change fails the narrowed arm. Writing the scroll on every repaint fails the same-width arm (2 writes); a resize handler that remembers its own width fails the turned-while-hidden arm (page 420px). Without the box's `overflow-x: clip` the long-callout arm fails at all four sizes (page 406px at 375). Re-centring from the browser-clamped scroll fails the widened arm (101 vs 108). Without the note's sticky rule the note arm fails (the note slides 54px off to the left). With the old 30px margin the dragged-to-the-edge arm fails (face 9px from the box, glow reach 20). Without .orgtight the edge-callout arm fails at all four sizes (27 to 54px outside); without the hub's own margin the hub arm fails (9px outside). Without the guard in paintOrg the dragged node is detached (by the poll, even with the resize path guarded); without the release catching up the chart stays at the old width. A hold keyed on the drag flag alone leaves the chart blank after a failed poll mid-drag (0 of 5 drawn). Leaving the scroll alone when a ring is added fails the ring arm (62 vs centre 114); without the focus/name attributes the keyboard arm fails on them (Chromium focuses a scroller by itself, so the arrow keys alone would not discriminate). Without the reset on the failed-poll path, the failed-poll arm fails in both engines (the failure is made in the page's own fetch: the board's service worker carries requests past page.route in WebKit). Gate runs Chromium; `ENGINES=chromium,webkit` by hand (WebKit is an engine approximation, not Safari). |
| `render-mention-blue-2922.js` | kosmos#2922 (Josh 6.59): a project-room `@name` that names an agent ON that project is highlighted bright blue (`.pjmention`) so the reader sees it will flag that agent; a name matching nobody stays plain. Hermetic (file://), calls the page's own `pjRichSpans` with a room `agentNames` Set. Arms (chromium + webkit, light + dark): a valid `@agent` -> a `.pjmention` span and `@nobody` stays plain; `**@mona**` highlights INSIDE the `<strong>` and `(@mona)` keeps its parens (the token-splitter only peels the tail, so the leading marker re-attach is a computed result a source read cannot see); the dialogue path (no `agentNames`) never highlights, proving the room scoping; and the computed `.pjmention` color clears WCAG AA (>=4.5:1) against the real message-bubble grounds (the `--k-sunk` and `--usermsg-tint` washes, blended) in both themes -- a computed color on a semi-transparent ground a source read cannot judge, and the arm that reds on the first cut's bare-page contrast miss. |
| `render-consolidated-layouts.js` | The consolidated view under each Agents layout: no org chart over the rails, and an empty centre that says what to press (#774). Also the #3218 tightening: the four column fractions (16.6/16.6/50/16.6), the Members card hidden with its agents grouped to the top of the Agents list, Tasks/Files 50/50, and View All pinned in each section header. |
...
| `render-consolidated-newagent-3053.js` | Consolidated view: clicking New Agent (`#rail-agents-new`) stays in the consolidated view and opens the create panel in the display column (`#panel-create` relocated into `#panel-projects` via `placeCreatePanel`/`openConsolidatedCreate`) instead of kicking back to the tab view (#3053), keeping the projects list beside it; navigating to a project hides create, leaving the consolidated view restores it to the top level, the create form sits at the top-left of the column at its 34rem form width (not a narrow island), and `#pj-none` is suppressed while it is open. CONTROL: the tab view keeps its full-page create. Both themes. |
| `render-consolidated-projects-3052.js` | Consolidated view: the projects panel has its OWN view (the readable list) instead of inheriting the Projects TAB's saved grid/roadmap layout (#3052, Josh 6.63 testing). Drives the shipped `placeProjectsView` + `layoutApply`: from both a GRID (`#pj-list.asgrid`) and a ROADMAP (`body.pj-roadmap`) tab layout, entering consolidated forces the list (clears both classes), and leaving restores the saved layout. Asserts the force is DISPLAY-ONLY (does not overwrite `kosmos.layout.projects`). CONTROL: the tab layout is confirmed set before each entry. Both themes. |
| `render-project-members-3387.js` | kosmos#3387 (Josh feedback 2026-09-21): in the CONSOLIDATED view with a project open, the agents column head reads "Project Members" (was "Agents") and its top `#rail-agents-new` + adds an EXISTING agent to that project (opens the shared `#am-modal`, picker `#pj-one-add` scoped to the free agents); a new `.alist-grouphdr` "Other Agents" sub-header (in place of the old plain rule) carries its own `.alist-newagent` + that creates a new agent. HERMETIC (drives `paintAgentList`/`setAgentsGrouped`/`openAddMemberModal` over file://). Asserts the rename, both +s doing distinct actions, and a CONTROL with no project open (head back to "Agents", no sub-header, top + opens create not the modal). Both themes. |
| `render-user-menu-3051.js` | kosmos#3051 (Josh 0.6.63, for 6.65): the upper-right nav is a user avatar + name button (`#userpop-btn`) that opens a dropdown (`#userpop-menu`). HERMETIC (loads web/index.html over file://, boots no server). Asserts the top nav is exactly Agents + Projects + Tasks (Tasks since #3559; no Settings tab), the button renders (avatar slot + name), the menu is closed by default (aria-expanded=false), opening it reveals the four items (the Settings link `#userpop-settings`, the light/dark `.themepick`, the board-view `.laypick`, the agent-status `#checked`), the Settings link opens `#panel-settings` with no tab lit (`settings` is in `BUTTONLESS`) and closes the menu, and Escape + an outside click each close it. Both themes. Red before #3051 landed, when the top nav still had a Settings tab, no `#userpop`, and the light/dark + view controls sit bare on the header row. |
| `render-firstrun-reentry-3359.js` | kosmos#3359 (0.6.84 new-user QA): Escape in the first-run wizard marks setup seen (and it never returns on its own, with no visible warning), so the loss is made recoverable by a "Guided setup" box in Settings > This computer whose button (`#set-rerun-setup`) reopens the wizard via `firstRunBoot(true, 1)`. HERMETIC (loads web/index.html over file://, stubs only `/api/first-run` to a returning user, no server). CONTROL: with the seen flag set the wizard does not auto-open at boot, so the reopen must take the click. Asserts the button exists in the `#s-sec-mac` section and is reachable, the screen-reader warning names the Settings re-entry (not "gone for good"), clicking it reopens the wizard on the welcome screen, and a Continue left disabled by a prior Escape completion is re-enabled on reopen (labelled "Get Started"). Reds on origin/main, where the button does not exist and the warning still claims permanence. |
...
| `render-login-expiry-3532.js` | The login-expiry advisory pill: warns per account before a Claude login expires, in warn/urgent/expired states, with an empty control (#3532). |
| `render-update-win32-manual.js` | win32-update-arm (S4) + win32-update-check (S1): Settings > Updates on a Windows bundle. The update answers are stubbed at the network edge (`update`/`updateManual`/`updateChannel` on the poll, `offer`/`manual`/`channel` on the check). ARMED: a normal bundle sends `update` and the card shows "Version X is ready." with an [Update] button, no Download link and no manual steps. MANUAL (a OneDrive/Program Files bundle): the manual-offer sentence and a visible Download link to the exact versioned zip (described by that sentence), the check button hidden, no channel tag. On staging, "Staging channel" shows and the link is the staged versioned zip. A could-not-read look names itself, and a press that cannot reach says so, never "Up to date.". The CONTROL: nothing newer gives "Up to date." with no link. ROLLBACK (win32-update-rollback, S5, #3017): when `updateRollback` names a kept previous build, the "Roll back to X" button shows (orthogonal to the offer arm, so it appears beside an up-to-date line here), and the CONTROL current state confirms it is hidden with nothing kept. The confirm-body and overlay Windows wording (including the rollback confirm body, the "Rolling back to X" overlay lead, and the "roll back did not take" failure line) are win32-served-only (they cannot render on macOS CI); `web.update-settle-win32.test.js` covers them. |
...
| `render-agentdm-3414.js` | #3414 (Josh): the single-agent DM reuses the project conversation UI. Hermetic (file://+fetch-stub+paintTalk), light+dark: agent rows are the room's `.msg` bubble (avatar, name bold inside, timestamp inside `.msg-bd`), user rows are `.msg.you` (mine avatar, no name), no old `.dm` markup; header "Direct Message to <agent>" in the big project-name font; search "Search"; the "Just between you and <agent>" hint element is gone; the box is the black `--k-bg` edge-to-edge treatment (no card radius). |
| `render-dm-emoji-3744.js` | #3744 (Josh, 2026-09-25: "We need emoji support in the agent DM."). Hermetic (file://, fetch stub, paintTalk), Mac and Windows: the DM composer has the room's grey smiley in its own box (same name, controls `#d-emoji`, muted glyph); a real mouse press opens the panel above the box and inside the window, holding the room's own `PJ_EMOJI` list; a pick inserts at the caret and keeps focus in the input; the keyboard opens, picks and closes it; Post sends the emoji and the row renders it; a box the agent cannot be reached through greys the smiley and closes the panel; the room's picker still inserts into the room's input and only one panel is open at a time. Fails 6 arms on main (no `#d-emoji-btn`). Screenshots to argv[2]. |
| `render-unread-edge-3743.js` | #3743 (Josh 2026-09-25 09:15): an unread agent message has a thin gold edge that fades once read. Hermetic (file://, fetch answered in the check, no polls). DM through the real paintTalk: opened with two unread, the newest two agent messages have the edge and history does not, in the brand gold #d6a62e inside the bubble (U1); on screen a moment with the window in front the edge goes, with a 1.2s fade (U2); a repaint does not bring back what was read, and a message that lands while open arrives with the edge (U3); with the window behind another a new message keeps its edge, and coming back starts the clock (U4, the control for U2); reduced motion drops the fade (U5). The project room through the real pjMarkSeen and paintRoom: opened with three unread, the newest three agent posts have the edge (U6). A message taller than the window is read a screenful at a time (U8); a room read that did not answer is not a first look (U9); a stale count does not re-edge what was read (U10); a thread's sets hold only what it shows (U11); the moment on screen is continuous, so leaving the window inside it starts it again (U12); a message over twenty thread-heights tall, read down a little at a time in its own scroll box, still loses its edge (U13); in dark and on Kosmos+ navy the edge is the gold at half strength (U14). The setup assistant's chat: a reply that came while folded has the edge and history does not, it goes once read, a reply landing while open gets it, and the hosted assistant's words never do (U15). No page errors (U7). chromium. |
| `render-dm-phone-718.js` | kosmos#718 (Josh: "make sure the designs fit well on mobile"). HERMETIC (file://, fetch stubbed). The DM thread at iPhone SE / iPhone 15 / mid Android / Pro Max widths, light+dark, plus 900 and 1280: no bubble outside its row (a message with files sat 112px off the left of an SE because the person's content-sized bubble took the one-line file name's width; the fix is a cap on that bubble, and a 120-character name covers 900 and 1280 too), every attachment card inside its bubble with its picture-preview slot sitting inside the card (the images themselves cannot load over file://), no table cell narrower than its longest word and the table's scroll box inside its bubble, neither the page nor the thread scrolls sideways, the gutter opposite each avatar equals avatar plus gap (#3340), the bubble tail's ground mask never reaches the avatar, every file name stays on one line, and at 1280 the person's short-named card bubble still fits its content. The gate runs Chromium only; `ENGINES=chromium,webkit` by hand adds WebKit, an engine approximation, not Safari. |
| `render-dm-chatfirst-718.js` | kosmos#718 chat first on a phone (some arms are regression guards that also pass on main) (Liu Kang decided the build, 2026-09-24). HERMETIC (file://, fetch stubbed). The agent page with Direct Message open at iPhone SE / iPhone 15 / mid Android / Pro Max, light+dark: the page is exactly the screen and the thread scrolls inside it, the composer and Post are on screen, the composer stays above a SIMULATED keyboard (the visible-height variable the page's own visualViewport listener writes) while the header steps aside, Post, add-a-file and the Profile tab are at least 44px, the visualViewport listener's keyboard detection (stubbed: drop, pinch, layout-shrinking keyboards, rotation with and without the keyboard up), a touch tap on Post, the emoji button and an emoji pick at 520px keeping focus in the text box, a long agent name, the CSS/script breakpoint pin, the emoji panel above a stubbed keyboard at 600x700, a swarm agent (cluster fits the slot, Stop now on screen with notes), Profile is one tap away, no sideways scroll. At 800 and 1280 the stacked tiles and the "Direct Message to" caption are unchanged. On any touchscreen past the phone width (667x375, 852x393, 932x430, 1024x768) the search box and the text box are 16px and the search row 44px; a mouse at 1280 keeps 13px and 15px. Gate runs Chromium; `ENGINES=chromium,webkit` by hand (WebKit is an engine approximation, not Safari). |
| `render-signin-visible-3892.js` | kosmos#3892 (Liu Kang: Sign in again must never hide). HERMETIC (file://, fetch stubbed). The agent's phone chat header with every note showing (usage-limit quote, stale instructions, Sign in again, Start this agent) at 375x667, 393x852, 412x915, 430x932, light and dark: Sign in again is inside the capped header, on top and 44px tall with the block scrolled to its top and to its bottom; Tab reaches Sign in again with its whole focus ring inside the header; larger phones keep the conversation height they have with Sign in again hidden (measured in the same run) and an SE gives up at most 32px; a swarm agent's Stop now is uncovered at the middle of every edge, including a swarm agent whose sign-in stopped as the page itself paints it (the case where two actions really share the header: a 190px floor, so an SE gives up at most 70px and larger phones at most 37px, with the composer and Post still on screen, checked as the page paints it), also with a long name and a wrapping task sentence (the name row stays one line); an agent whose sign-in stopped, painted by the page, gets Sign in again on screen; Sign in again alone leaves the cap unchanged; Start this agent with Sign in again is a defensive arm (they never show together today); at 800 and 1280 it is not pinned. `KOSMOS_PAGE=<index.html>` runs it against another build. WebKit is an engine approximation, not Safari. |
| `render-dm-tapreact-718.js` | kosmos#718 (Josh: "really flesh out all the native mobile apps"). HERMETIC (file://, fetch stubbed, touch emulated). TAP TO REACT in the agent DM on a touchscreen at the four phone sizes: a tap on the agent's message opens its reaction bar, every emoji takes its own tap, the bar stays inside the thread and above the composer, the first message's bar opens below it and still takes every tap, targets are 36px, the tap's sticky hover does not keep a closed bar showing; a second tap, a tap on your own message, between messages or on the text box closes it; a repaint keeps it, opening another agent drops it; picking an emoji reacts to that message (the react route gets its `at`) and lets go of focus. A mouse at 1280: a click opens nothing, hover still shows the bar. `KOSMOS_PAGE=<index.html>` runs it against another build. WebKit is an engine approximation, not Safari. |
| `render-agent-msg-gray-2805.js` | kosmos#2805 (Josh): an AGENT message in the DM view wears an ultra-light cream (`--agent-msg`) bubble; the person's own message keeps the royal-blue wash. #3260: one fixed color, no per-message variation. Hermetic (file://), renders an agent row beside a person row and asserts, in light and dark, that the agent bubble is filled (not the transparent #2660 state), is distinct from the person's blue, reads against the DM panel (did not dissolve into the surface), is a warm cream (not a neutral gray or the blue), and that a `data-am` attribute no longer changes its color. Asserts the RELATIONSHIP, not a literal rgba, so a retune of the cream stays green. |
| `render-dm-multiline-3208.js` | kosmos#3208 (Josh 6.72): the direct-agent "Talk to X" composer `#d-say` accepts multi-line messages. Root cause was that `#d-say` was an `<input type="text">`, which strips newlines, so a user's paragraph breaks compressed to one line; the render path (`.dm-b` pre-wrap + `pjRich`) already supported them. Boots a seeded server, opens an agent's Talk view, and asserts `#d-say` is a `<textarea>` that keeps a message's paragraph breaks, that a bare Enter is intercepted (sends) while Shift+Enter is not (newline), that the box autosizes and resets to one line, and that a two-paragraph message renders as multiple lines in the bubble. Negative control: 4 assertions fail on the pre-fix `<input>`. |
| `render-room-msgbox-2806.js` | kosmos#2806 (Josh, asks 1+2): in the project ROOM the person's OWN message body wears a light royal-blue box (`--usermsg-tint`) and an AGENT's wears an ultra-light cream box (`--agent-msg`), via a `.msg-bd` wrapper around the message body (the room's `.msg-b` class had neither color; #2805 only reached the DM's `.dm-b`). #3260: one fixed color, no per-message variation. Hermetic (file://), renders real `pjRoomRow` output for an operator row, an agent row, and a bodyless row, and asserts in light and dark that the operator box is filled and blue, the agent box is filled and a warm cream, the two are distinct, the agent box carries no `data-am`, and a bodyless row draws a `.msg` row but NO `.msg-bd`. Asserts the RELATIONSHIP, not a literal rgba. |
...
| `render-model-restart-interstitial.js` | kosmos#768-batch (Josh 0.6.47, 5.51.59): switching an agent's model shows a RESTART INTERSTITIAL -- the breathing Kosmos K over "Restarting the agent" -- held while the agent restarts, then the confirm dialog reduces to one line naming the provider ("Restarted on <provider>. Waking them..." since #2716). HERMETIC (file://, no server). Drives the shared changeDialog directly: `busyHtml`+`minBusyMs` paint the interstitial (button + small text hidden -> clean interstitial, no modal exit), HOLD it on success then render the sentence, render a FAILURE at once (success-only hold, so the modal never traps #1313), and leave a no-`busyHtml` caller byte-unchanged on plain "Working…" with the small text still shown (CONTROL). Then drives the real #d-model-go flow (stubbed POST, CURRENT set, hold shortened via `window.__kosmosRestartHoldMs`): the dialog shows the K "Restarting the agent" then reduces to "Restarted on <provider>. Waking them..." (#2716: the app sends the wake hello and render-autohello-switch-2716 covers how the line resolves). A source control pins the prod hold as RESTART_HOLD_MS = Math.max(2000, K_LOADER_CYCLE_MS) (#2692, one K-loader loop, 4400ms). chromium. |
...
| `render-handoff-restart-3492.js` | kosmos#3492 (Josh 2026-09-23): the restart confirm dialog's THIRD option, "Write a handoff, then restart" -- the app asks the live agent to write a handoff, WAITS for it, then restarts and points the fresh session at that handoff. HERMETIC (file://, every route fetch-stubbed). Drives the real `rst-handoff-go` handler and asserts the four-phase flow: it shows the "writing its handoff" status while polling `handoff-restart/status`, mounts the restart interstitial only once the handoff is fresh, and lands the "pointed the fresh session at its handoff" receipt after one ask / one restart / one pickup. The two safety arms: an UNWRITTEN handoff (poll never fresh) and a REFUSED ask BOTH restore the confirm view and NEVER POST /restart -- context is never lost to a mistimed restart. The loader's PAINT is owned by render-restart-kloader-2831; this asserts flow + DOM states. Source controls pin that the handler wires ask->status->restart->pickup and gates on placed+fresh. Timing seams (`HANDOFF_WAIT_MS`, `RESTART_READY_WINDOW_MS`, `window.__kosmosRestartHoldMs`) shortened. chromium. |
...
| `render-pjmode-style-3495.js` | kosmos#3495 restyle (Josh 2026-09-24 17:50 on 0.6.92 staging: the Create/Join toggle "looks like a wireframe... should just match our styling"). HERMETIC (file://). Drives the real `openAddProject` and `pjSetAddMode` in light and dark and asserts the app's segmented-control look: the chosen segment is the `--gold-bright` fill with `#14161a` ink and the other unfilled (before and after switching to Join), the two halves are equal, the bar spans the Name field's width on its own row below All projects with a hairline border, and no create field carries a top rule. Against main's pre-restyle page 16 of its 20 assertions fail. chromium. |
...
| `render-reassign-update-3050.js` | kosmos#3050 (Josh 6.63 testing): the Instructions-tab staleness note names the action ("This agent needs to be updated") and offers an Update button that reloads the current on-disk version in place, with the Kosmos K mark for the brief reload, replacing the dead-end "Reopen this agent to see the current version" text that had no control. Drives the real page and the real loadInstructions handler headless, mocking only the per-agent instructions endpoint (v1 -> v2) so the reload is shown fetching a changed file; red-capable on the box-reload arm (a CONTROL pins the stale v1 before the click). chromium. |
| `render-github-door.js` | The GitHub door on the Connections tab (#529), driven in a real browser: absent gh is the |
| `render-accounts-openai.js` | An OpenAI account added from the Accounts page with a pasted key, listed by provider and offered on the create form (#540); driven against a stand-in codex so no real key is involved. #3566: also drives the Gemini API-key step (reveal on pick, password field, empty-key refusal, gold success box, field cleared, and a late answer not shown under a provider picked since), with the engine route stubbed in the browser so no key leaves the machine. #3874: where Gemini on a Google subscription is offered (a Mac), picking Gemini first shows Sign in with Google / Use an API key with no key box; the check reads the board's own /api/antigravity to know whether to expect it, requires it (both buttons, no key box) at both Gemini picks, presses Use an API key, and prints a NOTE once when the board does not offer it. Its Gemini/Grok API-key section pins the board's `/api/runners` answer to "present" for gemini and grok (a failing board answer is passed through, or aborted if the board cannot be reached, and recorded as a FAIL; the tool-missing download path is covered by render-settings-agy-3874 and render-keyed-install-3713), so the key step shows on a Mac with or without those tools; `KOSMOS_BC_RUNNERS_ABSENT=1` is the control arm (tool missing: that section fails), and `KOSMOS_BC_RUNNERS_BOARD_ERROR=1` sends the runner read to an address the board does not serve, so the board-error branch must name its FAIL. |
...
PW=$(mktemp -d)
cd "$PW" && npm init -y && npm i playwright && npx playwright install chromium webkit   # webkit is REQUIRED by render-fields.js and render-talk-fill-2622.js
...
from every field check because they were measured not to paint their fill; a skipped slider not
in KNOWN_NATIVE_SLIDERS fails, so adding a native slider on purpose means adding its id there), that a
...
- render-push-718.js -- #718: the PWA push client end-to-end -- `/sw.js` serves as a worker, the client registers it and it controls the page, and a REAL push delivered through the worker's own `push` handler (via the Chromium DevTools `ServiceWorker.deliverPushMessage` verb) shows the notification MAPPED from the coordinator's real `{kind, agent, project, id, address}` payload. The board here runs on 127.0.0.1, which has no relay domain, so the tap URL is the board itself (`/`); the Mac click-through, and the #3689 rule that it must be a host one label under the board's own domain, are pinned in `web.sw-718.test.js`. The notification-RENDER assertion is HEADED-ONLY: headless Chromium keeps notification permission at `denied` even after `grantPermissions`, so `showNotification` rejects and `getNotifications()` returns `[]`; under `HEADED=0` (which includes the cut gate) that one read is skipped with a printed SKIP line. The push HANDLER is still asserted headless (#3565): the check wraps `showNotification` inside the live worker (test-side only, nothing added to the shipped service worker) and asserts the real handler called it with the mapped notification, and that the call settled as the mode allows (headed: resolved; headless: only the permission denial). It does not see whether the call was handed to `event.waitUntil`. The payload mapping is also covered by the node suite `web.sw-718.test.js`. Chromium-only by design (Web Push + the CDP verb are Chromium's). Needs a first-run-complete board.
