...
(async () => {
  ...
  // #3105: Henderson carries agents so its rail row exercises the meta #3105 hides: .pjfaces,
  // .pjcount, .pjpill. (#3133 removed the .pjsub "X sub-projects" count from the projects-list
  // row entirely, so this check no longer needs a sub-project fixture to exercise a .pjsub hide
  // -- that arm is dropped below.)
  ...
  try {
    ...
    const m = await page.evaluate((id) => {
      ...
      return {
        ...
        present: { pill: !!row.querySelector('.pjpill'), count: !!row.querySelector('.pjcount') },
        parts,
      };
    }, a.id);
    ...
    if (m.missing) { chk(false, 'a project row is on the page'); }
      ...
      /* #3105 (Josh, 6.68): the rail row is TITLE ONLY. The title is painted; the status
         pill and the agent count are NOT laid out (hidden in the rail), so rect() returns
         null for them. These absences are the load-bearing arms and they return the
         dangerous answer on origin/main, where both are shown. Henderson has agents
         (.pjfaces/.pjpill/.pjcount render), so #3105's hidden meta are exercised and proven
         hidden by this one row. (#3133 removed the .pjsub sub-project count entirely, so its
         hide arm retired with it.) */
      ...
      /* NON-VACUITY CONTROL: the fixture must actually render the meta before we can prove
         it is hidden. Henderson has agents (pill+count), so both nodes exist in the row
         markup. On origin/main they would also be VISIBLE, so the two hides below return the
         dangerous answer there. */
      chk(m.present && m.present.pill && m.present.count,
        'the rail row actually renders the pill and agent count (so the hides below are non-vacuous)',
