 ...
 * codes, and guards the class as far as a text check can (see the note above the last
 * test for what it does not cover).
 */
...
/* The spelling rules above can never be complete: optional chaining, a reversed ternary,
   ...
   matching none of them (blind review, pass 4, measured). So this rule REQUIRES the safe
   form instead: each execFile, spawn, spawnSync or exec call in a cli.* test must be
   followed, before the next call site, by a check that the code (or status) is a number
   leading straight into a reject or a throw. Comments are blanked first, so a
   commented-out check does not count. execFileSync and execSync throw on any failure and
   a promisified execFile rejects, so they are not call sites; a site that does not read
   the exit code at all says so with the marker "exit code not read (#3628)" and why.

   What this is NOT: a proof. A text check cannot see order or reachability, so a harness
   written to defeat it (resolve before the check, `if (false && ...)`, a swallowed throw)
   still passes (blind review, pass 5). It catches every spelling that existed here and the
   ordinary rewrites; the harnesses themselves are what make a kill fail. */
function blankComments(src) {
  return src.replace(/\/\*[\s\S]*?\*\/|(^|[^:'"\\])\/\/[^\n]*/g, (m, lead) => (lead || '') + ' '.repeat(m.length - (lead || '').length));
}
test('#3628: every exec call in a cli.* test checks the exit code is a number before using it', () => {
  const CALL = /(?<![.\w])(?:execFile|spawn|spawnSync|exec)\s*\(/g;
  const CHECK = /\.(?:code|status) !== 'number'\)\s*\{?\s*(?:reject|throw)\b/;
  const check = (src) => {
    const code = blankComments(src);
    const sites = [...code.matchAll(CALL)].filter((m) => !/promisify\(\s*$/.test(code.slice(Math.max(0, m.index - 20), m.index)));
    return sites.filter((m, i) => {
      const end = i + 1 < sites.length ? sites[i + 1].index : code.length;
      const win = code.slice(m.index, Math.min(end, m.index + 1000));
      const markerWin = src.slice(Math.max(0, m.index - 300), m.index);   // the marker is a comment, read from the raw text
      return !CHECK.test(win) && !markerWin.includes('exit code not read (#3628)');
    });
  };
  const Z = '0';
  ...
  // Holes found in review pass 5, now closed: a commented-out check, and a second call
  // inside a checked call's text window; spawn and spawnSync are call sites.
  assert.equal(check("execFile(CLI, a, {}, (err) => {\n  // if (err && typeof err.code !== 'number') { reject(err); return; }\n  resolve({ code: err ? err.code : " + Z + ' }); });').length, 1);
  assert.equal(check("execFile(CLI, a, {}, (err) => { if (err && typeof err.code !== 'number') { reject(err); return; } resolve({}); }); execFile(CLI, b, {}, (err) => resolve({ code: err?.code ?? " + Z + ' }));').length, 1);
  assert.equal(check('const r = spawnSync(CLI, a, { timeout: 5 }); assert.notEqual(r.status, ' + Z + ');').length, 1);
  assert.equal(check("const r = spawnSync(CLI, a, { timeout: 5 }); if (typeof r.status !== 'number') throw new Error('no exit code'); assert.notEqual(r.status, " + Z + ');').length, 0);
  assert.equal(check('const re = /x/; re.exec(s);').length, 0);
  const self = path.join(__dirname, 'cli.exit-code-mapping-3628.test.js');
  ...
  assert.deepEqual(bad, [], 'these exec calls never check that the exit code is a number');
  // 19 call sites in 17 files, measured when this was written. The floor equals the count,
  // so removing a site on purpose means lowering it here too.
