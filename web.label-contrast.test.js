 ...
 *
 * ⚠️ #firstrun IS THE HARD CASE HERE, NOT AN EXTRA. Every other themed rule
 * in this file assumes dark mode means a dark ground, but #firstrun pins
 * itself to white in every theme (its own single-look ruling). The plain
 * dark-mode red (#ff8c82) measured against #firstrun's ACTUAL white ground
 * comes out to ~2.03:1 -- caught in challenge-loop iteration 1, failing well
 * under the 3:1 floor in exactly the screen this fix was written for. The
 * fix is a #firstrun-scoped pin back to the light value, generated by
 * tools/sync-forced-theme.js like every other dark rule in this file, not a
 * fourth hand-written color.
 */
test('the attention glyph clears 3:1 in BOTH themes, and stays legible inside #firstrun\'s pinned-white subtree', () => {
  ...
  /* THE HARD CASE: #firstrun's real ground is white in every theme. The dark
     rule's plain value must NOT be what actually reaches this subtree. */
  const firstrunGroundInDarkMode = over(tint.light, hex('#ffffff'));
  const rWithoutPin = ratio(tick.dark, firstrunGroundInDarkMode);
  assert.ok(rWithoutPin < 3,
    'this assertion is the regression sensor: if the unpinned dark red now clears 3:1 on white, the '
    + 'measured motivation for the #firstrun pin below is stale and this test needs re-deriving, not deleting');
  const rWithPin = ratio(tick.light, firstrunGroundInDarkMode);
  assert.ok(rWithPin >= 3,
    'the #firstrun pin value itself does not clear 3:1 on the subtree\'s real (white) ground');
  ...
  /* The #firstrun pin, both generated forms, each restoring the LIGHT value. */
  for (const sel of ['\\:root\\:not\\(\\[data-theme="light"\\]\\) #firstrun \\.chk\\.att \\.chk-m',
    '\\:root\\[data-theme="dark"\\] #firstrun \\.chk\\.att \\.chk-m']) {
    const re = new RegExp(sel.replace(/\\\\/g, '\\') + '[^}]*#b3261e');
    assert.match(PAGE, re, 'the #firstrun pin restoring the light attention red is missing: ' + sel);
  }
});
