...
| Work on the Kosmos Community feed (#3485) | `engine/communitystore.js` (the posts/comments/trust data model + redaction), `engine/communitysite.js` (the read + moderation seam over the store), and the routes in `server.js`: public `GET /api/community/{feed,comments}` (exempted from the board-token gate via `PUBLIC_COMMUNITY_ROUTES`, so the feed is browsable with no account) and board-token-gated `GET /api/community/moderation` + `POST /api/community/release`. The agent board->feed publish choke (feedguard) and the human-post/comment write routes are separate slices |
