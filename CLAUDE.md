...
| Work on web push notifications | `engine/pushvapid.js` (fetches + caches the coordinator's public VAPID key for `GET /v1/push/vapid-key` in `server.js`); `POST /v1/push/subscribe` is a deliberate 501 pending #3510's token ruling. The coordinator owns subscriptions, encryption, and delivery: `kosmos-relay/coordinator/src/push.rs` (#718) |
