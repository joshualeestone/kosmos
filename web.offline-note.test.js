...
test('a server that ANSWERED with a refusal is not "not answering": the note paints only when nothing answered (#268)', async () => {
  ...
  const drive = async (fetchImpl) => {
    ...
    await new Function('paintOfflineNote', 'fetch', 'document', 'INSTR_EPOCH', 'boardEmpty', 'paintAddAgents', 'ORG_HTML', 'BOARD_LOOK_FAILED', 'BOARD_NEEDS_SIGNIN', 'setNavBadge', 'ringNewAgentMessages', 'setAgentsGrouped',
      `${page.lift(SCRIPT, 'tick')}\nreturn tick();`)(
      (down) => painted.push(down),
      fetchImpl,
      { getElementById: stub, querySelector: () => null, querySelectorAll: () => [] },
      0, () => '', () => {}, null, null, false, () => {}, () => {}, () => {}, // #3301: ringNewAgentMessages / #3387: setAgentsGrouped no-ops (tick calls them; not under test here)
    );
    return painted;
  };
...
test('#2023: BOARD_NEEDS_SIGNIN does not latch -- a non-403 outcome after a 403 clears it', async () => {
  ...
  const stub = () => ({ dataset: {}, innerHTML: '', className: '', textContent: '', hidden: true, closest: () => null, querySelector: () => null, querySelectorAll: () => [] });
  let fetchImpl;
  const tick = new Function('paintOfflineNote', 'fetch', 'document', 'INSTR_EPOCH', 'boardEmpty', 'paintAddAgents', 'ORG_HTML', 'BOARD_LOOK_FAILED', 'SIGNIN_SENTENCE', 'setNavBadge', 'ringNewAgentMessages', 'setAgentsGrouped',
    `${page.lift(SCRIPT, 'tick')}\nreturn tick;`)(
    () => {}, (...a) => fetchImpl(...a), { getElementById: stub, querySelector: () => null, querySelectorAll: () => [] }, 0, () => '', () => {}, null, null, page.liftConst(SCRIPT, 'SIGNIN_SENTENCE'), () => {}, () => {}, () => {}, // #3301: ringNewAgentMessages / #3387: setAgentsGrouped no-ops (tick calls them; not under test here)
  );
