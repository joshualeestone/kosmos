...
| 4 | `4-codex-AGENTS.md` -> `AGENTS.md` | a folder Claude has run in | a Codex/AGENTS.md-only agent (alternate format) | **name NOT picked up (finding)** | found() sees no CLAUDE.md -> offers the folder as an EMPTY-NAME adoptable; the Codex path returned nothing in the sandbox. So the name `Fixture Codex` is not read on this path. |
...
- **#1**: a scattered Kosmos-created file is re-offered when its agent is not currently
  ...
  avoid re-adoption.
- **#4**: an AGENTS.md-only (Codex) agent's NAME is not read by this path.
...
## Constraint
Fixtures #1 and #5 are FRESH files in the Lil-Nacho / hand-written shapes. They are
NOT the preserved real Lil Nacho files on Casey's machine (the live #1938 evidence);
