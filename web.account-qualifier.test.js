...
test('a name only one row carries is left exactly as it was', () => {
  ...
  assert.equal(q.get(DEFAULT_ROW), '', 'a unique name picked up a qualifier, which is noise on the common case');
  assert.equal(q.get(OTHER), '', 'a unique name picked up a qualifier');
});

test('when two rows share a login, BOTH are named, and differently', () => {
  const q = qualifiers([DEFAULT_ROW, SECOND_ROW, OTHER]);
  const a = q.get(DEFAULT_ROW);
  const b = q.get(SECOND_ROW);
  ...
  assert.equal(q.get(OTHER), '', 'an unrelated unique row was qualified because two OTHER rows collided');
});
...
test('a duplicated row with no label still comes out named', () => {
  ...
  assert.notEqual(q.get(bare), '', 'a duplicated row with no label came out unnamed, so its control shares a name again');
  assert.notEqual(q.get(bare), q.get(DEFAULT_ROW), 'the fallback produced the same name as its twin');
});
...
test('the key-tail rows collide on the name the SCREEN shows, not on email', () => {
  ...
  assert.notEqual(q.get(k1), q.get(k2), 'two rows showing one key tail are still indistinguishable');
  assert.notEqual(q.get(k1), '', 'a duplicated key-tail row was left unqualified');
});
...
/* The screen half. The control's accessible name is what named-controls reads,
   and it is a different string from the visible row, so it needs its own pin. */
