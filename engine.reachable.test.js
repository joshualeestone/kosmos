...
const EXCUSED = {
  ...
  setActiveWorld: 'engine/worlds.js (#1704): the world-SWITCH write path. Slice 2a wired the rest of the registry -- create, list, and the startup env-apply all have real callers in server.js now -- but the switch API and its board-restart lifecycle are slice 2b, so this export has no caller yet. Validated here so its refusal of an unknown id and its pointer persistence are proven before 2b exposes it. Remove this excuse when 2b wires the switch flow. NB: do not name a sibling function verbatim in this reason -- this file is scanned into the tested set, so naming an unwired one would make it a newly-flagged orphan.',
};
