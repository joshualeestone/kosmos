...
if [ -z "$adopt" ]; then
  ...
  if [ "$RUNNER" = codex ]; then
    ...
    # $_eng/$NODE_BIN resolution and same posture as the codex-dismiss shim above. Claude
    # arm only: the codex arm has its own trustCodexFolder + dismiss shim, and its
