...
const FNS = page.liftAll(SCRIPT, ['agentFileRow', 'paintAgentFiles', 'paintAgentFilesAll']);
...
function harness(respond, current = card('ana')) {
  ...
  const el = {};
  for (const id of ['d-files', 'd-files-list', 'd-files-msg', 'd-files-all', 'd-filesall-list', 'd-filesall-msg', 'd-filesall-more', 'd-sec-files']) el[id] = make(id);
  el['d-sec-files'].hidden = true;
  const document = {
  ...
  const make2 = new Function('document', 'fetch', 'fileIcon', 'pjSize', 'asSentence', 'env',
    ...
    + '\nreturn { paintAgentFiles, paintAgentFilesAll, resetStamp: () => { AGENT_FILES_STAMP = null; } };');
...
test('#3757: no files (a folder not made yet, or an empty one) shows no Files section at all', async () => {
  for (const answer of [{ ok: true, missing: true, files: [], total: 0, stamp: 'missing' }, { ok: true, files: [], total: 0, stamp: 'e' }]) {
    const h = harness(() => answer);
    await h.api.paintAgentFiles('ana');
    assert.equal(h.el['d-files'].hidden, true, 'an agent with no files still shows the Files section');
    assert.equal(h.el['d-files-msg'].textContent, '', 'an empty-state sentence was written');
    assert.equal(h.el['d-files-list'].children.length, 0);
    assert.equal(h.calls[0], '/api/agent/ana/files?limit=10', 'the list does not ask for the number it shows');
  }
});
...
test('a read the server refuses shows its reason; an answer for another agent is not painted', async () => {
  ...
  assert.equal(refused.el['d-files'].hidden, false, 'a read that failed is not "no files": its reason must show');
...
test('a 404 (no folder of its own) is no files in the sidebar, and on the Files screen says so in agent-page words; reasons are sentences', async () => {
  const h = harness(() => ({ __status: 404, ok: false, because: 'there is no agent by that name on this computer' }));
  await h.api.paintAgentFiles('ana');
  assert.equal(h.el['d-files'].hidden, true, '#3757: an agent with no folder of its own showed a Files section');
  ...
  await h.api.paintAgentFilesAll('ana');
  assert.match(h.el['d-filesall-msg'].textContent, /This agent has no folder of its own on this computer/);
  assert.doesNotMatch(h.el['d-filesall-msg'].textContent, /no agent by that name/);
...
test('the click handlers: agent re-checked after the await, success clears, a failed reveal keeps its reason', () => {
  const at = SCRIPT.indexOf("for (const [listId, msgId] of [['d-files-list', 'd-files-msg'], ['d-filesall-list', 'd-filesall-msg']]) document.getElementById(listId) && document.getElementById(listId).addEventListener('click'");
...
test('opening an agent clears the previous agent\'s Files rows before its own arrive', () => {
  ...
  assert.match(body, /fa\.hidden = true/, 'the previous agent\'s View All stays');
  assert.match(body, /'d-filesall-list', 'd-filesall-msg'/, 'the previous agent\'s Files screen rows stay');
});

test('#3757: the Files screen lists them all, with Open in Finder, and says what is beyond the cap', async () => {
  const many = Array.from({ length: 12 }, (_, i) => ({ name: 'f' + i + '.txt', size: 1 }));
  const h = harness((url) => ({ ok: true, total: 520, stamp: 'm', files: /limit=500/.test(url) ? many : many.slice(0, 10) }));
  await h.api.paintAgentFilesAll('ana');
  assert.equal(h.calls[0], '/api/agent/ana/files?limit=500');
  assert.equal(h.el['d-filesall-list'].children.length, 12);
  assert.equal(h.el['d-filesall-more'].hidden, false);
  assert.match(h.el['d-filesall-more'].textContent, /And 508 more in the folder/);
  // The Finder button lives on that screen now, not in the sidebar.
  const side = PAGE.slice(PAGE.indexOf('id="d-files"'), PAGE.indexOf('</section>', PAGE.indexOf('id="d-files"')));
