...
#
# These arms are the CONTROL THAT RETURNS THE DANGEROUS ANSWER: each would fail
# if a future edit re-added session-based owner preference ("signal (b)") to the
# resolver. They touch NO resolver logic -- only the same stubbed sensors every
# arm above uses -- so this guard is additive and off the P0 install path.
