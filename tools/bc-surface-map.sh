...
# source of truth, so this helper cannot disagree with the gate about the map, and it uses
# the SAME whole-token boundary match the gate uses (test-bc-surface-map.sh cross-checks the
# agreement on the seeded checks).
#
# USAGE
