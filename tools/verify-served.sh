...
if curl -fsS "$HOST/dist/Kosmos.pkg" -o "$ptmp"; then
  ...
  if command -v pkgutil >/dev/null 2>&1; then
    # OUR identity, by team id, not any Developer ID Installer's (named once, lib/signing-identity.sh, #3643).
    . "$REPO/tools/lib/signing-identity.sh"
    _vs_inst="${KOSMOS_INSTALLER_CERT:-$KOSMOS_SIGN_INSTALLER_DEFAULT}"
    if pkgutil --check-signature "$ptmp" 2>/dev/null | grep -qF "$_vs_inst"; then say "/dist/Kosmos.pkg signature" "$_vs_inst"
