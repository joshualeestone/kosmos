...
'use strict';
/**
 * #3955: release.sh step 1c. Is web/whats-new.json the highlights for the version being cut?
 *
...
function main(argv) {
  ...
  try { raw = fs.readFileSync(file, 'utf8'); } catch {
    process.stderr.write('web/whats-new.json is missing. Write the highlights for ' + version
  ...
  try { obj = JSON.parse(raw); } catch {
    process.stderr.write('web/whats-new.json is not valid JSON.\n');
    return 1;
  }
  const bad = whatsnew.problems(obj, version);
  if (bad.length) {
    process.stderr.write('web/whats-new.json is not ready for ' + version + ':\n' + bad.map((b) => '  - ' + b).join('\n') + '\n'
      + 'Fix it and commit it before cutting, or set KOSMOS_CUT_NO_WHATS_NEW=1 to cut with no "What\'s new" window.\n');
    return 1;
  }
  process.stdout.write('web/whats-new.json: ' + obj.highlights.length + ' highlight(s) for ' + version + '\n');
  return 0;
}
