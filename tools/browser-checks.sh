...
# it extends the #1921 observed-liveness badge into the OTHER /api/accounts-fed consumers. Three
# arms -- the shared helper matrix (acctUsableLogin/acctUnknownLive across every badge + the
# badge-less fallback), the paintConnLive summary via a fetch stub (counts usable logins, EXCLUDES
# rejected -- the #874 defect), and the paintAccountPicker eligibility via a seeded ACCOUNTS global
# (a rejected current account is signed out and offers the working target; a working one is the
# control). Proven RED on the pre-fix page by observed behavior ("3 accounts connected", no move
# prompt for a rejected account); no server, so it sits in this no-URL loop.
